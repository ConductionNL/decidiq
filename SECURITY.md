...
Example — fetch the latest decidiq SBOM:

```bash
curl -sL https://github.com/ConductionNL/decidiq/releases/latest/download/sbom.cdx.json | jq .
```

Example — fetch the SBOM for a specific historical release:

```bash
curl -sL https://github.com/ConductionNL/decidiq/releases/download/v1.0.0/sbom.cdx.json | jq .
```
