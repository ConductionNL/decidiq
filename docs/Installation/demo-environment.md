...
This page gets a working Decidiq running on your own machine in two commands. You end with meetings, decisions and minutes.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/decidiq/development/decidiq-compose.yaml
```
...
==> installing decidiq <version>
==> apps present: decidiq integriq openregister thematiq
```
...
```bash
curl -s http://localhost:8614/status.php
```
...
| What | Where |
| --- | --- |
| **Decidiq** | [http://localhost:8614/apps/decidiq/](http://localhost:8614/apps/decidiq/) |
| Admin interface | [http://localhost:8614](http://localhost:8614) — `admin` / `admin` |
...
| `decidiq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing decidiq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8614/apps/decidiq/"
...
curl -s -u admin:admin "http://localhost:8614/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f decidiq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f decidiq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f decidiq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
