 ...
 * `decidiq`. Every schema this app already owns still carries
 * `application = 'decidesk'`, so the pair matches nothing — and
 ...
 *
 * Measured on a live install before this step existed: 98 schemas still under
 * `decidesk` and zero under `decidiq`, and none of their slugs collide — so
 * all 98 move. The gap is fleet-wide, not specific to one app — 231 under
 * `docudesk`, 200 under `openconnector`, 180 under `procest`, 21 under
 * `softwarecatalog`, 17 under `openbuild`, 2 each under `planix`,
 * `larpingapp` and `hrmq`.
 *
 ...
 *
 * @spec exclude No canonical spec covers the `decidesk` -> `decidiq` schema
...
class MigrateSchemaApplicationId implements IRepairStep {
	 ...
	 * @var string
	 */
	public const OLD_APP_ID = 'decidesk';
	 ...
	 * @var string
	 */
	public const NEW_APP_ID = 'decidiq';
	...
	public function getName(): string {
		return 'Move Decidiq schemas onto the decidiq application id';
	}//end getName()
