...
/**
 * Decidesk Action Authorization Service
 *
 * Implements the ADR-023 action-level authorization pattern: each controller
 * method declares an action name (e.g. "minutes.generate-draft") and delegates
 * the authorization decision to this service, which resolves the action
 * against an admin-configured matrix stored in IAppConfig.
 *
 ...
 * declared action — first-install safe posture. Admin broadens via the
 * settings UI.
 *
 * @category Service
...
class ActionAuthService
{
     ...
     * @param string $action Dot-separated action name (e.g. "minutes.generate-draft").
     *
     * @return void
     *
