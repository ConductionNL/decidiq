...
class VotingService
{
     ...
     * @param VotingRoundRules|null $roundRules           The configurable decision rules; null = all defaults
     *
     ...
     * resolving a tie under tieBreakRule 'chair-decides': the value is persisted on
     * the round BEFORE the tally so the result computation can resolve the tie, and
     * the audit trail records how it was broken. Fail closed: a casting vote on a
     * round whose tie-break rule is not 'chair-decides', or with a value other than
     * for/against, is refused. Caller-side chair authorization is enforced by
    ...
    {
        return $this->closer->close(votingRoundId: $votingRoundId, chairCasting: $chairCasting);
     ...
     * anonymisation step. It is a separate named method rather than a boolean flag
     * so that irreversible data loss is explicit at every call site.
     *
     * @param string      $votingRoundId The voting round UUID
     * @param string|null $chairCasting  Optional chair casting vote ('for'|'against') resolving a tie
     *
    ...
    {
        return $this->closer->closeAnonymised(votingRoundId: $votingRoundId, chairCasting: $chairCasting);
     ...
     *
     * - base = F + A ('exclude', default) or F + A + B ('count' — abstentions make
     *   every threshold harder).
     * - T == 0 -> 'invalid'.
     ...
     *
     * Returns aggregate vote counts, the preselected option, and no individual vote
