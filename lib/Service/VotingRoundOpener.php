...
class VotingRoundOpener
{
    ...
    public function checkQuorum(string $meetingId): bool
    {
        ...
        // ParticipantResolver, which resolves the meeting -> governance-body link and
        // the participant memberships from BOTH the structured relation list and the
        // flat field-keyed relation map produced by the standard OpenRegister object
        // API. The previous inline logic read '$meeting["relations"]' as a structured
        // list and filtered on '_relations.governance-body', neither of which matches
        // OR-object-API data, so it always counted 0 and failed closed.
     ...
     * -> body template default -> built-in default. The caller always passes
     * explicit values, so it always wins; the template only fills nulls. Unknown
     * rule values are rejected, never silently defaulted.
     *
    ...
    public function openVotingRound(
        ...
        // Parliamentary ordering (motion-amendment spec) and the lifecycle transition
        // below apply to fresh rounds only: a revote re-opens a question that was
        // already in order and never left 'voting'.
        ...
        // Preset UUIDs are validated against active memberships; the eligible ones
        // become participant relations on the round.
        ...
        // Fail-soft announcements: the activity-feed entry and the preference-aware
        // "pending vote" notifications (user-settings spec), fanned out to each
        // participant's active absence delegate.
