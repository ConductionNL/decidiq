OC.L10N.register(
    "decidiq",
    {
        "2026": "2026",
        "2027": "2027",
        "#": "#",
        "+ Action item": "+ Actiepunt",
        "+ Action item for {title}": "+ Actiepunt voor {title}",
        "1 hour before": "1 uur van tevoren",
        "1 week before": "1 week van tevoren",
        "24 hours before": "24 uur van tevoren",
        "30d": "30d",
        "4 hours before": "4 uur van tevoren",
        "48 hours before": "48 uur van tevoren",
        "7d": "7d",
        "90d": "90d",
        "@objectId": "@objectId",
        "A council with committees and factions. Motions, commitments, information letters and a regulations register.": "Een raad met commissies en fracties. Moties, toezeggingen, raadsinformatiebrieven en een verordeningenregister.",
        "A delegation needs an end date — it expires automatically.": "Een delegatie heeft een einddatum nodig — deze verloopt automatisch.",
        "A draft type cannot back new agenda items.": "Een concepttype kan geen nieuwe agendapunten dragen.",
        "A draft type cannot back new meetings.": "Een concepttype kan geen nieuwe vergaderingen dragen.",
        "A governance event (decision, meeting, vote or resolution) happened in Decidiq": "Een bestuursgebeurtenis (besluit, vergadering, stemming of resolutie) vond plaats in Decidiq",
        "A members' meeting with a board. House rules, weighted votes, audit statements and annual accounts.": "Een ledenvergadering met een bestuur. Huishoudelijk reglement, gewogen stemmen, kascommissieverklaringen en jaarrekeningen.",
        "A short setup to get this app ready. Nothing here is required; you can close it and come back later.": "Een korte installatie om deze app klaar te zetten. Niets hiervan is verplicht; je kunt dit sluiten en later terugkomen.",
        "A sign-off has escalated to you": "Er is een parafering naar jou geëscaleerd",
        "A sign-off is waiting, on behalf of a colleague": "Er wacht een parafering, namens een collega",
        "A sign-off passed its term and was taken as approved": "Een parafering verstreek en is als akkoord gelezen",
        "A sign-off passed its term and was taken as refused": "Een parafering verstreek en is als afwijzing gelezen",
        "A step your colleague was asked to sign passed its term without an answer. Silence escalates there, so it is now yours, with a new term.": "Een stap die je collega moest tekenen verstreek zonder antwoord. De stap legt vast dat stilte escaleert, dus hij is nu van jou, met een nieuwe termijn.",
        "A supervisory board, an executive board and a management team. Terms, conflicts of interest and evaluations.": "Een raad van commissarissen, een raad van bestuur en een managementteam. Zittingstermijnen, belangenverstrengeling en evaluaties.",
        "A works council inside a company. Advice requests, consent requests and staff consultation.": "Een ondernemingsraad binnen een bedrijf. Adviesaanvragen, instemmingsaanvragen en achterbanraadpleging.",
        "AI": "AI",
        "AI generated": "AI-gegenereerd",
        "AI-generated draft": "Door AI gegenereerd concept",
        "Aangenomen": "Aangenomen",
        "Aanlever-deadline": "Aanleverdeadline",
        "Aanlever-deadline default": "Standaard aanleverdeadline",
        "Abandoned": "Losgelaten",
        "Absent from": "Afwezig vanaf",
        "Absent until (delegation expires automatically)": "Afwezig tot (delegatie verloopt automatisch)",
        "Abstain": "Onthouding",
        "Abstention handling": "Behandeling van onthoudingen",
        "Abstentions": "Onthoudingen",
        "Abstentions count toward base": "Onthoudingen tellen mee in de basis",
        "Abstentions excluded from base": "Onthoudingen tellen niet mee in de basis",
        "Accept": "Accepteren",
        "Accept correction": "Correctie accepteren",
        "Accepted": "Geaccepteerd",
        "Access level": "Toegangsniveau",
        "Accession date": "Datum van toetreding",
        "Accession withdrawal": "Toetreding uittreding",
        "Account": "Account",
        "Account id of the person who confirmed the exit. Optional until confirmed.": "Account-id van de persoon die het vertrek heeft bevestigd. Optioneel tot het bevestigd is.",
        "Account linking": "Account koppeling",
        "Account matching failed (admin access required).": "Account koppelen mislukt (beheerderstoegang vereist).",
        "Account matching failed.": "Account koppelen mislukt.",
        "Accountability": "Verantwoording",
        "Accountability date": "Datum verantwoording",
        "Accounted for": "Verantwoord",
        "Achieved": "Behaald",
        "Achterbanraadpleging": "Achterbanraadpleging",
        "Action": "Actie",
        "Action Item Completion Rate": "Afrondingsgraad actiepunten",
        "Action Items": "Actiepunten",
        "Action item": "Actiepunt",
        "Action item assigned": "Actiepunt toegewezen",
        "Action item completion %": "Actiepunten afgerond %",
        "Action item completion rate": "Afrondingsgraad actiepunten",
        "Action item title": "Titel actiepunt",
        "Action items": "Actiepunten",
        "Action taken": "Ondernomen actie",
        "Actions": "Acties",
        "Activate agenda item": "Agendapunt activeren",
        "Activate item": "Item activeren",
        "Activate {title}": "{title} activeren",
        "Active": "Actief",
        "Active agenda item": "Actief agendapunt",
        "Active decisions": "Actieve besluiten",
        "Active {title}": "Actief: {title}",
        "Active: {title}": "Actief: {title}",
        "Actor": "Actor",
        "Actor type": "Type actor",
        "Actual Duration": "Werkelijke duur",
        "Actual duration": "Werkelijke duur",
        "Actual notice (hours)": "Werkelijke oproeptermijn (uren)",
        "Add a sub-item under \"{title}\".": "Voeg een subpunt toe onder \"{title}\".",
        "Add action item": "Actiepunt toevoegen",
        "Add action item for {title}": "Actiepunt toevoegen voor {title}",
        "Add agenda item": "Agendapunt toevoegen",
        "Add co-author": "Mede-auteur toevoegen",
        "Add comment": "Reactie toevoegen",
        "Add integration": "Integratie toevoegen",
        "Add member": "Lid toevoegen",
        "Add motion": "Motie toevoegen",
        "Add participant": "Deelnemer toevoegen",
        "Add recurring items": "Terugkerende agendapunten toevoegen",
        "Add related decision": "Gerelateerd besluit toevoegen",
        "Add relation": "Relatie toevoegen",
        "Add selected": "Geselecteerde toevoegen",
        "Add signer": "Ondertekenaar toevoegen",
        "Add speaker to queue": "Spreker aan wachtrij toevoegen",
        "Add state": "Status toevoegen",
        "Add sub-item": "Subpunt toevoegen",
        "Add sub-item under {title}": "Subpunt toevoegen onder {title}",
        "Add to queue": "Aan wachtrij toevoegen",
        "Add transition": "Overgang toevoegen",
        "Added text": "Toegevoegde tekst",
        "Adding…": "Toevoegen…",
        "Additional fields items of this kind carry.": "Extra velden die punten van dit soort bevatten.",
        "Additional templates": "Aanvullende sjablonen",
        "Adjourned": "Verdaagd",
        "Admitted": "Toegelaten",
        "Adopt all consent agenda items": "Alle hamerstukken vaststellen",
        "Adopt consent agenda": "Hamerstukken vaststellen",
        "Adopted": "Aangenomen",
        "Adopting body": "Vaststellend orgaan",
        "Adoption date": "Vaststellingsdatum",
        "Advance phase": "Volgende fase",
        "Advance to next BOB phase for {title}": "Naar volgende BOB-fase voor {title}",
        "Adverse": "Afkeurend",
        "Advice": "Advies",
        "Advice date": "Adviesdatum",
        "Advice document": "Adviesdocument",
        "Advice issued": "Advies uitgebracht",
        "Advice request": "Adviesaanvraag",
        "Advice with conditions": "Advies met voorwaarden",
        "Advisory": "Adviserend",
        "Advisory body": "Adviesorgaan",
        "Advisory opinion": "Advies",
        "Advisory opinions": "Adviezen",
        "Advisory request": "Adviesaanvraag",
        "Advisory stage": "Adviesfase",
        "Afdoening evidence": "Bewijs van afdoening",
        "Afdoening note": "Toelichting bij afdoening",
        "Affirmation": "Belofte",
        "Against": "Tegen",
        "Agenda": "Agenda",
        "Agenda Item": "Agendapunt",
        "Agenda Items": "Agendapunten",
        "Agenda builder": "Agendabouwer",
        "Agenda completion": "Agenda-afronding",
        "Agenda item": "Agendapunt",
        "Agenda item integrations": "Agendapuntintegraties",
        "Agenda item title": "Agendapunttitel",
        "Agenda item type": "Agendapunttype",
        "Agenda item types": "Soorten agendapunten",
        "Agenda item types admitted on this kind of meeting.": "Agendapunttypen die op dit soort vergadering zijn toegestaan.",
        "Agenda item {n}: {title}": "Agendapunt {n}: {title}",
        "Agenda items": "Agendapunten",
        "Agenda items ({n})": "Agendapunten ({n})",
        "Agenda items, drag to reorder": "Agendapunten, sleep om te herordenen",
        "Agenda policy": "Agendabeleid",
        "Agenda-item notes": "Aantekeningen bij agendapunt",
        "Agendised": "Geagendeerd",
        "Akte article": "Akte-artikel",
        "Aligned at": "Afgestemd op",
        "All": "Alles",
        "All changes saved": "Alle wijzigingen opgeslagen",
        "All connections": "Alle koppelingen",
        "All participants already added as signers.": "Alle deelnemers zijn al toegevoegd als ondertekenaars.",
        "All set": "Klaar",
        "All statuses": "Alle statussen",
        "All urgent": "Alle spoedbesluiten",
        "Allocated time (minutes)": "Toegewezen tijd (minuten)",
        "Allow decide without vote": "Besluiten zonder stemming toestaan",
        "Allowed agenda item types": "Toegestane agendapunttypen",
        "Allowed hold types": "Toegestane bekledingstypen",
        "Allowed trigger roles": "Toegestane triggerrollen",
        "Already a member — skipped": "Al lid — overgeslagen",
        "Amended by": "Gewijzigd door",
        "Amendement indienen": "Amendement indienen",
        "Amendementen": "Amendementen",
        "Amendment": "Amendement",
        "Amendment internal regulations": "Wijziging huishoudelijk reglement",
        "Amendment of arrangement": "Wijziging regeling",
        "Amendment text": "Amendementtekst",
        "Amendments": "Amendementen",
        "Amendments are voted before the main motion, most far-reaching first. Only the chair can save the order.": "Over amendementen wordt vóór de hoofdmotie gestemd, het meest verstrekkende eerst. Alleen de voorzitter kan de volgorde opslaan.",
        "Amends": "Wijzigt",
        "Amount Delta (€)": "Bedragwijziging (€)",
        "An existing person with this email will be reused; otherwise a new one is created.": "Een bestaande persoon met dit e-mailadres wordt hergebruikt; anders wordt een nieuwe persoon aangemaakt.",
        "An opaque identifier for the respondent, used when answers are anonymous or the respondent is not a recorded person.": "Een niet-herleidbare aanduiding van de respondent, gebruikt als reacties anoniem zijn of de respondent geen vastgelegd persoon is.",
        "Ancillary positions intake": "Nevenfuncties intake",
        "Annual accounts": "Jaarrekening",
        "Annual report": "Jaarverslag",
        "Annuleren": "Annuleren",
        "Anonymous intake rate limit (per hour)": "Limiet anonieme inname (per uur)",
        "Anonymous reactions allowed": "Anonieme reacties toegestaan",
        "Anonymous responses": "Anonieme reacties",
        "Answer": "Antwoord",
        "Answer summary": "Samenvatting van het antwoord",
        "Answered": "Beantwoord",
        "Answered by": "Beantwoord door",
        "Answered on": "Beantwoord op",
        "Answers": "Antwoorden",
        "Any background the parties need to answer.": "De achtergrond die de partijen nodig hebben om te kunnen antwoorden.",
        "Any other business": "Rondvraag",
        "Any qualification or remark the committee recorded.": "Elke voorbehoud of opmerking die de commissie heeft vastgelegd.",
        "App id of the fleet app this route was held on behalf of (e.g. dossiq). Empty for routes authored in decidiq itself. Half of the idempotency key; never an authorization input.": "App-id van de applicatie waarvoor deze route wordt bewaard (bijvoorbeeld dossiq). Leeg voor routes die in decidiq zelf zijn gemaakt. De helft van de idempotentiesleutel; nooit een autorisatiegrond.",
        "Appointed": "Benoemd",
        "Appointed by": "Benoemd door",
        "Appointed memberships": "Benoemde lidmaatschappen",
        "Appointment": "Benoeming",
        "Approval": "Vaststelling",
        "Approval of previous minutes": "Vaststelling notulen vorige vergadering",
        "Approval workflow": "Goedkeuringsproces",
        "Approval workflow error": "Fout in goedkeuringsproces",
        "Approve": "Goedkeuren",
        "Approve or reject citizen reactions before they count toward a consultation.": "Keur burgerreacties goed of af voordat ze meetellen voor een consultatie.",
        "Approve proposal {title}": "Voorstel goedkeuren: {title}",
        "Approve reaction": "Reactie goedkeuren",
        "Approved": "Goedgekeurd",
        "Approved at": "Goedgekeurd op",
        "Approved at {date} by {names}": "Goedgekeurd op {date} door {names}",
        "Approving": "Goedkeurend",
        "Approving counts this reaction toward the consultation and allows it to be published.": "Goedkeuren laat deze reactie meetellen voor de consultatie en maakt publicatie mogelijk.",
        "Archival retention period (days)": "Archiveringsbewaartermijn (dagen)",
        "Archive": "Archiveren",
        "Archived": "Gearchiveerd",
        "Arrangement": "Regeling",
        "Article": "Artikel",
        "Articles of association": "Statuten",
        "Asked by": "Gesteld door",
        "Asked on": "Gesteld op",
        "Asking body": "Vragend orgaan",
        "Assemble meeting package": "Vergaderstukken samenstellen",
        "Assembles convocation, quorum, voting results, and the adopted decision texts into a tamper-evident package in the meeting folder.": "Bundelt oproeping, quorum, stemuitslagen en de aangenomen besluitteksten tot een manipulatiebestendig pakket in de vergadermap.",
        "Assembling…": "Samenstellen…",
        "Assign a role to {name}.": "Wijs een rol toe aan {name}.",
        "Assign groups": "Groepen toewijzen",
        "Assign spokesperson": "Spreker toewijzen",
        "Assign spokesperson for {title}": "Spreker toewijzen voor {title}",
        "Assigned body": "Toegewezen orgaan",
        "Assigned person": "Toegewezen persoon",
        "Assignee": "Toegewezen aan",
        "Association (assoc)": "Vereniging (assoc)",
        "Association or VvE": "Vereniging of VvE",
        "At max term": "Op maximale termijn",
        "At most {max} rows can be imported at once.": "Er kunnen maximaal {max} regels tegelijk worden geïmporteerd.",
        "At risk": "Risico",
        "Attach recording": "Opname koppelen",
        "Attach with consent": "Koppelen met toestemming",
        "Attaching a recording for transcription requires confirming that all participants were informed that the meeting was recorded (AVG/GDPR). The recording and raw transcript stay restricted to this governance body and are never published.": "Het koppelen van een opname voor transcriptie vereist bevestiging dat alle deelnemers zijn geïnformeerd dat de vergadering werd opgenomen (AVG). De opname en het ruwe transcript blijven beperkt tot dit bestuursorgaan en worden nooit gepubliceerd.",
        "Attachments": "Bijlagen",
        "Attendance": "Aanwezigheid",
        "Attendance mode": "Wijze van deelname",
        "Attendance rate": "Aanwezigheidspercentage",
        "Attendance status": "Aanwezigheidsstatus",
        "Audience": "Doelgroep",
        "Audience body": "Doelorgaan",
        "Audience group": "Doelgroep",
        "Audience size": "Omvang van het publiek",
        "Audience type": "Type publiek",
        "Audience user group": "Gebruikersgroep",
        "Audit statement": "Kascommissie verklaring",
        "Audit statements": "Kascommissie verklaringen",
        "AuditStatement": "AuditStatement",
        "Author": "Auteur",
        "Author name": "Naam van de auteur",
        "Authorisation": "Machtiging",
        "Authorisation above threshold": "Machtiging boven drempel",
        "Authority delegation": "Bevoegdheidstoedeling",
        "Authority delegations": "Bevoegdheidstoedelingen",
        "Authorization instrument": "Machtigingsinstrument",
        "Authorizing decision": "Machtigend besluit",
        "Autosave failed — retrying on next edit": "Automatisch opslaan mislukt — nieuwe poging bij de volgende bewerking",
        "Availability": "Beschikbaarheid",
        "Available transitions": "Beschikbare overgangen",
        "Average actual duration: {minutes} min": "Gemiddelde werkelijke duur: {minutes} min",
        "Awaiting ratification": "Wacht op bekrachtiging",
        "Award criteria": "Gunningscriteria",
        "Awarded to": "Gegund aan",
        "BOB phase": "BOB-fase",
        "BOB phase for {title}": "BOB-fase voor {title}",
        "BOB phase progression": "Voortgang BOB-fase",
        "Back": "Terug",
        "Back to agenda item": "Terug naar agendapunt",
        "Back to boards": "Terug naar besturen",
        "Back to decision": "Terug naar besluit",
        "Back to meeting": "Terug naar vergadering",
        "Back to meeting detail": "Terug naar vergaderdetail",
        "Back to meetings": "Terug naar vergaderingen",
        "Back to motion": "Terug naar motie",
        "Back to resolutions": "Terug naar besluiten",
        "Background": "Achtergrond",
        "Bedrag delta": "Bedrag delta",
        "Beeldvorming": "Beeldvorming",
        "Begrotingspost": "Begrotingspost",
        "Begrotingswijziging": "Begrotingswijziging",
        "Bekrachtiging deadline": "Bekrachtigingstermijn",
        "Beleidsregel": "Beleidsregel",
        "Besluitvorming": "Besluitvorming",
        "Bestuurder": "Bestuurder",
        "Bestuursorgaan (TOOI id)": "Bestuursorgaan (TOOI-id)",
        "Binding": "Bindend",
        "Blocked": "Geblokkeerd",
        "Board election": "Bestuursverkiezing",
        "Board elections": "Bestuursverkiezing",
        "Board meetings": "Bestuursvergaderingen",
        "Board name": "Bestuursnaam",
        "Board not found": "Bestuur niet gevonden",
        "Board self-evaluation": "Zelfevaluatie bestuur",
        "Boards": "Besturen",
        "Bodies": "Organen",
        "Body": "Orgaan",
        "Body composition": "Samenstelling van een orgaan",
        "Body compositions": "Samenstelling van organen",
        "Body configuration": "Orgaanconfiguratie",
        "Body details": "Gegevens bestuursorgaan",
        "Body members": "Leden van het orgaan",
        "Body-wide": "Orgaanbreed",
        "Boekjaar": "Boekjaar",
        "Both": "Beide",
        "Budget": "Begroting",
        "Budget Impact": "Begrotingsimpact",
        "Budget Line": "Begrotingslijn",
        "Budget amendment": "Begrotingswijziging",
        "Budget document": "Begrotingsstuk",
        "Budget round": "Begrotingsronde",
        "Budget round updated": "Begrotingsronde bijgewerkt",
        "Built-in": "Ingebouwd",
        "Bulk turnover": "Wisselronde",
        "By law": "Verordening",
        "By method": "Op methode",
        "By outcome": "Op uitkomst",
        "By priority": "Op prioriteit",
        "By result": "Op resultaat",
        "By stage": "Op fase",
        "By status": "Op status",
        "By type": "Op type",
        "By when an answer is expected.": "Wanneer een reactie uiterlijk wordt verwacht.",
        "COI ({n})": "BV ({n})",
        "CSV file": "CSV-bestand",
        "CVDR": "CVDR",
        "Calendar": "Kalender",
        "Cancel": "Annuleren",
        "Cannot publish: no agenda items.": "Kan niet publiceren: geen agendapunten.",
        "Cast": "Uitgebracht",
        "Cast at": "Uitgebracht op",
        "Cast your vote": "Breng uw stem uit",
        "Cast: {cast} / {total}": "Uitgebracht: {cast} / {total}",
        "Casting vote failed": "Beslissende stem uitbrengen mislukt",
        "Casting vote: against": "Beslissende stem: tegen",
        "Casting vote: for": "Beslissende stem: voor",
        "Catalog id": "Catalogus-id",
        "Categories where this body needs a different majority than its templates use by default.": "Categorieën waarvoor dit orgaan een andere meerderheid nodig heeft dan de sjablonen standaard hanteren.",
        "Category": "Categorie",
        "Chair": "Voorzitter",
        "Chair only": "Alleen voorzitter",
        "Change it in your personal settings.": "Wijzig dit in je persoonlijke instellingen.",
        "Change role": "Rol wijzigen",
        "Change spokesperson": "Spreker wijzigen",
        "Channel": "Kanaal",
        "Child goal achieved rate": "Behaalgraad subdoelen",
        "Choose what happens to this body's meeting recordings and raw transcripts after the minutes are approved. The approved minutes always remain the official record.": "Kies wat er gebeurt met de vergaderopnames en ruwe transcripten van dit orgaan nadat de notulen zijn goedgekeurd. De goedgekeurde notulen blijven altijd het officiële verslag.",
        "Choose which Decidiq events notify you and how they are delivered.": "Kies over welke Decidiq-gebeurtenissen je meldingen ontvangt en hoe ze worden bezorgd.",
        "Citation": "Citeertitel",
        "Citeertitel": "Citeertitel",
        "Citing decisions": "Verwijzende besluiten",
        "Citizen participation": "Burgerparticipatie",
        "Citizen participation defaults": "Standaardwaarden burgerparticipatie",
        "Citizen portal (citizen)": "Burgerportaal (citizen)",
        "Clear delegation": "Delegatie wissen",
        "Click Meetings in the menu": "Klik op Vergaderingen in het menu",
        "Click New and save a meeting": "Klik op Nieuw en sla een vergadering op",
        "Close": "Sluiten",
        "Close At (optional)": "Sluiten om (optioneel)",
        "Close Voting Round": "Stemronde sluiten",
        "Close agenda item": "Agendapunt afsluiten",
        "Close cycle": "Cyclus afsluiten",
        "Close item": "Item sluiten",
        "Close the voting round now? Members who have not voted yet will not be counted.": "Stemronde nu sluiten? Leden die nog niet hebben gestemd tellen niet mee.",
        "Close voting round": "Stemronde sluiten",
        "Close voting round? {notVoted} of {total} members have not voted yet.": "Stemronde sluiten? {notVoted} van {total} leden hebben nog niet gestemd.",
        "Closed": "Gesloten",
        "Closes": "Sluit",
        "Closing": "Sluiting",
        "Closing time (optional)": "Sluitingstijd (optioneel)",
        "Co-Signatories": "Medeondertekenaars",
        "Co-authors": "Mede-auteurs",
        "Comma-separated dates, e.g. 2026-07-14, 2026-08-11": "Kommagescheiden data, bijv. 2026-07-14, 2026-08-11",
        "Comment": "Reactie",
        "Comments": "Reacties",
        "Commitment": "Toezegging",
        "Commitment settlement rate": "Afhandelingsgraad toezeggingen",
        "Commitments": "Toezeggingen",
        "Commitments over deadline": "Toezeggingen over de termijn",
        "Committee": "Commissie",
        "Communication": "Communicatie",
        "Communication preferences": "Communicatievoorkeuren",
        "Communication preferences saved.": "Communicatievoorkeuren opgeslagen.",
        "Company board": "Raad van bestuur",
        "Competent body": "Bevoegd orgaan",
        "Competent positions": "Bevoegde functies",
        "Completed": "Afgerond",
        "Component body": "Samenstellend orgaan",
        "Composite body": "Samengesteld orgaan",
        "Composition type": "Samenstellingstype",
        "Concept": "Concept",
        "Conclude vote": "Stemming afronden",
        "Confidential": "Vertrouwelijk",
        "Confidentiality": "Geheimhouding",
        "Confidentiality ground": "Geheimhoudingsgrond",
        "Confidentiality grounds": "Geheimhoudingsgronden",
        "Confidentiality register": "Geheimhoudingsregister",
        "Confidentiality restriction": "Geheimhouding",
        "Confidentiality restrictions": "Geheimhoudingen",
        "Confidentiality status timeline": "Tijdlijn geheimhoudingsstatus",
        "Configuration": "Configuratie",
        "Configure the OpenRegister schema mappings for all Decidiq object types.": "Configureer de OpenRegister schema-koppelingen voor alle Decidiq objecttypes.",
        "Configure the app settings": "Configureer de app-instellingen",
        "Configure, per governance body, where adopted decisions, public agendas, and approved minutes are published. Anonymous read access is served exclusively through OpenCatalogi / OpenRegister — never an app-local public page.": "Configureer per bestuursorgaan waar aangenomen besluiten, openbare agenda’s en goedgekeurde notulen worden gepubliceerd. Anonieme toegang verloopt uitsluitend via OpenCatalogi / OpenRegister — nooit via een app-eigen openbare pagina.",
        "Configured": "Geconfigureerd",
        "Confirm": "Bevestigen",
        "Confirm adoption": "Vaststelling bevestigen",
        "Confirm recording consent": "Bevestig toestemming voor opname",
        "Confirmed": "Bevestigd",
        "Conflict of interest": "Belangenverstrengeling",
        "Conflict of interest declarations": "Verklaringen belangenverstrengeling",
        "Connection": "Koppeling",
        "Consent agenda items": "Hamerstukken",
        "Consent agenda items (hamerstukken)": "Hamerstukken",
        "Consent granted": "Instemming verleend",
        "Consent refused": "Instemming geweigerd",
        "Consent request": "Instemmingsverzoek",
        "Consolidated text": "Geconsolideerde tekst",
        "Constituency consultation": "Achterbanraadpleging",
        "Constitutive document": "Oprichtingsdocument",
        "Consultation": "Consultatie",
        "Consultation response": "Reactie op raadpleging",
        "Consultation responses": "Reacties op raadplegingen",
        "Consultation updated": "Consultatie bijgewerkt",
        "Consultations": "Consultaties",
        "Contact methods": "Contactmethoden",
        "Content": "Inhoud",
        "Context": "Context",
        "Contract": "Contract",
        "Contract renewal": "Contractverlenging",
        "Contrary to advice": "Afwijkend van advies",
        "Control how Decidiq presents itself for your account.": "Bepaal hoe Decidiq zich voor jouw account presenteert.",
        "Controls mode-specific labels across the app — for example what a governance body is called. Default: government.": "Bepaalt de modusspecifieke labels in de hele app — bijvoorbeeld hoe een bestuursorgaan heet. Standaard: overheid.",
        "Corporate (corp)": "Bedrijf (corp)",
        "Corporate-governance extension (beyond Popolo, MCCG): board-member independence, mode=corp. NOT the same question as `external`, which records whether the member sits from outside the administrative organ. A member can be external and non-independent, or internal and independent; neither value is derivable from the other.": "Corporate-governance-uitbreiding (buiten Popolo, MCCG): onafhankelijkheid van bestuursleden, mode=corp. NIET dezelfde vraag als `external`, dat vastlegt of het lid van buiten het bestuursorgaan komt. Een lid kan extern én niet-onafhankelijk zijn, of intern én onafhankelijk; geen van beide volgt uit de ander.",
        "Correction": "Correctie",
        "Correction suggestions": "Correctievoorstellen",
        "Cost per agenda item": "Kosten per agendapunt",
        "Cost trend": "Kostentrend",
        "Could not approve the reaction": "Kon de reactie niet goedkeuren",
        "Could not attach the recording.": "Kon de opname niet koppelen.",
        "Could not create decision.": "Besluit aanmaken mislukt.",
        "Could not create minutes.": "Notulen aanmaken mislukt.",
        "Could not create or match a person.": "Kon geen persoon aanmaken of matchen.",
        "Could not create proposal.": "Kon voorstel niet aanmaken.",
        "Could not create the action item.": "Het actiepunt kon niet worden aangemaakt.",
        "Could not generate the draft.": "Kon het concept niet genereren.",
        "Could not load action items": "Kon actiepunten niet laden",
        "Could not load agenda items": "Kon agendapunten niet laden",
        "Could not load amendments": "Kon amendementen niet laden",
        "Could not load analytics": "Kon analyses niet laden",
        "Could not load decision-making.": "Kon besluitvorming niet laden.",
        "Could not load decisions": "Besluiten konden niet worden geladen",
        "Could not load evaluations": "Kon evaluaties niet laden",
        "Could not load group members.": "Kon groepsleden niet laden.",
        "Could not load groups (admin access required).": "Kon groepen niet laden (beheerderstoegang vereist).",
        "Could not load groups.": "Kon groepen niet laden.",
        "Could not load members": "Kon leden niet laden",
        "Could not load minutes": "Notulen konden niet worden geladen",
        "Could not load motions": "Kon moties niet laden",
        "Could not load parent motion": "Kon ouder-motie niet laden",
        "Could not load participants": "Kon deelnemers niet laden",
        "Could not load participation rounds": "Kon participatierondes niet laden",
        "Could not load related decisions": "Gerelateerde besluiten konden niet worden geladen",
        "Could not load route": "Route kon niet worden geladen",
        "Could not load routed documents": "Kon doorgestuurde documenten niet laden",
        "Could not load signers": "Kon ondertekenaars niet laden",
        "Could not load template assignment": "Kon sjabloontoewijzing niet laden",
        "Could not load the diff": "Kon het tekstverschil niet laden",
        "Could not load the moderation queue": "Kon de moderatiewachtrij niet laden",
        "Could not load the ondermandaat chain": "Kon de ondermandaatketen niet laden",
        "Could not load transcription state.": "Kon de transcriptiestatus niet laden.",
        "Could not load versions": "Kon versies niet laden",
        "Could not load votes": "Kon stemmen niet laden",
        "Could not load voting overview": "Stemmingsoverzicht kon niet worden geladen",
        "Could not load voting results": "Stemresultaten konden niet worden geladen",
        "Could not re-align the transcript.": "Kon het transcript niet opnieuw uitlijnen.",
        "Could not record your vote": "Kon uw stem niet registreren",
        "Could not reject the reaction": "Kon de reactie niet afkeuren",
        "Could not save retention policy": "Kon bewaarbeleid niet opslaan",
        "Could not save the retention policy.": "Kon het bewaarbeleid niet opslaan.",
        "Could not start transcription.": "Kon de transcriptie niet starten.",
        "Could not submit your proposal": "Kon uw voorstel niet indienen",
        "Could not submit your reaction": "Kon uw reactie niet indienen",
        "Could not update status.": "Kon status niet bijwerken.",
        "Council information letter": "Raadsinformatiebrief",
        "Council information letters": "Raadsinformatiebrieven",
        "Council proposal": "Raadsvoorstel",
        "Council registry": "Griffie",
        "Council turnover batch": "Raadswisseling batch",
        "Count": "Aantal",
        "Countersign": "Medeondertekening",
        "Counts only": "Alleen aantallen",
        "Create": "Aanmaken",
        "Create Agenda Item": "Agendapunt aanmaken",
        "Create Decision": "Besluit aanmaken",
        "Create Governance Body": "Bestuursorgaan aanmaken",
        "Create Meeting": "Vergadering aanmaken",
        "Create Participant": "Deelnemer aanmaken",
        "Create board": "Bestuur aanmaken",
        "Create decision": "Besluit aanmaken",
        "Create meeting": "Vergadering aanmaken",
        "Create minutes": "Notulen aanmaken",
        "Create process template": "Processjabloon aanmaken",
        "Create proposal for this object": "Voorstel aanmaken voor dit object",
        "Create template": "Sjabloon aanmaken",
        "Create your first board to get started.": "Maak je eerste bestuur aan om te beginnen.",
        "Currency": "Valuta",
        "Current": "Huidig",
        "Cycle step": "Stap in de cyclus",
        "Dashboard": "Dashboard",
        "Date": "Datum",
        "Date after which this type may no longer be used. Empty means indefinitely.": "Datum waarna dit type niet meer gebruikt mag worden. Leeg betekent onbeperkt.",
        "Date format": "Datumnotatie",
        "Date from which this type may be used.": "Datum vanaf wanneer dit type gebruikt mag worden.",
        "Date the membership ends. The end-membership step sets Membership.endDate to this value.": "Datum waarop het lidmaatschap eindigt. De stap end-membership zet Membership.endDate op deze waarde.",
        "Date the person was formally installed. Completing the installation step requires this, installationType and installationMeeting.": "Datum waarop de persoon formeel is geïnstalleerd. Om de installatiestap af te ronden zijn deze datum, installationType en installationMeeting nodig.",
        "Days after approval": "Dagen na goedkeuring",
        "De datum waarop het besluit is gepubliceerd. Onderscheiden van `publishedAt`, dat het moment van publiceren in dit systeem vastlegt; dit is de datum die het besluit zelf draagt. BRC `publicatiedatum`.": "De datum waarop het besluit is gepubliceerd. Onderscheiden van `publishedAt`, dat het moment van publiceren in dit systeem vastlegt; dit is de datum die het besluit zelf draagt. BRC `publicatiedatum`.",
        "De datum waarop het besluit is verzonden aan de belanghebbende. BRC `verzenddatum`.": "De datum waarop het besluit is verzonden aan de belanghebbende. BRC `verzenddatum`.",
        "De datum waarop het besluit vervalt. Onderscheiden van `effectiveDate`, die zegt wanneer het ingaat. BRC `vervaldatum`.": "De datum waarop het besluit vervalt. Onderscheiden van `effectiveDate`, die zegt wanneer het ingaat. BRC `vervaldatum`.",
        "Deadline": "Deadline",
        "Deadline {date}": "Deadline {date}",
        "Death": "Overlijden",
        "Debat": "Debat",
        "Debat openen": "Debat openen",
        "Debate": "Debat",
        "Decided": "Besloten op",
        "Decidiq": "Decidiq",
        "Decidiq governance": "Decidiq bestuur",
        "Decidiq personal settings": "Persoonlijke Decidiq-instellingen",
        "Decidiq settings": "Decidiq instellingen",
        "Decision": "Besluit",
        "Decision \"%1$s\" was published": "Besluit \"%1$s\" is gepubliceerd",
        "Decision \"%1$s\" was recorded": "Besluit \"%1$s\" is vastgelegd",
        "Decision date": "Besluitdatum",
        "Decision integrations": "Besluitintegraties",
        "Decision list": "Besluitenlijst",
        "Decision maker": "Besluitvormer",
        "Decision outcome recorded when status=decided. Null when status is pending/active/skipped. `returned` is the sign-off case and behaves unlike the rest: it re-opens an EARLIER stage rather than ending this one, which is why `rejected` and `deferred` could not express it, both end a stage and neither reopens one.": "Uitkomst van het besluit, vastgelegd bij status=decided. Leeg bij pending/active/skipped. `returned` is het parafering-geval en gedraagt zich anders dan de rest: het heropent een EERDERE stap in plaats van deze af te sluiten — daarom konden `rejected` en `deferred` het niet uitdrukken, want beide sluiten een stap af en heropenen er geen.",
        "Decision policy": "Besluitbeleid",
        "Decision published": "Besluit gepubliceerd",
        "Decision received": "Besluit ontvangen",
        "Decision requested by %1$s": "Besluit aangevraagd door %1$s",
        "Decision requested by %1$s for %2$s.": "Besluit aangevraagd door %1$s voor %2$s.",
        "Decision requested by %1$s.": "Besluit aangevraagd door %1$s.",
        "Decision template": "Besluitsjabloon",
        "Decision templates": "Besluitsjablonen",
        "Decision type": "Besluittype",
        "Decision types this position is competent to decide.": "Besluittypen waarover deze functie bevoegd is te beslissen.",
        "Decision {object} was published": "Besluit {object} is gepubliceerd",
        "Decision {object} was recorded": "Besluit {object} is vastgelegd",
        "Decisions": "Besluiten",
        "Decisions awaiting your vote": "Besluiten die op uw stem wachten",
        "Decisions taken on this item…": "Besluiten over dit agendapunt…",
        "Decisions, Tasks and Commitments, your Organisation and shared Registers are one click away in the menu. The documentation covers the rest.": "Besluiten, Taken en toezeggingen, je Organisatie en gedeelde Registers staan één klik verderop in het menu. De documentatie behandelt de rest.",
        "Deck board error": "Deck-bordfout",
        "Declare conflict of interest": "Belangenverstrengeling melden",
        "Declare conflict of interest for this agenda item": "Belangenverstrengeling melden voor dit agendapunt",
        "Declared": "Opgegeven",
        "Deed date": "Aktedatum",
        "Deed of division": "Splitsingsakte",
        "Deelnemer UUID": "Deelnemer UUID",
        "Default OpenCatalogi catalog (UUID)": "Standaard OpenCatalogi-catalogus (UUID)",
        "Default duration (minutes)": "Standaardduur (minuten)",
        "Default language": "Standaardtaal",
        "Default majority": "Standaardmeerderheid",
        "Default moderation policy": "Standaard moderatiebeleid",
        "Default process template": "Standaard processjabloon",
        "Default quorum": "Standaardquorum",
        "Default quorum rule": "Standaardquorumregel",
        "Default view": "Standaardweergave",
        "Default vote threshold": "Standaardstemdrempel",
        "Default voting rule": "Standaard stemregel",
        "Default: 24 hours and 1 hour before the meeting.": "Standaard: 24 uur en 1 uur voor de vergadering.",
        "Defaults applied to new consultations and budget rounds. Staff can override per round.": "Standaardwaarden voor nieuwe consultaties en begrotingsrondes. Medewerkers kunnen per ronde afwijken.",
        "Deferred": "Aangehouden",
        "Define the state machine, voting rule and quorum policy a governance body follows. Built-in templates are read-only but can be duplicated.": "Definieer de toestandsmachine, stemregel en quorumbeleid die een bestuursorgaan volgt. Ingebouwde sjablonen zijn alleen-lezen maar kunnen worden gedupliceerd.",
        "Delegans": "Delegans",
        "Delegans: {who}": "Delegans: {who}",
        "Delegataris": "Delegataris",
        "Delegataris: {who}": "Delegataris: {who}",
        "Delegate": "Gedelegeerde",
        "Delegate body": "Gemandateerd orgaan",
        "Delegate task": "Taak delegeren",
        "Delegatie": "Delegatie",
        "Delegating body": "Mandaterend orgaan",
        "Delegating party description": "Omschrijving van de mandaterende partij",
        "Delegation": "Delegatie",
        "Delegation / mandate": "Delegatie of mandaat",
        "Delegation and absence": "Delegatie en afwezigheid",
        "Delegation does not include voting rights. A formal proxy (volmacht) is required for voting.": "Delegatie omvat geen stemrecht. Voor stemmen is een formele volmacht vereist.",
        "Delegation saved.": "Delegatie opgeslagen.",
        "Delegations": "Delegaties",
        "Delegations & mandates": "Delegaties en mandaten",
        "Delegator": "Delegator",
        "Delete": "Verwijderen",
        "Delete action item": "Actiepunt verwijderen",
        "Delete agenda item": "Agendapunt verwijderen",
        "Delete amendment": "Amendement verwijderen",
        "Delete failed.": "Verwijderen mislukt.",
        "Delete motion": "Motie verwijderen",
        "Delete recording and transcript": "Opname en transcript verwijderen",
        "Delete recording only": "Alleen opname verwijderen",
        "Deliberating": "In beraadslaging",
        "Delivery channels": "Bezorgkanalen",
        "Delivery method": "Bezorgmethode",
        "Demo data (optional)": "Demovoorbeelddata (optioneel)",
        "Denominator": "Noemer",
        "Describe the agenda item": "Beschrijf het agendapunt",
        "Describe the correction you propose. The chair or secretary reviews every suggestion before approving the minutes.": "Beschrijf de correctie die je voorstelt. De voorzitter of secretaris beoordeelt elk voorstel voordat de notulen worden goedgekeurd.",
        "Describe your reason for declaring a conflict of interest": "Beschrijf uw reden voor het melden van een belangenverstrengeling",
        "Description": "Omschrijving",
        "Detailed rule": "Nadere regel",
        "Digital Documents": "Digitale documenten",
        "Directed to": "Gericht aan",
        "Directiestatuut": "Directiestatuut",
        "Discard section": "Sectie verwerpen",
        "Discharge": "Decharge",
        "Discussion": "Discussie",
        "Discussion notes": "Besprekingsnotities",
        "Dismiss": "Afwijzen",
        "Display": "Weergave",
        "Display position in the agenda item type picker.": "Weergavepositie in de keuzelijst voor agendapunttypen.",
        "Display position in the body’s roster.": "Weergavepositie in de ledenlijst van het orgaan.",
        "Display preferences": "Weergavevoorkeuren",
        "Display preferences saved.": "Weergavevoorkeuren opgeslagen.",
        "Disposed": "Afgedaan",
        "Dissolution": "Opheffing",
        "Dissolved": "Opgeheven",
        "Document format": "Documentformaat",
        "Document generation error": "Fout bij documentgeneratie",
        "Document stored at {path}": "Document opgeslagen op {path}",
        "Document version": "Documentversie",
        "Documentation": "Documentatie",
        "Documents": "Documenten",
        "Documents received": "Stukken ontvangen",
        "Domain": "Domein",
        "Done": "Gereed",
        "Draft": "Concept",
        "Draft budget": "Ontwerpbegroting",
        "Due": "Deadline",
        "Due date": "Einddatum",
        "Due this week": "Deze week vervallen",
        "Due {date}": "Uiterlijk {date}",
        "Duplicate": "Dupliceren",
        "Duplicate row — skipped": "Dubbele regel — overgeslagen",
        "Duration (min)": "Duur (min)",
        "During the configured period your delegate receives your Decidiq notifications and can follow your pending votes and action items.": "Gedurende de ingestelde periode ontvangt je gedelegeerde jouw Decidiq-meldingen en kan deze je openstaande stemmingen en actiepunten volgen.",
        "Dutch": "Nederlands",
        "E-mail stemmen": "E-mail stemmen",
        "Edit": "Bewerken",
        "Edit Agenda Item": "Agendapunt bewerken",
        "Edit Amendment": "Amendement bewerken",
        "Edit Governance Body": "Bestuursorgaan bewerken",
        "Edit Meeting": "Vergadering bewerken",
        "Edit Motion": "Motie bewerken",
        "Edit Participant": "Deelnemer bewerken",
        "Edit action item": "Actiepunt bewerken",
        "Edit agenda item": "Agendapunt bewerken",
        "Edit amendment": "Amendement bewerken",
        "Edit motion": "Motie bewerken",
        "Edit process template": "Processjabloon bewerken",
        "Effective": "Van kracht",
        "Effective status": "Werkingsstatus",
        "Efficiency": "Efficiëntie",
        "Email": "E-mail",
        "Email Voting": "E-mail stemmen",
        "Email links": "E-mailkoppelingen",
        "Email voting": "E-mail stemmen",
        "Emails": "E-mails",
        "Enable email vote reply parsing": "E-mail stemantwoorden verwerken inschakelen",
        "Enable email voting": "E-mail stemmen inschakelen",
        "Enable voting by email reply": "Stemmen via e-mailantwoord inschakelen",
        "Enact": "In werking stellen",
        "Enacted": "In werking getreden",
        "End Date": "Einddatum",
        "End date": "Einddatum",
        "End membership": "Lidmaatschap beeindigen",
        "End of council membership": "Einde raadslidmaatschap",
        "End of term": "Einde termijn",
        "End reason": "Reden van beëindiging",
        "Ended": "Beëindigd",
        "Engagement": "Betrokkenheid",
        "Engagement records": "Betrokkenheidsregistraties",
        "Engagement score": "Betrokkenheidsscore",
        "English": "Engels",
        "Enter a valid email address.": "Voer een geldig e-mailadres in.",
        "Entries": "Regels",
        "Er is al een volmacht geregistreerd voor deze deelnemer in deze stemronde": "Er is al een volmacht geregistreerd voor deze deelnemer in deze stemronde",
        "Error": "Fout",
        "Estimated Duration": "Geschatte duur",
        "Every schema, generated values": "Elk schema, gegenereerde waarden",
        "Every step of this route has been taken.": "Alle stappen van deze route zijn gezet.",
        "Ex officio": "Ambtshalve",
        "Example: {example}": "Voorbeeld: {example}",
        "Exception dates": "Uitzonderingsdata",
        "Executive board": "College",
        "Executive board paper": "Collegestuk",
        "Exit confirmation": "Exit bevestiging",
        "Exit date": "Uittredingsdatum",
        "Expected type": "Verwacht type",
        "Expired": "Verlopen",
        "Export": "Exporteren",
        "Export agenda": "Agenda exporteren",
        "Extend 10 min": "10 min verlengen",
        "Extend 10 minutes": "10 minuten verlengen",
        "Extend 5 min": "5 min verlengen",
        "Extend 5 minutes": "5 minuten verlengen",
        "External charter": "Statuut extern",
        "External integrations linked to this agenda item — open the sidebar to link emails, browse files, notes, tags, tasks and the audit trail.": "Externe integraties gekoppeld aan dit agendapunt — open de zijbalk om e-mails te koppelen en bestanden, notities, labels, taken en het auditspoor te bekijken.",
        "External integrations linked to this agenda item. Linked emails, files and tasks appear on the page itself; notes, tags and the audit trail stay in the sidebar.": "Externe integraties gekoppeld aan dit agendapunt. Gekoppelde e-mails, bestanden en taken staan op de pagina zelf; notities, labels en het audittrail blijven in de zijbalk.",
        "External integrations linked to this decision dossier — open the sidebar to link emails, browse files, notes, tags, tasks and the audit trail.": "Externe integraties gekoppeld aan dit besluitdossier — open de zijbalk om e-mails te koppelen en bestanden, notities, labels, taken en het auditspoor te bekijken.",
        "External integrations linked to this decision dossier. Linked emails, the Action items board (Deck) and files appear on the page itself; notes, tags and the audit trail stay in the sidebar.": "Externe integraties gekoppeld aan dit besluitdossier. Gekoppelde e-mails, het actiepuntenbord (Deck) en bestanden staan op de pagina zelf; notities, labels en het audittrail blijven in de zijbalk.",
        "External integrations linked to this meeting — open the sidebar to browse linked articles, files, notes, tags, tasks and the audit trail.": "Externe integraties gekoppeld aan deze vergadering — open de zijbalk om gekoppelde artikelen, bestanden, notities, tags, taken en het auditspoor te bekijken.",
        "External integrations linked to this meeting. The Action items board (Deck), Discussion (Talk), files and notes appear on the page itself; linked articles, tags and the audit trail stay in the sidebar.": "Externe integraties gekoppeld aan deze vergadering. Het actiepuntenbord (Deck), Discussie (Talk), bestanden en notities staan op de pagina zelf; gekoppelde artikelen, labels en het audittrail blijven in de zijbalk.",
        "External integrations linked to this motion — open the sidebar to browse the Discussion (Talk), files, notes, tags, tasks and the audit trail.": "Externe integraties gekoppeld aan dit agendapunt — open de zijbalk om de Discussie (Talk), bestanden, notities, tags, taken en het auditspoor te bekijken.",
        "External integrations linked to this motion. Open the sidebar for Discussion (Talk), files, notes, tags, tasks and the audit trail.": "Externe integraties gekoppeld aan deze motie. Open de zijbalk voor Discussie (Talk), bestanden, notities, labels, taken en het audittrail.",
        "External member": "Extern lid",
        "External reference": "Externe referentie",
        "External register identifier": "Identificatie in extern register",
        "Externe statuut": "Extern statuut",
        "Faction": "Fractie",
        "Factions": "Fracties",
        "Factions & bodies": "Fracties & Organen",
        "Factions & committees": "Fracties & commissies",
        "Failed": "Mislukt",
        "Failed to add member.": "Toevoegen van lid mislukt.",
        "Failed to add signer.": "Toevoegen van ondertekenaar mislukt.",
        "Failed to cast vote": "Stem uitbrengen mislukt",
        "Failed to change role.": "Wijzigen van rol mislukt.",
        "Failed to close the cycle.": "Sluiten van de cyclus mislukt.",
        "Failed to generate the report.": "Genereren van het rapport mislukt.",
        "Failed to link participant.": "Koppelen van deelnemer mislukt.",
        "Failed to load action items.": "Laden van actiepunten mislukt.",
        "Failed to load agenda.": "Laden van agenda mislukt.",
        "Failed to load amendments.": "Laden van amendementen mislukt.",
        "Failed to load analytics.": "Laden van analyses mislukt.",
        "Failed to load decisions.": "Laden van besluiten mislukt.",
        "Failed to load evaluations.": "Laden van evaluaties mislukt.",
        "Failed to load governance bodies.": "Bestuursorganen konden niet worden geladen.",
        "Failed to load lifecycle state.": "Laden van levenscyclusstatus mislukt.",
        "Failed to load members.": "Laden van leden mislukt.",
        "Failed to load minutes.": "Laden van notulen mislukt.",
        "Failed to load motions.": "Laden van moties mislukt.",
        "Failed to load parent motion.": "Laden van ouder-motie mislukt.",
        "Failed to load participants.": "Laden van deelnemers mislukt.",
        "Failed to load publication state.": "Publicatiestatus kon niet worden geladen.",
        "Failed to load related decisions.": "Laden van gerelateerde besluiten mislukt.",
        "Failed to load route.": "Laden van de route mislukt.",
        "Failed to load routed documents.": "Laden van doorgestuurde documenten mislukt.",
        "Failed to load signers.": "Laden van ondertekenaars mislukt.",
        "Failed to load the amendment diff.": "Laden van het amendementsverschil mislukt.",
        "Failed to load the governance body.": "Laden van het bestuursorgaan mislukt.",
        "Failed to load the meeting.": "Kon de vergadering niet laden.",
        "Failed to load the minutes.": "De notulen konden niet worden geladen.",
        "Failed to load the ondermandaat chain.": "Laden van de ondermandaatketen is mislukt.",
        "Failed to load the version timeline.": "Laden van de versietijdlijn is mislukt.",
        "Failed to load votes.": "Laden van stemmen mislukt.",
        "Failed to load voting overview.": "Laden van stemmingsoverzicht mislukt.",
        "Failed to load voting results.": "Laden van stemresultaten mislukt.",
        "Failed to open voting round": "Stemronde kon niet worden geopend",
        "Failed to publish agenda.": "Publiceren van agenda mislukt.",
        "Failed to publish the summary.": "Publiceren van de samenvatting mislukt.",
        "Failed to reimport register.": "Register opnieuw importeren mislukt.",
        "Failed to save the template assignment.": "Opslaan van de sjabloontoewijzing mislukt.",
        "Failed to start the evaluation.": "Starten van de evaluatie mislukt.",
        "Failed to submit your response.": "Verzenden van uw reactie mislukt.",
        "Features & roadmap": "Functies en roadmap",
        "File": "Bestand",
        "Files": "Bestanden",
        "Fill in the agenda item details. The chair will approve or reject your proposal.": "Vul de agendapuntdetails in. De voorzitter keurt uw voorstel goed of wijst het af.",
        "Financial statements": "Jaarrekening",
        "Financial year": "Boekjaar",
        "Flow": "Flow",
        "Flows": "Flows",
        "Flows are what happens without anyone clicking: a reminder before a deadline passes, a confirmation sent on submission. This is where you read and edit them. Nothing to build now.": "Flows zijn wat er gebeurt zonder dat iemand klikt: een herinnering voordat een termijn verstrijkt, een bevestiging bij indiening. Hier lees en bewerk je ze. Je hoeft nu niets te bouwen.",
        "For": "Voor",
        "For / Against / Abstain": "Voor / Tegen / Onthouding",
        "For support, contact us at": "Neem voor ondersteuning contact met ons op via",
        "For support, contact us at {email}": "Voor ondersteuning, neem contact met ons op via {email}",
        "For: {for} — Against: {against} — Abstain: {abstain}": "Voor: {for} — Tegen: {against} — Onthouding: {abstain}",
        "Format": "Notatie",
        "Formerly: {citation}": "Voorheen: {citation}",
        "Fractie": "Fractie",
        "Fraction": "Breukdeel",
        "Framework memorandum": "Kadernota",
        "Free-form label linking the records created by one intake run. Optional; drives the batch filter on the index.": "Vrij label dat de records uit één instroomronde aan elkaar koppelt. Optioneel; stuurt het batchfilter op het overzicht.",
        "Free-form label linking the records created by one turnover run. Optional; drives the batch filter on the index.": "Vrij label dat de records uit één wisselronde aan elkaar koppelt. Optioneel; stuurt het batchfilter op het overzicht.",
        "Free-text source note naming the splitsingsakte article this deviation stems from.": "Free-text source note naming the splitsingsakte article this deviation stems from.",
        "French": "Frans",
        "Frequency": "Frequentie",
        "From": "Van",
        "Function": "Functie",
        "Functional type of this stage: determines the role it plays in the route. `endorsement` is the parafering case: a sign-off that is not itself the decision.": "Functioneel type van deze stap — bepaalt welke rol zij in de route speelt. `endorsement` is het parafering-geval: een akkoord dat zelf niet het besluit is.",
        "Geen actieve stemronde.": "Geen actieve stemronde.",
        "Geen amendementen.": "Geen amendementen.",
        "Geen moties gevonden.": "Geen moties gevonden.",
        "Geheime stemming": "Geheime stemming",
        "Gemeentewet": "Gemeentewet",
        "General": "Algemeen",
        "Generate document": "Document genereren",
        "Generate draft minutes": "Conceptnotulen genereren",
        "Generate meeting series": "Vergaderreeks genereren",
        "Generate proof package": "Bewijspakket genereren",
        "Generate report": "Rapport genereren",
        "Generate series": "Reeks genereren",
        "Generated at": "Gegenereerd op",
        "Generated documents": "Gegenereerde documenten",
        "Generating…": "Genereren…",
        "Gepubliceerd naar ORI": "Gepubliceerd naar ORI",
        "German": "Duits",
        "Getting started": "Aan de slag",
        "Gewogen stemming": "Gewogen stemming",
        "Gift": "Geschenk",
        "Gifts": "Geschenken",
        "Give floor": "Woord geven",
        "Give floor to {name}": "Woord geven aan {name}",
        "Giver": "Gever",
        "Global search": "Globaal zoeken",
        "Goal": "Doel",
        "Goals": "Doelen",
        "Governance": "Bestuur",
        "Governance Bodies": "Bestuursorganen",
        "Governance Body": "Bestuursorgaan",
        "Governance bodies": "Bestuursorganen",
        "Governance body": "Bestuursorgaan",
        "Governance body composition": "Samenstelling van orgaan",
        "Governance context": "Bestuurscontext",
        "Governance email": "E-mailadres voor bestuurszaken",
        "Governance health": "Bestuurlijke gezondheid",
        "Governance model": "Bestuursmodel",
        "Governing body": "Bestuursorgaan",
        "Governing document": "Bestuurlijk document",
        "Governing documents": "Bestuurlijke documenten",
        "Government (gov)": "Overheid (gov)",
        "Grant": "Verlenen",
        "Grant Proxy": "Volmacht verlenen",
        "Grant proxy": "Volmacht verlenen",
        "Grant proxy to": "Volmacht verlenen aan",
        "Grantor": "Volmachtgever",
        "Grants competence for": "Verleent bevoegdheid voor",
        "Ground": "Grond",
        "Guest": "Gast",
        "Handed over": "Overgedragen",
        "Handled": "Behandeld",
        "Handopsteking": "Handopsteking",
        "Handopsteking resultaat opslaan": "Handopsteking resultaat opslaan",
        "Held over": "Aangehouden",
        "Het RSIN van de organisatie die verantwoordelijk is voor het besluit. Een RSIN en niet een `$ref`: de BRC identificeert de verantwoordelijke met dit nummer, en een verwijzing die alleen binnen deze instantie oplost zou het contract breken.": "Het RSIN van de organisatie die verantwoordelijk is voor het besluit. Een RSIN en niet een `$ref`: de BRC identificeert de verantwoordelijke met dit nummer, en een verwijzing die alleen binnen deze instantie oplost zou het contract breken.",
        "Hide": "Verbergen",
        "Hide meeting cost": "Vergaderkosten verbergen",
        "History": "Geschiedenis",
        "Hold": "Vorm",
        "Hold type": "Bekledingstype",
        "Holder": "Gemachtigde",
        "Horizon": "Horizon",
        "How": "Hoe",
        "How MANY members are required for a valid sitting. Not how a quorum is calculated: that is `quorumRule`. Deliberately has NO default: an unset value means 'not specified', and a default of 0 would assert that no members are needed. Awb 7:13 objection advisory committees state this as a number.": "HOEVEEL leden vereist zijn voor een geldige zitting. Niet hoe een quorum wordt berekend — dat is `quorumRule`. Bewust ZONDER standaardwaarde: een lege waarde betekent 'niet gespecificeerd', en een standaard van 0 zou beweren dat er geen leden nodig zijn. Bezwaaradviescommissies (Awb 7:13) noemen dit als getal.",
        "How a quorum is CALCULATED (e.g. majority). Not how many members are required: that is `quorum`, which is a number. A calculation method cannot express a statutory minimum member count.": "Hoe een quorum wordt BEREKEND (bijv. meerderheid). Niet hoeveel leden vereist zijn — dat is `quorum`, een getal. Een rekenmethode kan geen wettelijk minimum aantal leden uitdrukken.",
        "How long before the meeting an item may still be submitted.": "Hoe lang voor de vergadering een punt nog ingediend mag worden.",
        "How many people may hold this position at the same time.": "Hoeveel personen deze functie tegelijk mogen bekleden.",
        "How many representatives the component sends.": "Hoeveel afgevaardigden het samenstellende orgaan stuurt.",
        "How many terms one person may serve back to back.": "Hoeveel termijnen één persoon aaneengesloten mag dienen.",
        "How rounds ended, and how the votes fell.": "Hoe rondes eindigden, en hoe de stemmen vielen.",
        "How the parties who may answer are determined.": "Hoe wordt bepaald wie mag reageren.",
        "How the person was formally installed, where the organisation marks that at all. Free text: a council takes an oath or an affirmation, a board signs a deed, an association records a vote, and none of those is every organisation's word.": "Hoe de persoon formeel is geïnstalleerd, voor zover de organisatie dat vastlegt. Vrije tekst: een raad legt een eed of een belofte af, een bestuur tekent een akte, een vereniging noteert een stemming, en geen van die woorden geldt voor elke organisatie.",
        "How the position is held, for example regular or interim.": "Hoe de functie wordt bekleed, bijvoorbeeld regulier of interim.",
        "How the quorum is calculated.": "Hoe het quorum wordt berekend.",
        "How this ground is classified, in whatever terms the organisation's own law or statutes use.": "Hoe deze grond is ingedeeld, in de termen die de eigen wet of statuten van de organisatie gebruiken.",
        "How to contest this besluit, copied from the decision at the moment it was published. A reader was told the legal ground the besluit rests on and not how to object to it, because the allow-list carried legalBasis and not this (REQ-DWP-007). Stamped, never read live: a term that changes next year must not change what this publication told someone last year.": "Hoe u bezwaar maakt tegen dit besluit, overgenomen uit het besluit op het moment van publicatie. Een lezer zag wel de wettelijke grondslag waarop het besluit rust en niet hoe daartegen bezwaar te maken, omdat de allow-list legalBasis droeg en deze niet (REQ-DWP-007). Vastgelegd, nooit live gelezen: een termijn die volgend jaar verandert mag niet veranderen wat deze publicatie iemand vorig jaar vertelde.",
        "How votes fell": "Hoe de stemmen vielen",
        "Huishoudelijk reglement": "Huishoudelijk reglement",
        "I confirm participants were informed of the recording.": "Ik bevestig dat deelnemers zijn geïnformeerd over de opname.",
        "Id of the ApprovalRoute this stage was instantiated from. Written at instantiation; empty on stages that predate the runtime absorption, which announce no cross-app conclusion.": "Id van de ApprovalRoute waaruit deze fase is aangemaakt. Geschreven bij het aanmaken; leeg bij fasen van voor de runtime-overname, die geen conclusie naar een andere app aankondigen.",
        "Idea box": "Ideeënbus",
        "Implemented by": "Uitgevoerd door",
        "Implements": "Geeft uitvoering aan",
        "Import failed.": "Importeren mislukt.",
        "Import from CSV": "Importeren uit CSV",
        "Import from Nextcloud group": "Importeren uit Nextcloud-groep",
        "Import members": "Leden importeren",
        "Import {count} members": "{count} leden importeren",
        "Importing...": "Importeren...",
        "Importing…": "Importeren…",
        "Imposed": "Opgelegd",
        "Imposed at": "Opgelegd op",
        "Imposed by": "Opgelegd door",
        "Imposed by the chair": "Opgelegd door de voorzitter",
        "Imposed by the executive board": "Opgelegd door het college",
        "Imposed by the governing body": "Opgelegd door het bestuursorgaan",
        "In Deck": "In Deck",
        "In app": "In app",
        "In effect": "In werking",
        "In execution": "In uitvoering",
        "In force": "Geldend",
        "In force from {date}": "In werking vanaf {date}",
        "In force since": "In werking sinds",
        "In handling": "In behandeling",
        "In line with advice": "Conform advies",
        "In line with consent": "Conform instemming",
        "In person, digital or hybrid": "Fysiek, digitaal of hybride",
        "In preparation": "In voorbereiding",
        "In progress": "In behandeling",
        "In review": "In beoordeling",
        "In treatment": "In behandeling",
        "In use": "In gebruik",
        "In voorbereiding": "In voorbereiding",
        "Inactive": "Inactief",
        "Include with agenda item": "Betrekken bij agendapunt",
        "Incoming document": "Ingekomen stuk",
        "Incoming documents": "Ingekomen documenten",
        "Individual": "Individueel",
        "Induction pack": "Introductiepakket",
        "Informatiecategorie": "Informatiecategorie",
        "Information about the current Decidiq installation": "Informatie over de huidige Decidiq-installatie",
        "Informational": "Informatief",
        "Ingediend": "Ingediend",
        "Ingetrokken": "Ingetrokken",
        "Initial lifecycle state": "Beginstatus",
        "Initial state": "Beginstatus",
        "Install OpenRegister": "OpenRegister installeren",
        "Install governance configuration that other organisations have published: a council with its committees and factions, an association with its members' meeting, or the flows that move a decision through them.": "Installeer bestuurlijke configuratie die andere organisaties hebben gepubliceerd: een raad met commissies en fracties, een vereniging met een ledenvergadering, of de flows die een besluit erdoorheen leiden.",
        "Instances in series {series}": "Exemplaren in reeks {series}",
        "Integrations": "Integraties",
        "Interface language follows your Nextcloud account language.": "De interfacetaal volgt de taal van je Nextcloud-account.",
        "Interim succession": "Tussentijdse opvolging",
        "Internal": "Intern",
        "Internal regulations": "Huishoudelijk reglement",
        "Interpellation request": "Interpellatieverzoek",
        "Interpellations": "Interpellaties",
        "Interval": "Interval",
        "Invalid email address": "Ongeldig e-mailadres",
        "Invalid row": "Ongeldige regel",
        "Invitation": "Uitnodiging",
        "Invitations": "Uitnodigingen",
        "Invite Co-Signatories": "Medeondertekenaars uitnodigen",
        "Involved in handling": "Betrokken bij behandeling",
        "Italian": "Italiaans",
        "Item": "Punt",
        "Item closed ({minutes} min)": "Punt afgesloten ({minutes} min)",
        "Items of this kind": "Agendapunten van deze soort",
        "Items per page": "Items per pagina",
        "Jaarrekening": "Jaarrekening",
        "Joined": "Ingetreden",
        "Jurisdiction": "Rechtsgebied",
        "Kadernota": "Kadernota",
        "Kascommissie": "Kascommissie",
        "Kascommissie report": "Verslag kascommissie",
        "Kascommissie verklaring": "Kascommissieverklaring",
        "Kascommissie verklaringen": "Kascommissieverklaringen",
        "Keep at least one delivery channel enabled. Use the per-event switches to mute specific notifications.": "Houd ten minste één bezorgkanaal ingeschakeld. Gebruik de schakelaars per gebeurtenis om specifieke meldingen te dempen.",
        "Keep everything": "Alles bewaren",
        "Key": "Sleutel",
        "Kind": "Soort",
        "Koppelen": "Koppelen",
        "Label": "Label",
        "Laden…": "Laden…",
        "Language": "Taal",
        "Lapsed": "Vervallen",
        "Last checked": "Laatst gecontroleerd",
        "Latest meeting: {title}": "Laatste vergadering: {title}",
        "Leave empty to skip catalog routing": "Laat leeg om catalogusroutering over te slaan",
        "Leave empty to use your Nextcloud account email.": "Laat leeg om het e-mailadres van je Nextcloud-account te gebruiken.",
        "Left": "Uitgetreden",
        "Legacy citation": "Oude citeertitel",
        "Legal basis": "Wettelijke grondslag",
        "Length of one term in this position, in months.": "Duur van één zittingstermijn in deze functie, in maanden.",
        "Less than 24 hours remaining": "Minder dan 24 uur resterend",
        "Let's take a quick spin through decision-making. We'll schedule a meeting together so you can see how the pieces fit, and you'll add the record yourself.": "Laten we snel door de besluitvorming lopen. We plannen samen een vergadering zodat je ziet hoe alles in elkaar grijpt, en jij legt het record zelf vast.",
        "Letter organisation": "Brief organisatie",
        "Letter resident": "Brief inwoner",
        "Lifecycle": "Levenscyclus",
        "Lifecycle states": "Levenscyclusstatussen",
        "Lifecycle unavailable": "Levenscyclus niet beschikbaar",
        "Lifted": "Opgeheven",
        "Limited": "Beperkt",
        "Line": "Regel",
        "Link a motion to this agenda item": "Een motie koppelen aan dit agendapunt",
        "Link email": "E-mail koppelen",
        "Link motion": "Motie koppelen",
        "Linked Meeting": "Gekoppelde vergadering",
        "Linked emails": "Gekoppelde e-mails",
        "Linked motions": "Gekoppelde moties",
        "Live meeting": "Live vergadering",
        "Live meeting view": "Live vergaderingweergave",
        "Load a small example dataset so the lists, detail pages and dashboards show a working product straight away. The data is obviously sample data, it is safe to run more than once, and it can be removed afterwards. Skip this on a production install.": "Laad een kleine voorbeeldset zodat de lijsten, detailpagina's en dashboards meteen een werkend product laten zien. De data is duidelijk voorbeelddata, veilig om meerdere keren uit te voeren en achteraf te verwijderen. Sla dit over op een productie-installatie.",
        "Load the example data": "Laad de voorbeeldgegevens",
        "Loading action items…": "Actiepunten laden…",
        "Loading agenda…": "Agenda laden…",
        "Loading amendments…": "Amendementen laden…",
        "Loading analytics…": "Analyses laden…",
        "Loading chain…": "Keten laden…",
        "Loading decisions…": "Besluiten laden…",
        "Loading group members…": "Groepsleden laden…",
        "Loading members…": "Leden laden…",
        "Loading minutes…": "Notulen laden…",
        "Loading motions…": "Moties laden…",
        "Loading participants…": "Deelnemers laden…",
        "Loading related decisions…": "Gerelateerde besluiten laden…",
        "Loading routed documents…": "Doorgestuurde documenten laden…",
        "Loading route…": "Route laden…",
        "Loading series instances…": "Reeksexemplaren laden…",
        "Loading signers…": "Ondertekenaars laden…",
        "Loading template assignment…": "Sjabloontoewijzing laden…",
        "Loading versions…": "Versies laden…",
        "Loading votes…": "Stemmen laden…",
        "Loading voting overview…": "Stemmingsoverzicht laden…",
        "Loading voting results…": "Stemresultaten laden…",
        "Loading…": "Laden…",
        "Loads the set you picked. The data is obviously sample data, it is safe to run more than once, and you can delete it afterwards.": "Laadt de set die je koos. De gegevens zijn herkenbaar voorbeeldgegevens, je kunt dit meer dan een keer uitvoeren en je kunt ze daarna verwijderen.",
        "Loads the sets you picked. The data is obviously sample data, it is safe to run more than once, and you can delete it afterwards.": "Laadt de sets die je koos. De gegevens zijn herkenbaar voorbeeldgegevens, je kunt dit meer dan een keer uitvoeren en je kunt ze daarna verwijderen.",
        "Location": "Locatie",
        "Logo URL": "Logo-URL",
        "Long-term agenda": "Langetermijnagenda",
        "Long-term agenda item": "Langetermijnagendapunt",
        "Machtiging": "Machtiging",
        "Machtiging document": "Machtigingsdocument",
        "Made by": "Gedaan door",
        "Majority overrides": "Afwijkende meerderheden",
        "Majority threshold": "Meerderheidsdrempel",
        "Manage the OpenRegister configuration for Decidiq.": "Beheer de OpenRegister-configuratie voor Decidiq.",
        "Management charter": "Directiestatuut",
        "Management point": "Beheerspunt",
        "Mandaat": "Mandaat",
        "Mandate": "Mandaat",
        "Mandatory": "Verplicht",
        "Manual only": "Alleen handmatig",
        "Markdown": "Markdown",
        "Markdown fallback": "Markdown-terugval",
        "Market consultations": "Marktconsultaties",
        "Max consecutive terms": "Maximaal aantal opeenvolgende termijnen",
        "Maximum consecutive terms": "Maximum aantal aaneengesloten termijnen",
        "Medeondertekenaars": "Medeondertekenaars",
        "Medeondertekenaars uitnodigen": "Medeondertekenaars uitnodigen",
        "Meeting": "Vergadering",
        "Meeting \"%1$s\" moved to \"%2$s\"": "Vergadering \"%1$s\" is overgegaan naar \"%2$s\"",
        "Meeting cost": "Vergaderkosten",
        "Meeting date": "Vergaderdatum",
        "Meeting duration": "Vergaderduur",
        "Meeting integrations": "Vergaderintegraties",
        "Meeting not found": "Vergadering niet gevonden",
        "Meeting outcome": "Vergaderuitkomst",
        "Meeting package assembled": "Vergaderstukken samengesteld",
        "Meeting reminder": "Vergaderherinnering",
        "Meeting reminder timing": "Tijdstip vergaderherinnering",
        "Meeting scheduled": "Vergadering gepland",
        "Meeting status distribution": "Statusverdeling vergaderingen",
        "Meeting type": "Vergaderingstype",
        "Meeting type (deprecated enum)": "Vergaderingstype (verouderde lijst)",
        "Meeting types": "Vergadersoorten",
        "Meeting view": "Vergaderweergave",
        "Meeting {object} moved to \"%1$s\"": "Vergadering {object} is overgegaan naar \"%1$s\"",
        "Meetings": "Vergaderingen",
        "Meetings ({n})": "Vergaderingen ({n})",
        "Meetings and attendance": "Vergaderingen en aanwezigheid",
        "Meetings are the heart of Decidiq. Open Meetings from the menu to see everything on the agenda.": "Vergaderingen vormen de kern van Decidiq. Open Vergaderingen in het menu om alles op de agenda te zien.",
        "Meetings without a date": "Vergaderingen zonder datum",
        "Member": "Lid",
        "Member consultations": "Ledenraadplegingen",
        "Member offboarding": "Vertrek van een lid",
        "Member onboarding": "Aantreden van een lid",
        "Members": "Leden",
        "Members ({n})": "Leden ({n})",
        "Membership": "Lidmaatschap",
        "Mention": "Vermelden",
        "Mentioned in a comment": "Genoemd in een reactie",
        "Method": "Methode",
        "Minimum attendees for a valid sitting.": "Minimum aantal aanwezigen voor een geldige zitting.",
        "Minute taking": "Notuleren",
        "Minutes": "Notulen",
        "Minutes (live)": "Notulen (live)",
        "Minutes ({n})": "Notulen ({n})",
        "Minutes attendance rendering": "Weergave aanwezigheid in notulen",
        "Minutes awaiting approval": "Notulen die wachten op vaststelling",
        "Minutes detail": "Notulendetail",
        "Minutes lifecycle": "Levenscyclus notulen",
        "Minutes policy": "Notulenbeleid",
        "Missing name": "Naam ontbreekt",
        "Missing statutory ALV agenda items": "Ontbrekende wettelijk verplichte ALV-agendapunten",
        "Mjop adoption": "Mjop vaststelling",
        "Mode": "Modus",
        "Modelreglement": "Modelreglement",
        "Modelreglement preset": "Modelreglement-preset",
        "Modelreglement presets": "Modelreglement-presets",
        "Moderation note (optional)": "Moderatienotitie (optioneel)",
        "Moderation queue": "Moderatiewachtrij",
        "Mogelijk conflict": "Mogelijk conflict",
        "Mogelijk conflict met ander amendement — raadpleeg de griffier": "Mogelijk conflict met ander amendement — raadpleeg de griffier",
        "Monetary Amounts": "Geldbedragen",
        "Most recent": "Meest recent",
        "Most recent rounds": "Meest recente rondes",
        "Motie intrekken": "Motie intrekken",
        "Motie koppelen": "Motie koppelen",
        "Motion": "Motie",
        "Motion Details": "Motiedetails",
        "Motion integrations": "Motie-integraties",
        "Motion text": "Motietekst",
        "Motions": "Moties",
        "Move action item to another status": "Actiepunt naar een andere status verplaatsen",
        "Move amendment down": "Amendement omlaag verplaatsen",
        "Move amendment up": "Amendement omhoog verplaatsen",
        "Move {name} down": "{name} omlaag verplaatsen",
        "Move {name} up": "{name} omhoog verplaatsen",
        "Move {title} down": "{title} omlaag verplaatsen",
        "Move {title} up": "{title} omhoog verplaatsen",
        "Municipalities act": "Gemeentewet",
        "Municipality": "Gemeente",
        "My action items": "Mijn actiepunten",
        "My declarations": "Mijn opgaven",
        "NC group": "Nextcloud-groep",
        "Nadere regel": "Nadere regel",
        "Name": "Naam",
        "Name of the position as the organisation calls it.": "Naam van de functie zoals de organisatie die noemt.",
        "Names of role-holders": "Namen van functiehouders",
        "Natural person": "Natuurlijk persoon",
        "Negative": "Negatief",
        "Negative advice": "Negatief advies",
        "New board": "Nieuw bestuur",
        "New meeting": "Nieuwe vergadering",
        "New member": "Nieuw lid",
        "Next meeting": "Volgende vergadering",
        "Next month": "Volgende maand",
        "Next phase": "Volgende fase",
        "Nextcloud group": "Nextcloud-groep",
        "Nextcloud locale (default)": "Nextcloud-taalinstelling (standaard)",
        "Nextcloud notification": "Nextcloud-melding",
        "Nextcloud user id linked to the person by the account-linking step. Optional until linked; the induction-pack step depends on it.": "Nextcloud-gebruikers-id dat via de stap account-linking aan de persoon is gekoppeld. Optioneel tot het gekoppeld is; de stap induction-pack hangt ervan af.",
        "Niet ingediend": "Niet ingediend",
        "No": "Nee",
        "No account — manual linking needed": "Geen account — handmatig koppelen nodig",
        "No action items assigned to you": "Geen actiepunten aan jou toegewezen",
        "No action items spawned by this decision yet.": "Nog geen actiepunten voortkomend uit dit besluit.",
        "No active budget rounds": "Geen actieve begrotingsrondes",
        "No active motions": "Geen actieve moties",
        "No active voting round.": "Geen actieve stemronde.",
        "No advice": "Geen advies",
        "No advisory-opinion requests reference this decision yet.": "Nog geen adviesaanvragen verwijzen naar dit besluit.",
        "No agenda item types configured yet. An example set seeds some, or add your own.": "Nog geen soorten agendapunten ingesteld. Een voorbeeldset levert er een paar, of voeg er zelf een toe.",
        "No agenda items use this kind yet.": "Nog geen agendapunten van deze soort.",
        "No agenda items yet for this meeting.": "Nog geen agendapunten voor deze vergadering.",
        "No agenda items.": "Geen agendapunten.",
        "No amendments": "Geen amendementen",
        "No amendments for this motion yet.": "Nog geen amendementen voor deze motie.",
        "No amendments for this motion.": "Geen amendementen voor deze motie.",
        "No answer within the term": "Geen antwoord binnen de termijn",
        "No audio files found in this meeting's folder. Upload a recording to the meeting folder, then refresh.": "Geen audiobestanden gevonden in de map van deze vergadering. Upload een opname naar de vergadermap en vernieuw.",
        "No board meetings yet": "Nog geen bestuursvergaderingen",
        "No boards yet": "Nog geen besturen",
        "No body compositions configured yet.": "Nog geen samenstelling van organen ingesteld.",
        "No co-signatories yet.": "Nog geen medeondertekenaars.",
        "No consultation responses yet.": "Nog geen reacties op raadplegingen.",
        "No consultations were held on this decision.": "Over dit besluit is niemand geraadpleegd.",
        "No consultations yet.": "Nog geen raadplegingen.",
        "No corrections suggested.": "Geen correcties voorgesteld.",
        "No decision-making linked to this object yet.": "Nog geen besluitvorming gekoppeld aan dit object.",
        "No decisions yet": "Nog geen besluiten",
        "No decisions yet for this meeting.": "Nog geen besluiten voor deze vergadering.",
        "No documents generated yet.": "Nog geen documenten gegenereerd.",
        "No draft minutes exist for this meeting yet.": "Er bestaan nog geen conceptnotulen voor deze vergadering.",
        "No factions registered under this body yet.": "Nog geen fracties geregistreerd onder dit orgaan.",
        "No gifts or invitations declared for this body yet.": "Nog geen geschenken of uitnodigingen opgegeven voor dit orgaan.",
        "No governance bodies found.": "Geen bestuursorganen gevonden.",
        "No hourly rate configured on this governance body — set one to see the running cost.": "Geen uurtarief ingesteld voor dit bestuursorgaan — stel er een in om de lopende kosten te zien.",
        "No incoming documents routed to this meeting yet.": "Nog geen ingekomen documenten doorgestuurd naar deze vergadering.",
        "No interpellation requests scheduled for this meeting.": "Geen interpellatieverzoeken gepland voor deze vergadering.",
        "No linked governance body.": "Geen gekoppeld bestuursorgaan.",
        "No linked meeting.": "Geen gekoppelde vergadering.",
        "No linked motion.": "Geen gekoppelde motie.",
        "No linked motions.": "Geen gekoppelde moties.",
        "No meeting is linked to these minutes — the proof package needs a meeting.": "Er is geen vergadering aan deze notulen gekoppeld — het bewijspakket vereist een vergadering.",
        "No meeting linked — the voting round cannot be opened": "Vergadering niet gekoppeld — stemronde kan niet worden geopend",
        "No meeting types configured yet. An example set seeds some, or add your own.": "Nog geen vergadersoorten ingesteld. Een voorbeeldset levert er een paar, of voeg er zelf een toe.",
        "No meetings": "Geen vergaderingen",
        "No meetings found. Create a meeting to see status distribution.": "Geen vergaderingen gevonden. Maak een vergadering aan om de statusverdeling te bekijken.",
        "No meetings recorded for this body yet.": "Nog geen vergaderingen geregistreerd voor dit orgaan.",
        "No meetings scheduled for this body yet.": "Nog geen vergaderingen gepland voor dit orgaan.",
        "No meetings yet": "Nog geen vergaderingen",
        "No member consultations reference this decision yet.": "Nog geen ledenraadplegingen verwijzen naar dit besluit.",
        "No members linked to this body yet.": "Nog geen leden gekoppeld aan dit orgaan.",
        "No minutes yet for this meeting.": "Nog geen notulen voor deze vergadering.",
        "No more participants available to link.": "Geen deelnemers meer beschikbaar om te koppelen.",
        "No motion is linked to this decision, so there are no voting results.": "Er is geen motie aan dit besluit gekoppeld, dus er zijn geen stemresultaten.",
        "No motion linked to this decision item.": "Geen motie gekoppeld aan dit besluitpunt.",
        "No motions for this agenda item yet.": "Nog geen moties voor dit agendapunt.",
        "No motions for this agenda item.": "Geen moties voor dit agendapunt.",
        "No motions found for this meeting.": "Geen moties gevonden voor deze vergadering.",
        "No object store available": "Geen objectstore beschikbaar",
        "No open consultations": "Geen open consultaties",
        "No oral questions submitted for this meeting yet.": "Nog geen mondelinge vragen ingediend voor deze vergadering.",
        "No organisations participate in this shared body yet.": "Nog geen organisaties nemen deel aan dit gemeenschappelijk orgaan.",
        "No other meetings in this series yet.": "Nog geen andere vergaderingen in deze reeks.",
        "No other positions declared for this body yet.": "Nog geen nevenfuncties opgegeven voor dit orgaan.",
        "No parent motion": "Geen ouder-motie",
        "No parent motion linked.": "Geen gekoppelde moedermotie.",
        "No participants found.": "Geen deelnemers gevonden.",
        "No participants linked to this meeting yet.": "Nog geen deelnemers gekoppeld aan deze vergadering.",
        "No pending votes": "Geen openstaande stemmen",
        "No position types configured yet. An example set seeds some, or add your own.": "Nog geen functiesoorten ingesteld. Een voorbeeldset levert er een paar, of voeg er zelf een toe.",
        "No positions configured for this body yet.": "Nog geen functies ingesteld voor dit orgaan.",
        "No proposals submitted yet. Citizens submit these through the public Participation page.": "Nog geen voorstellen ingediend. Inwoners dienen die in via de publieke Participatiepagina.",
        "No proposals to vote on yet": "Nog geen voorstellen om op te stemmen",
        "No proposed text": "Geen voorgestelde tekst",
        "No proxy authorizations registered for this meeting yet.": "Nog geen volmachten geregistreerd voor deze vergadering.",
        "No public consultations reference this decision yet.": "Nog geen publieke consultaties verwijzen naar dit besluit.",
        "No reactions awaiting moderation": "Geen reacties die op moderatie wachten",
        "No reactions submitted yet. Citizens submit these through the public Participation page.": "Nog geen reacties ingediend. Inwoners dienen die in via de publieke Participatiepagina.",
        "No recurring agenda items found.": "Geen terugkerende agendapunten gevonden.",
        "No related decisions": "Geen gerelateerde besluiten",
        "No related meetings.": "Geen gerelateerde vergaderingen.",
        "No related participants.": "Geen gerelateerde deelnemers.",
        "No resolutions yet": "Nog geen besluiten",
        "No responses recorded yet.": "Nog geen reacties vastgelegd.",
        "No responses were recorded for this decision.": "Bij dit besluit zijn geen reacties vastgelegd.",
        "No results found": "Geen resultaten gevonden",
        "No retirement schedule generated for this body yet.": "Nog geen rooster van aftreden gegenereerd voor dit orgaan.",
        "No self-evaluation cycles yet for this body.": "Nog geen zelfevaluatiecycli voor dit orgaan.",
        "No settings available yet": "Nog geen instellingen beschikbaar",
        "No sign-off route on this object yet.": "Nog geen paraferingsroute op dit object.",
        "No signatures collected yet.": "Nog geen handtekeningen verzameld.",
        "No signers added yet.": "Nog geen ondertekenaars toegevoegd.",
        "No speakers in the queue.": "Geen sprekers in de wachtrij.",
        "No speech-to-text provider is installed on this instance. You can still attach a recording and record consent; transcription becomes available once a provider (e.g. a local Whisper app) is installed.": "Er is geen spraak-naar-tekstaanbieder geïnstalleerd op deze instantie. Je kunt nog steeds een opname koppelen en toestemming vastleggen; transcriptie wordt beschikbaar zodra een aanbieder (bijv. een lokale Whisper-app) is geïnstalleerd.",
        "No spokesperson assigned.": "Geen spreker toegewezen.",
        "No staged route configured": "Geen gefaseerde route ingesteld",
        "No stand-in was found for your sign-off": "Er is geen vervanger gevonden voor jouw parafering",
        "No term rule configured for this body yet.": "Nog geen zittingsregel ingesteld voor dit orgaan.",
        "No term set": "Geen termijn ingesteld",
        "No time allocated — elapsed time is tracked for analytics.": "Geen tijd toegewezen — verstreken tijd wordt bijgehouden voor analyse.",
        "No transitions available from this state.": "Geen overgangen beschikbaar vanuit deze status.",
        "No unassigned participants available.": "Geen niet-toegewezen deelnemers beschikbaar.",
        "No upcoming meetings": "Geen aankomende vergaderingen",
        "No versions have been recorded for this record yet.": "Voor dit record zijn nog geen versies vastgelegd.",
        "No versions yet": "Nog geen versies",
        "No view": "Geen zienswijze",
        "No votes recorded for this decision yet.": "Nog geen stemmen vastgelegd voor dit besluit.",
        "No votes recorded for this motion yet.": "Nog geen stemmen geregistreerd voor deze motie.",
        "No voting recorded for this meeting": "Geen stemmingen vastgelegd voor deze vergadering",
        "No voting recorded for this meeting.": "Geen stemmingen vastgelegd voor deze vergadering.",
        "No voting round for this item.": "Geen stemronde voor dit item.",
        "No voting rounds yet": "Nog geen stemrondes",
        "No works-council consultation requests reference this decision yet.": "Nog geen OR-adviesaanvragen verwijzen naar dit besluit.",
        "No zienswijzen adopted this decision as their raadsbesluit yet.": "Nog geen zienswijzen hebben dit besluit als raadsbesluit aangemerkt.",
        "No zienswijzerondes for this shared body yet.": "Nog geen zienswijzerondes voor dit gemeenschappelijk orgaan.",
        "Nobody assigned": "Niemand toegewezen",
        "Nobody holds a position on this body yet.": "Niemand vervult een functie bij dit orgaan.",
        "Nobody holds a position yet.": "Niemand vervult een functie.",
        "Nobody holds this position yet.": "Niemand vervult deze functie.",
        "Nobody is recorded as your substitute, so this sign-off is still waiting on you. Its deadline has not moved.": "Er staat niemand als jouw vervanger geregistreerd, dus deze parafering wacht nog steeds op jou. De termijn is niet verschoven.",
        "Nog geen medeondertekenaars.": "Nog geen medeondertekenaars.",
        "None": "Geen",
        "None, I will set this up myself": "Geen, ik richt dit zelf in",
        "Not adopted": "Niet overgenomen",
        "Not appointed": "Niet benoemd",
        "Not available": "Niet beschikbaar",
        "Not configured": "Niet geconfigureerd",
        "Not enough data": "Onvoldoende gegevens",
        "Not handled": "Niet behandeld",
        "Not issued": "Niet uitgebracht",
        "Not now": "Niet nu",
        "Not published.": "Niet gepubliceerd.",
        "Not pursued": "Niet doorgezet",
        "Not submitted": "Niet ingediend",
        "Not treated": "Niet behandeld",
        "Not yet asked": "Nog niet gevraagd",
        "Notarial proof package": "Notarieel bewijspakket",
        "Notary": "Notaris",
        "Noted for information": "Voor kennisgeving aannemen",
        "Notes": "Notities",
        "Nothing is imported. You start with an empty app and add your own bodies, meetings and decisions.": "Er wordt niets geïmporteerd. Je begint met een lege app en voegt zelf organen, vergaderingen en besluiten toe.",
        "Nothing to project.": "Niets om te projecteren.",
        "Notice deliveries ({n})": "Oproepingsbezorgingen ({n})",
        "Notification preferences": "Notificatievoorkeuren",
        "Notification preferences saved.": "Meldingsvoorkeuren opgeslagen.",
        "Notification, display, delegation and communication preferences for your account.": "Meldings-, weergave-, delegatie- en communicatievoorkeuren voor jouw account.",
        "Notifications": "Meldingen",
        "Notify me about": "Stuur mij meldingen over",
        "Notify on decision published": "Melding bij gepubliceerd besluit",
        "Notify on meeting scheduled": "Melding bij geplande vergadering",
        "Notify on mention": "Melding bij vermelding",
        "Notify on task assigned": "Melding bij toegewezen taak",
        "Notify on vote opened": "Melding bij geopende stemming",
        "Number": "Nummer",
        "ORI API endpoint URL": "ORI API eindpunt-URL",
        "ORI Endpoint": "ORI-eindpunt",
        "ORI endpoint": "ORI-eindpunt",
        "ORI endpoint URL for publishing voting results": "ORI-eindpunt-URL voor het publiceren van stemresultaten",
        "ORI niet geconfigureerd": "ORI niet geconfigureerd",
        "ORI not configured": "ORI niet geconfigureerd",
        "ORI-eindpunt": "ORI-eindpunt",
        "Oath": "Eed",
        "Object type": "Objecttype",
        "Objection decision": "Beslissing op bezwaar",
        "Observer": "Waarnemer",
        "Offboarding": "Uitstroom",
        "Offboarding traject": "Uitstroomtraject",
        "Offers": "Aanbiedingen",
        "Official title": "Officiële titel",
        "On agenda": "Op de agenda",
        "Onboarding": "Instroom",
        "Onboarding traject": "Instroomtraject",
        "Once someone asks colleagues to sign off on this, every step appears here with what they did and why.": "Zodra iemand collega's vraagt hierop te paraferen, verschijnt elke stap hier, met wat ze deden en waarom.",
        "Ondermandaat chain": "Ondermandaatketen",
        "Ondermandaten": "Ondermandaten",
        "Ondersteunen": "Ondersteunen",
        "Only the chair or secretary can open this cycle.": "Alleen de voorzitter of secretaris kan deze cyclus openen.",
        "Onthouding": "Onthouding",
        "Onthoudingen": "Onthoudingen",
        "Ontwerpbegroting": "Ontwerpbegroting",
        "Oordeelsvorming": "Oordeelsvorming",
        "Open": "Openen",
        "Open Debate": "Debat openen",
        "Open Decidiq": "Decidiq openen",
        "Open Flows in the menu": "Open Flows in het menu",
        "Open Voting Round": "Stemronde openen",
        "Open action items": "Openstaande actiepunten",
        "Open consultations": "Open consultaties",
        "Open consultations and participatory budget rounds for your governance body.": "Open consultaties en participatieve begrotingsrondes voor uw bestuursorgaan.",
        "Open for responses": "Open voor reacties",
        "Open in Deck": "Openen in Deck",
        "Open in decidiq": "Openen in decidiq",
        "Open items": "Openstaande items",
        "Open live meeting view": "Live vergaderingweergave openen",
        "Open package folder": "Map met vergaderstukken openen",
        "Open parent motion": "Ouder-motie openen",
        "Open settings": "Instellingen openen",
        "Open the decision that replaced this one": "Open het besluit dat dit besluit vervangt",
        "Open the documentation to keep going": "Open de documentatie om verder te gaan",
        "Open vote": "Stemming openen",
        "Open voting": "Stemming openen",
        "Open voting round": "Stemronde openen",
        "OpenCatalogi is not installed — the record received the public predicate but was not routed to a catalog.": "OpenCatalogi is niet geïnstalleerd — het record kreeg het openbaar-predicaat maar is niet naar een catalogus gestuurd.",
        "OpenRegister is required": "OpenRegister is vereist",
        "OpenRegister register ID": "OpenRegister register-ID",
        "Opened": "Geopend",
        "Openen": "Openen",
        "Opening": "Opening",
        "Operations (ops)": "Uitvoering (ops)",
        "Optional display label for this composition.": "Optioneel weergavelabel voor deze samenstelling.",
        "Optional object UUID linking what the step is about: the meeting where the person was installed, the induction pack that was delivered, the membership that was created.": "Optionele object-UUID die vastlegt waar de stap over gaat: de vergadering waarin de persoon is geïnstalleerd, het geleverde introductiepakket, het aangemaakte lidmaatschap.",
        "Optional quorum fraction (e.g. '2/3') required for this category per the akte. Null means no quorum beyond the body default.": "Optional quorum fraction (e.g. '2/3') required for this category per the akte. Null means no quorum beyond the body default.",
        "Oral question": "Mondelinge vraag",
        "Oral questions": "Mondelinge vragen",
        "Order": "Volgorde",
        "Order saved": "Volgorde opgeslagen",
        "Ordered states an item of this kind moves through.": "Geordende statussen die een punt van dit soort doorloopt.",
        "Orders": "Bestellingen",
        "Organisation": "Organisatie",
        "Organisation mode": "Organisatiemodus",
        "Organization": "Organisatie",
        "Organization defaults applied to meetings, decisions, and generated documents": "Organisatiestandaarden voor vergaderingen, besluiten en gegenereerde documenten",
        "Organization name": "Organisatienaam",
        "Organization settings saved": "Organisatie-instellingen opgeslagen",
        "Origin commitment": "Oorspronkelijke toezegging",
        "Other": "Overig",
        "Other activities": "Overige activiteiten",
        "Other position": "Nevenfunctie",
        "Other positions": "Nevenfuncties",
        "Outcome": "Uitkomst",
        "Outstanding": "Uitstaand",
        "Over limit": "Over de limiet",
        "Over time": "Over de tijd",
        "Overall score: {score}": "Totaalscore: {score}",
        "Overdue": "Te laat",
        "Overdue actions": "Te late acties",
        "Overig": "Overig",
        "Overlapping edit conflict detected": "Overlappend bewerkingsconflict gedetecteerd",
        "Overlegvergadering": "Overlegvergadering",
        "Owner": "Eigenaar",
        "Owning body": "Eigenaarsorgaan",
        "P&C cycle": "P&C-cyclus",
        "P&C cycles": "P&C-cycli",
        "P&C step": "P&C-stap",
        "PDF (via Docudesk when available)": "PDF (via Docudesk indien beschikbaar)",
        "Package assembly failed": "Samenstellen vergaderstukken mislukt",
        "Package assembly failed.": "Samenstellen vergaderstukken mislukt.",
        "Parafering": "Parafering",
        "Parent Motion": "Moedermotie",
        "Parent motion": "Ouder-motie",
        "Parent motion not found": "Ouder-motie niet gevonden",
        "Participant": "Deelnemer",
        "Participant UUID": "Deelnemer UUID",
        "Participants": "Deelnemers",
        "Participating organisations": "Deelnemende organisaties",
        "Participation": "Participatie",
        "Participatory budget": "Participatiebudget",
        "Participatory budget rounds": "Participatieve begrotingsrondes",
        "Participatory budgets": "Participatieve begrotingen",
        "Partly adopted": "Gedeeltelijk overgenomen",
        "Party": "Partij",
        "Party affiliation": "Partijaffiliatie",
        "Pause": "Pauzeren",
        "Pause timer": "Timer pauzeren",
        "Paused": "Gepauzeerd",
        "Pending": "Openstaand",
        "Pending vote": "Openstaande stemming",
        "Pending votes": "Openstaande stemmen",
        "Pending votes: %s": "Openstaande stemmen: %s",
        "Per person": "Per persoon",
        "Per-dimension and free-text breakdowns are hidden: too few respondents to protect anonymity.": "Uitsplitsingen per dimensie en vrije tekst zijn verborgen: te weinig respondenten om anonimiteit te waarborgen.",
        "Person": "Persoon",
        "Personal data note": "Persoonsgegevens notitie",
        "Personal settings": "Persoonlijke instellingen",
        "Petition": "Petitie",
        "Phone for urgent matters": "Telefoonnummer voor dringende zaken",
        "Photo": "Foto",
        "Pick a participant": "Kies een deelnemer",
        "Pick a participant to link to this governance body.": "Kies een deelnemer om aan dit orgaan te koppelen.",
        "Pick a participant to link to this meeting.": "Kies een deelnemer om aan deze vergadering te koppelen.",
        "Pick a participant to request a signature from.": "Kies een deelnemer om een handtekening van te vragen.",
        "Pick a report to open it.": "Kies een rapport om het te openen.",
        "Pick the organisation this app runs for and you get a worked example of it: bodies, meetings, decisions and templates that fit. Installing the app plants nothing on its own, so this is the only step that adds data. Choose \"None\" on a production install.": "Kies de organisatie waarvoor deze app draait en je krijgt een uitgewerkt voorbeeld: organen, vergaderingen, besluiten en sjablonen die daarbij passen. De app zet zelf niets klaar, dus dit is de enige stap die gegevens toevoegt. Kies \"Geen\" op een productieomgeving.",
        "Pick the organisation this app runs for. You get a worked example of it: bodies, meetings, decisions and templates that fit. Pick several if you want to compare them. Installing the app plants nothing on its own, so this step is the only one that adds data. Choose \"None\" on a production install.": "Kies de organisatie waarvoor deze app draait. Je krijgt een uitgewerkt voorbeeld: organen, vergaderingen, besluiten en sjablonen die daarbij passen. Kies er meerdere als je ze wilt vergelijken. De app zet zelf niets klaar, dus dit is de enige stap die gegevens toevoegt. Kies \"Geen\" op een productieomgeving.",
        "Placeholder: comment added": "Placeholder: reactie toegevoegd",
        "Placeholder: status changed to Review": "Placeholder: status gewijzigd naar Review",
        "Placeholder: user opened a record": "Placeholder: gebruiker opende een record",
        "Planned": "Gepland",
        "Planned agenda": "Termijnagenda",
        "Planned agenda item": "Punt op de termijnagenda",
        "Planned length, used to prefill the end date.": "Geplande duur, gebruikt om de einddatum vooraf in te vullen.",
        "Planned period": "Geplande periode",
        "Planning": "Planning",
        "Planning cycle": "Planning- en controlcyclus",
        "Planning cycle step": "Stap in de planning- en controlcyclus",
        "Planning cycle template": "Sjabloon voor de planning- en controlcyclus",
        "Policy": "Beleid",
        "Policy rule": "Beleidsregel",
        "Political group assignment": "Fractie toewijzing",
        "Portefeuillehouder": "Portefeuillehouder",
        "Portfolio holder": "Portefeuillehouder",
        "Position": "Functie",
        "Position hold": "Functiebekleding",
        "Position holder": "Functievervuller",
        "Position holders": "Functievervullers",
        "Position type": "Functietype",
        "Position types": "Functiesoorten",
        "Positions": "Functies",
        "Positions additionally competent to take decisions of this type.": "Functies die daarnaast bevoegd zijn besluiten van dit type te nemen.",
        "Positive": "Positief",
        "Positive advice": "Positief advies",
        "Positive with reservations": "Positief met kanttekeningen",
        "Possible conflict with another amendment — consult the clerk": "Mogelijk conflict met een ander amendement — raadpleeg de griffier",
        "Post-moderation (auto-approve authenticated)": "Namoderatie (ingelogde reacties automatisch goedkeuren)",
        "Postponed": "Verschoven",
        "Power of attorney": "Volmacht",
        "Pre-moderation (approve before counting)": "Voormoderatie (goedkeuren voor meetellen)",
        "Preferences": "Voorkeuren",
        "Preferred language for communications": "Voorkeurstaal voor communicatie",
        "Previous month": "Vorige maand",
        "Private": "Privé",
        "Process template": "Processjabloon",
        "Process templates": "Processjablonen",
        "Processed": "Verwerkt",
        "Processing": "Bezig",
        "Processing notes": "Toelichting op de verwerking",
        "Products": "Producten",
        "Project to Deck": "Projecteren naar Deck",
        "Projected to Deck": "Geprojecteerd naar Deck",
        "Projection failed.": "Projectie mislukt.",
        "Prompt on transition": "Vragen bij overgang",
        "Proof package sealed (SHA-256 {hash}).": "Bewijspakket verzegeld (SHA-256 {hash}).",
        "Properties": "Eigenschappen",
        "Proposal": "Voorstel",
        "Proposal submitted": "Voorstel ingediend",
        "Proposal title": "Titel voorstel",
        "Proposals": "Voorstellen",
        "Propose": "Voorstellen",
        "Propose agenda item": "Agendapunt voorstellen",
        "Proposed": "Voorgesteld",
        "Proposed items": "Voorgestelde agendapunten",
        "Proposer": "Indiener",
        "Proxies are granted per voting round from the voting panel.": "Volmachten worden per stemronde verleend vanuit het stempaneel.",
        "Proxy authorization": "Volmacht",
        "Proxy authorizations": "Volmachten",
        "Proxy row & meeting": "Volmachtregel en vergadering",
        "Public": "Openbaar",
        "Public consultation": "Publieke consultatie",
        "Public consultations": "Publieke consultaties",
        "Public publication": "Openbare publicatie",
        "Publicatie in behandeling": "Publicatie in behandeling",
        "Publicatiedatum": "Publicatiedatum",
        "Publication": "Publicatie",
        "Publication configuration saved.": "Publicatieconfiguratie opgeslagen.",
        "Publication error": "Publicatiefout",
        "Publication failed": "Publicatie mislukt",
        "Publication failed.": "Publicatie mislukt.",
        "Publication history": "Publicatiegeschiedenis",
        "Publication pending": "Publicatie in behandeling",
        "Publication warning": "Publicatiewaarschuwing",
        "Publiceren naar ORI": "Publiceren naar ORI",
        "Publish": "Publiceren",
        "Publish agenda": "Agenda publiceren",
        "Publish allocation": "Toewijzing publiceren",
        "Publish now": "Nu publiceren",
        "Publish results": "Resultaten publiceren",
        "Publish summary": "Samenvatting publiceren",
        "Publish this decision?": "Dit besluit publiceren?",
        "Publish to ORI": "Publiceren naar ORI",
        "Published": "Gepubliceerd",
        "Published as {oriType} (version {version}) on {date}": "Gepubliceerd als {oriType} (versie {version}) op {date}",
        "Published publicly": "Openbaar gepubliceerd",
        "Published to ORI": "Gepubliceerd naar ORI",
        "Publishing to the OpenCatalogi catalog failed.": "Publiceren naar de OpenCatalogi-catalogus is mislukt.",
        "Put": "Gesteld",
        "Qualified": "Met voorbehoud",
        "Qualified majority (2/3)": "Gekwalificeerde meerderheid (2/3)",
        "Qualified majority (2/3) with elevated quorum.": "Gekwalificeerde meerderheid (2/3) met verhoogd quorum.",
        "Qualified majority (3/4)": "Gekwalificeerde meerderheid (3/4)",
        "Question": "Vraag",
        "Questions raised": "Vragen gesteld",
        "Quick actions": "Snelle acties",
        "Quorum": "Quorum",
        "Quorum %": "Quorum %",
        "Quorum (members)": "Quorum (leden)",
        "Quorum Met": "Quorum gehaald",
        "Quorum Percentage": "Quorumpercentage",
        "Quorum Required": "Quorum vereist",
        "Quorum Rule": "Quorumregel",
        "Quorum fraction": "Quorum fraction",
        "Quorum niet bereikt": "Quorum niet bereikt",
        "Quorum not reached": "Quorum niet bereikt",
        "Quorum required": "Vereist quorum",
        "Quorum required before voting": "Quorum vereist voor stemming",
        "Quorum rule": "Quorumregel",
        "Raadswisseling batch": "Raadswisseling batch",
        "Ranked Choice": "Voorkeursstemmen",
        "Ratification": "Bekrachtiging",
        "Ratification deadline": "Bekrachtigingstermijn",
        "Ratified": "Bekrachtigd",
        "Rationale": "Toelichting",
        "Re-align to agenda": "Opnieuw uitlijnen op agenda",
        "Reaction": "Reactie",
        "Reaction approved": "Reactie goedgekeurd",
        "Reaction moderation queue": "Moderatiewachtrij reacties",
        "Reaction rejected": "Reactie afgekeurd",
        "Reactions": "Reacties",
        "Realised": "Gerealiseerd",
        "Reappointable": "Herbenoembaar",
        "Reason for recusal": "Reden voor ontheffing",
        "Reason for the correction (optional)": "Reden voor de correctie (optioneel)",
        "Reason, required to reject": "Reden, verplicht bij afwijzen",
        "Reason:": "Reden:",
        "Received": "Ontvangen",
        "Recent activity": "Recente activiteit",
        "Recent decisions": "Recente besluiten",
        "Recipient": "Ontvanger",
        "Reclaim task": "Taak terugvorderen",
        "Reclaimed": "Teruggevorderd",
        "Record decision": "Besluit vastleggen",
        "Recorded absences": "Geregistreerde afwezigheden",
        "Recorded during minute-taking on agenda item: {title}": "Vastgelegd tijdens het notuleren bij agendapunt: {title}",
        "Recording retention": "Bewaren van opnames",
        "Recording source": "Opnamebron",
        "Rectification publishes a corrected new version and withdraws the current one in a single operation. The new version references the version it corrects.": "Rectificatie publiceert een gecorrigeerde nieuwe versie en trekt de huidige in één bewerking in. De nieuwe versie verwijst naar de versie die wordt gecorrigeerd.",
        "Rectified": "Gerectificeerd",
        "Rectify": "Rectificeren",
        "Rectify publication": "Publicatie rectificeren",
        "Rectify…": "Rectificeren…",
        "Recurring": "Terugkerend",
        "Recurring series": "Terugkerende reeks",
        "Reference to the Meeting where the person was installed. The meeting itself is owned elsewhere and only linked here.": "Verwijzing naar de vergadering waarin de persoon is geïnstalleerd. De vergadering zelf hoort elders thuis en wordt hier alleen gekoppeld.",
        "Reference to the Membership created as an outcome. Optional until created: an intake often onboards a person before the membership row exists.": "Verwijzing naar het lidmaatschap dat hieruit is ontstaan. Optioneel tot het bestaat: bij een instroomronde begint het aantreden vaak voordat het lidmaatschap is vastgelegd.",
        "Reference to the Membership to be end-dated. Required. End-dating happens on the end-membership step, never as an automatic side effect.": "Verwijzing naar het lidmaatschap dat een einddatum krijgt. Verplicht. Het einddatum zetten gebeurt in de stap end-membership, nooit als automatisch neveneffect.",
        "Reference to the governance body the person is joining. Required. With targetRole it derives the body-role group mapping the assign-groups step uses.": "Verwijzing naar het orgaan waar de persoon toetreedt. Verplicht. Samen met targetRole leidt dit de groepskoppeling af die de stap assign-groups gebruikt.",
        "Reference to the governance body the person is leaving. Required. With the person's role it derives the body-role group mapping the revoke-groups step uses.": "Verwijzing naar het orgaan dat de persoon verlaat. Verplicht. Samen met de rol van de persoon leidt dit de groepskoppeling af die de stap revoke-groups gebruikt.",
        "Referenced by": "Verwezen door",
        "Referred to executive board for disposal": "In handen college ter afdoening",
        "Referred to executive board for preparation": "In handen college ter voorbereiding",
        "Refers to": "Verwijst naar",
        "Refused": "Geweigerd",
        "Register": "Register",
        "Register Configuration": "Registerconfiguratie",
        "Register successfully reimported.": "Register succesvol opnieuw geïmporteerd.",
        "Registered": "Geregistreerd",
        "Registers": "Registers",
        "Reglement": "Reglement",
        "Reglement source": "Bron van het reglement",
        "Reglement van orde": "Reglement van orde",
        "Regulation": "Regeling",
        "Regulation version": "Versie van de regeling",
        "Regulations": "Reglement",
        "Reimport Register": "Register opnieuw importeren",
        "Reject": "Afwijzen",
        "Reject correction": "Correctie afwijzen",
        "Reject minutes": "Notulen afkeuren",
        "Reject proposal {title}": "Voorstel afwijzen: {title}",
        "Reject reaction": "Reactie afkeuren",
        "Rejected": "Verworpen",
        "Rejecting": "Tegenstemmend",
        "Rejection comment": "Toelichting bij afkeuring",
        "Rejection reason": "Reden voor afkeuring",
        "Reject…": "Afkeuren…",
        "Related": "Gerelateerd",
        "Related Meetings": "Gerelateerde vergaderingen",
        "Related Participants": "Gerelateerde deelnemers",
        "Related decisions": "Gerelateerde besluiten",
        "Relation rejected": "Relatie geweigerd",
        "Relation type": "Relatietype",
        "Relocation": "Verhuizing",
        "Reminder": "Herinnering",
        "Reminder 3 months": "Rappel 3 maanden",
        "Reminder 6 months": "Rappel 6 maanden",
        "Remove": "Verwijderen",
        "Remove failed.": "Verwijderen mislukt.",
        "Remove from body": "Uit orgaan verwijderen",
        "Remove from consent agenda": "Uit hamerstukken halen",
        "Remove from meeting": "Uit vergadering verwijderen",
        "Remove member": "Lid verwijderen",
        "Remove member from workspace": "Lid uit werkruimte verwijderen",
        "Remove relation": "Relatie verwijderen",
        "Remove signer": "Ondertekenaar verwijderen",
        "Remove spokesperson": "Spreker verwijderen",
        "Remove state": "Status verwijderen",
        "Remove transition": "Overgang verwijderen",
        "Remove {name} from queue": "{name} uit wachtrij verwijderen",
        "Remove {title} from consent agenda": "{title} uit hamerstukken halen",
        "Removed text": "Verwijderde tekst",
        "Remunerated": "Bezoldigd",
        "Reopen round (revote)": "Ronde heropenen (herstemming)",
        "Repealed": "Ingetrokken",
        "Repealed by": "Ingetrokken door",
        "Repeals": "Trekt in",
        "Replaced": "Vervangen",
        "Reply": "Antwoorden",
        "Report adoption": "Vaststelling verslag",
        "Reported": "Gemeld",
        "Reports": "Rapporten",
        "Representation seat": "Afvaardigingszetel",
        "Representation seats": "Afvaardigingszetels",
        "Requested": "Aangevraagd",
        "Requested amount": "Gevraagd bedrag",
        "Requested by": "Aangevraagd door",
        "Requested response": "Gevraagde reactie",
        "Requester": "Verzoeker",
        "Requesting body": "Verzoekend orgaan",
        "Requires confirmation": "Vereist bekrachtiging",
        "Reserve fund contribution": "Reservefonds dotatie",
        "Resignation": "Ontslag op eigen verzoek",
        "Resolution": "Resolutie",
        "Resolution \"%1$s\" was adopted": "Resolutie \"%1$s\" is aangenomen",
        "Resolution not found": "Besluit niet gevonden",
        "Resolution {object} was adopted": "Resolutie {object} is aangenomen",
        "Resolutions": "Besluiten",
        "Resolutions ({n})": "Besluiten ({n})",
        "Resolutions are proposed from a board meeting.": "Besluiten worden voorgesteld vanuit een bestuursvergadering.",
        "Resolve thread": "Discussie afronden",
        "Respond anonymously": "Anoniem reageren",
        "Respondent": "Respondent",
        "Respondent body": "Reagerende organisatie",
        "Respondent identifier": "Aanduiding van de respondent",
        "Response": "Reactie",
        "Responses": "Reacties",
        "Restore version": "Versie herstellen",
        "Restricted": "Beperkt",
        "Restrictions": "Beperkingen",
        "Result": "Resultaat",
        "Result:": "Uitslag:",
        "Resultaat opslaan": "Resultaat opslaan",
        "Results published": "Resultaten gepubliceerd",
        "Results published (with a warning)": "Resultaten gepubliceerd (met een waarschuwing)",
        "Resume": "Hervatten",
        "Resume timer": "Timer hervatten",
        "Retention policy": "Bewaarbeleid",
        "Retirement schedule": "Rooster van aftreden",
        "Retirement schedules": "Roosters van aftreden",
        "Retraction from the OpenCatalogi catalog failed and is pending retry — the record is no longer publicly readable but the catalog still lists it.": "Intrekking uit de OpenCatalogi-catalogus is mislukt en wordt opnieuw geprobeerd — het record is niet meer openbaar leesbaar, maar de catalogus vermeldt het nog wel.",
        "Retry transcription": "Transcriptie opnieuw proberen",
        "Returned to draft": "Teruggezet naar concept",
        "Reviewed": "Beoordeeld",
        "Revise agenda": "Agenda herzien",
        "Revoke Proxy": "Volmacht intrekken",
        "Revoke groups": "Groepen intrekken",
        "Revoke proxy": "Volmacht intrekken",
        "Revoked": "Ingetrokken",
        "Role": "Rol",
        "Ronde": "Ronde",
        "Rounds adopted": "Aangenomen rondes",
        "Rounds held": "Gehouden rondes",
        "Rounds tied": "Rondes met staken van stemmen",
        "Route": "Route",
        "Route Complete": "Route afgerond",
        "Routing advice": "Routeringsadvies",
        "Routing confirmed": "Routering bevestigd",
        "Routing determined": "Routering vastgesteld",
        "Rules of procedure": "Reglement van orde",
        "Rules: {threshold} · {abstentions} · {tieBreak} — base: {base}": "Regels: {threshold} · {abstentions} · {tieBreak} — basis: {base}",
        "Running processes": "Lopende processen",
        "Sample values generated from the schemas themselves. Use it to exercise the data model, not to demo the app.": "Voorbeeldwaarden die uit de schema's zelf worden gegenereerd. Gebruik dit om het datamodel te testen, niet om de app te demonstreren.",
        "Save": "Opslaan",
        "Save Result": "Resultaat opslaan",
        "Save communication preferences": "Communicatievoorkeuren opslaan",
        "Save delegation": "Delegatie opslaan",
        "Save display preferences": "Weergavevoorkeuren opslaan",
        "Save failed.": "Opslaan mislukt.",
        "Save notification preferences": "Meldingsvoorkeuren opslaan",
        "Save order": "Volgorde opslaan",
        "Save publication configuration": "Publicatieconfiguratie opslaan",
        "Save result": "Resultaat opslaan",
        "Save retention policy": "Bewaarbeleid opslaan",
        "Save show-of-hands result": "Handopsteking resultaat opslaan",
        "Save voting order": "Stemvolgorde opslaan",
        "Saved": "Opgeslagen",
        "Saving failed.": "Opslaan mislukt.",
        "Saving the order failed": "Opslaan van de volgorde mislukt",
        "Saving the order failed.": "Opslaan van de volgorde mislukt.",
        "Saving …": "Opslaan …",
        "Saving...": "Opslaan...",
        "Saving…": "Opslaan…",
        "Schedule": "Plannen",
        "Schedule a board meeting": "Plan een bestuursvergadering",
        "Schedule a meeting from a board's detail page.": "Plan een vergadering vanaf de detailpagina van een bestuur.",
        "Schedule board meeting": "Bestuursvergadering plannen",
        "Schedule your first meeting. Give it a title and a date, then save. You can add agenda items, motions and decisions later.": "Plan je eerste vergadering. Geef die een titel en een datum en sla op. Agendapunten, moties en besluiten kun je later toevoegen.",
        "Scheduled": "Gepland",
        "Scheduled Date": "Geplande datum",
        "Scheduled date": "Geplande datum",
        "Scope": "Reikwijdte",
        "Search across all governance data": "Zoek in alle bestuursgegevens",
        "Search decisions…": "Besluiten zoeken…",
        "Search meetings, motions, decisions…": "Zoek vergaderingen, moties, besluiten…",
        "Search results": "Zoekresultaten",
        "Searching…": "Zoeken…",
        "Seat position": "Zetelfunctie",
        "Seats": "Zetels",
        "Secret Ballot": "Geheime stemming",
        "Secret ballot": "Geheime stemming",
        "Secret ballot with candidate rounds.": "Geheime stemming met kandidaatrondes.",
        "Secretary": "Secretaris",
        "Section discarded — write your own text in the minutes editor.": "Sectie verworpen — schrijf je eigen tekst in de notuleneditor.",
        "Section summary": "Samenvatting van sectie",
        "Select a motion from the same meeting to link.": "Selecteer een motie uit dezelfde vergadering om te koppelen.",
        "Select a participant": "Selecteer een deelnemer",
        "Select a relation type and a target decision.": "Selecteer een relatietype en een doelbesluit.",
        "Self-evaluation": "Zelfevaluatie",
        "Send notice": "Convocatie versturen",
        "Sender": "Afzender",
        "Sent": "Verzonden",
        "Sent at": "Verzonden op",
        "Series": "Reeks",
        "Series error": "Reeksfout",
        "Series generated": "Reeks gegenereerd",
        "Series generation failed.": "Reeksgeneratie mislukt.",
        "Set Up Body": "Orgaan instellen",
        "Settings": "Instellingen",
        "Settings error": "Instellingenfout",
        "Settings saved successfully": "Instellingen succesvol opgeslagen",
        "Settled": "Afgedaan",
        "Settled commitment": "Afgedane toezegging",
        "Share of the governance body present at this meeting, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Het aandeel van het bestuursorgaan dat bij deze vergadering aanwezig is, in procenten. Wordt door OpenRegister bij het opslaan gematerialiseerd uit x-openregister-calculations. Wordt nooit door decidiq geschreven.",
        "Share of this goal's direct child goals that are achieved, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Het aandeel directe subdoelen van dit doel dat is behaald, in procenten. Wordt door OpenRegister bij het opslaan gematerialiseerd uit x-openregister-calculations. Wordt nooit door decidiq geschreven.",
        "Share of this goal's linked action items that are completed, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Het aandeel actiepunten bij dit doel dat is afgerond, in procenten. Wordt door OpenRegister bij het opslaan gematerialiseerd uit x-openregister-calculations. Wordt nooit door decidiq geschreven.",
        "Share of this goal's linked commitments that are settled, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Het aandeel toezeggingen bij dit doel dat is afgehandeld, in procenten. Wordt door OpenRegister bij het opslaan gematerialiseerd uit x-openregister-calculations. Wordt nooit door decidiq geschreven.",
        "Share of this meeting's action items that are completed, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Het aandeel actiepunten van deze vergadering dat is afgerond, in procenten. Wordt door OpenRegister bij het opslaan gematerialiseerd uit x-openregister-calculations. Wordt nooit door decidiq geschreven.",
        "Shared body": "Gemeenschappelijk orgaan",
        "Shared-body participations": "Deelnames gemeenschappelijke regeling",
        "Shifted": "Verschoven",
        "Shortened debate and voting windows.": "Verkorte debat- en stemtermijnen.",
        "Show": "Tonen",
        "Show meeting cost": "Vergaderkosten tonen",
        "Show of Hands": "Handopsteking",
        "Show of hands": "Handopsteking",
        "Sign": "Ondertekenen",
        "Sign now": "Nu ondertekenen",
        "Signature": "Handtekening",
        "Signatures": "Handtekeningen",
        "Signed": "Ondertekend",
        "Signers": "Ondertekenaars",
        "Signing failed.": "Ondertekenen mislukt.",
        "Simple majority (50%+1)": "Gewone meerderheid (50%+1)",
        "Simple majority of votes cast, default quorum.": "Gewone meerderheid van uitgebrachte stemmen, standaardquorum.",
        "Simulated": "Gesimuleerd",
        "Skipped": "Overgeslagen",
        "Sluiten": "Sluiten",
        "Sluitingstijd (optioneel)": "Sluitingstijd (optioneel)",
        "Source app": "Bronapplicatie",
        "Spanish": "Spaans",
        "Speaker queue": "Sprekerswachtrij",
        "Speaking duration": "Spreektijd",
        "Speaking limit (min)": "Spreeklimiet (min)",
        "Speaking-time distribution": "Verdeling spreektijd",
        "Specialized templates": "Gespecialiseerde sjablonen",
        "Specialized templates apply to specific decision types; the default applies when none is chosen.": "Gespecialiseerde sjablonen gelden voor specifieke besluittypen; het standaardsjabloon geldt wanneer er geen is gekozen.",
        "Speeches": "Bijdragen",
        "Splitsingsakte": "Splitsingsakte",
        "Spokesperson": "Spreker",
        "Stable short name for the position within its body.": "Vaste korte naam voor de functie binnen het orgaan.",
        "Stage": "Fase",
        "Standard decision": "Standaardbesluit",
        "Standpunt": "Standpunt",
        "Start": "Starten",
        "Start date": "Startdatum",
        "Start decision-making process": "Besluitvormingsproces starten",
        "Start deliberation": "Beraadslaging starten",
        "Start evaluation": "Evaluatie starten",
        "Start process": "Proces starten",
        "Start taking minutes": "Beginnen met notuleren",
        "Start timer": "Timer starten",
        "Started": "Gestart",
        "Starter overview with sample KPIs and activity placeholders. Replace this view with your own data.": "Startoverzicht met voorbeeld-KPI's en activiteitsplaceholders. Vervang dit scherm door je eigen gegevens.",
        "State machine": "Toestandsmachine",
        "State name": "Statusnaam",
        "States": "Statussen",
        "Status": "Status",
        "Status message": "Statusmelding",
        "Statute amendment": "Statutenwijziging",
        "Statuten": "Statuten",
        "Statutory basis": "Wettelijke grondslag",
        "Stem tegen": "Stem tegen",
        "Stem uitbrengen mislukt": "Stem uitbrengen mislukt",
        "Stem voor": "Stem voor",
        "Stemmen tegen": "Stemmen tegen",
        "Stemmen voor": "Stemmen voor",
        "Stemmethode": "Stemmethode",
        "Stemronde": "Stemronde",
        "Stemronde is al geopend — volmacht kan niet meer worden ingetrokken": "Stemronde is al geopend — volmacht kan niet meer worden ingetrokken",
        "Stemronde is gesloten": "Stemronde is gesloten",
        "Stemronde is nog niet geopend": "Stemronde is nog niet geopend",
        "Stemronde openen": "Stemronde openen",
        "Stemronde openen mislukt": "Stemronde openen mislukt",
        "Stemronde sluiten": "Stemronde sluiten",
        "Stemronde sluiten? {notVoted} van {total} leden hebben nog niet gestemd.": "Stemronde sluiten? {notVoted} van {total} leden hebben nog niet gestemd.",
        "Step {step}": "Stap {step}",
        "Step {step} of {total}": "Stap {step} van {total}",
        "Steps": "Stappen",
        "Still to do: stage {seq} ({maker})": "Nog te doen: fase {seq} ({maker})",
        "Stop": "Stoppen",
        "Stop {name}": "{name} stoppen",
        "Store": "Store",
        "Sub-item title": "Titel van subpunt",
        "Sub-item: {title}": "Subpunt: {title}",
        "Sub-items of {title}": "Subpunten van {title}",
        "Subject": "Onderwerp",
        "Subject description": "Omschrijving van het onderwerp",
        "Subject documents": "Onderliggende documenten",
        "Submission phase": "Indieningsfase",
        "Submission window (hours)": "Indieningstermijn (uren)",
        "Submit Amendment": "Amendement indienen",
        "Submit Motion": "Motie indienen",
        "Submit amendment": "Amendement indienen",
        "Submit anonymously": "Anoniem verzenden",
        "Submit declaration": "Verklaring indienen",
        "Submit for review": "Indienen ter beoordeling",
        "Submit proposal": "Voorstel indienen",
        "Submit reaction": "Reactie indienen",
        "Submit suggestion": "Voorstel indienen",
        "Submitted": "Ingediend",
        "Submitted At": "Ingediend op",
        "Submitter": "Indiener",
        "Substitute": "Vervanger",
        "Suggest a correction": "Een correctie voorstellen",
        "Suggest order": "Volgorde voorstellen",
        "Suggest order, most far-reaching first": "Volgorde voorstellen, meest verstrekkende eerst",
        "Superseded": "Vervangen",
        "Superseded by": "Vervangen door",
        "Supersedes": "Vervangt",
        "Support": "Ondersteuning",
        "Support this motion": "Ondersteunen",
        "Support threshold type": "Type steundrempel",
        "Support threshold value": "Waarde steundrempel",
        "Swearing in": "Beediging",
        "TOOI URI": "TOOI-URI",
        "Table": "Tabel",
        "Talk recording": "Talk-opname",
        "Tallying": "Tellen",
        "Target OpenCatalogi catalog": "Doel-OpenCatalogi-catalogus",
        "Target agenda item": "Doelagendapunt",
        "Target decision": "Doelbesluit",
        "Target document": "Doeldocument",
        "Task": "Taak",
        "Task group": "Werkgroep",
        "Task status": "Taakstatus",
        "Task title": "Taaktitel",
        "Tasks": "Taken",
        "Tasks & Commitments": "Taken en toezeggingen",
        "Team members": "Teamleden",
        "Technical questions": "Technische vragen",
        "Tegen": "Tegen",
        "Template assignment saved": "Sjabloontoewijzing opgeslagen",
        "Template category": "Sjablooncategorie",
        "Tenders": "Aanbestedingen",
        "Term": "Termijn",
        "Term (months)": "Zittingstermijn (maanden)",
        "Term End": "Termijn einde",
        "Term Start": "Termijn start",
        "Term duration (months)": "Zittingsduur (maanden)",
        "Term entries": "Zittingstermijnen",
        "Term entry": "Zittingstermijn",
        "Term no.": "Termijnnr.",
        "Term number": "Termijnnummer",
        "Term passed": "Termijn verstreken",
        "Term rule": "Zittingsregel",
        "Term rules": "Zittingsregels",
        "Text": "Tekst",
        "Text changes": "Tekstwijzigingen",
        "That action was refused.": "Die actie is geweigerd.",
        "That relation already exists.": "Die relatie bestaat al.",
        "The CSV file contains no rows.": "Het CSV-bestand bevat geen regels.",
        "The CSV must have a header row with name and email columns.": "De CSV moet een kopregel met de kolommen name en email hebben.",
        "The Nextcloud group being asked, when the audience is one.": "De Nextcloud-groep die wordt geraadpleegd, als de doelgroep er een is.",
        "The action failed.": "De actie is mislukt.",
        "The agenda item this consultation feeds.": "Het agendapunt waarvoor deze raadpleging input levert.",
        "The agenda item type this item is an instance of.": "Het agendapunttype waarvan dit punt een instantie is.",
        "The agenda item where the accounts were adopted.": "Het agendapunt waar de jaarrekening is vastgesteld.",
        "The amendment voting order has been saved.": "De stemvolgorde van de amendementen is opgeslagen.",
        "The answer in the respondent's own words.": "De reactie in de eigen woorden van de respondent.",
        "The body putting the question.": "Het orgaan dat de vraag stelt.",
        "The body the membership sits on.": "Het orgaan waarin het lidmaatschap zitting heeft.",
        "The body whose accounts were examined.": "Het orgaan waarvan de jaarrekening is onderzocht.",
        "The choices someone may pick from, one per entry. Only used when the field type is a pick from a list; leave it empty for every other field type.": "De keuzes waaruit iemand mag kiezen, één per regel. Wordt alleen gebruikt als het veldtype een keuze uit een lijst is; laat het leeg bij elk ander veldtype.",
        "The commitment this letter settles, when it settles one.": "De toezegging die deze brief afdoet, als hij er een afdoet.",
        "The commitment this planned item follows up, when it follows one.": "De toezegging waar dit geplande punt op volgt, als het ergens op volgt.",
        "The committee’s conclusion on the accounts for that year.": "De conclusie van de commissie over de jaarrekening van dat jaar.",
        "The consultation being answered.": "De raadpleging waarop dit een reactie is.",
        "The consultation of the constituency this request started, when one was started. Optional: a request with none keeps a usable lifecycle and a detail page with an empty reference. This capability defines no poll mechanics of its own; it only points at one.": "De achterbanraadpleging die bij deze aanvraag is gestart, als er een is gestart. Optioneel: een aanvraag zonder raadpleging houdt een bruikbare status en een detailpagina met een lege verwijzing. Deze functie regelt zelf niets over het raadplegen; ze verwijst er alleen naar.",
        "The current status of this regulation. The lifecycle moves it from in preparation to adopted, in effect and lapsed.": "De huidige status van deze regeling. De levenscyclus loopt van in voorbereiding naar vastgesteld, in werking en vervallen.",
        "The decision that adopted this answer.": "Het besluit waarin deze reactie is overgenomen.",
        "The decision that made this appointment.": "Het besluit waarmee deze benoeming is gedaan.",
        "The decision the answers inform.": "Het besluit waarvoor de reacties input zijn.",
        "The decision type a vote on this kind of item produces.": "Het besluittype dat een stemming over dit soort punt oplevert.",
        "The decision type this decision is an instance of.": "Het besluittype waarvan dit besluit een instantie is.",
        "The document that constitutes this body: a splitsingsakte, articles of association, statutes or a gemeenschappelijke regeling.": "Het document waarop dit orgaan berust: een splitsingsakte, oprichtingsakte, statuten of een gemeenschappelijke regeling.",
        "The document the answer was given in.": "Het document waarin de reactie is gegeven.",
        "The end date must not be before the start date.": "De einddatum mag niet voor de startdatum liggen.",
        "The financial year this statement covers.": "Het boekjaar waarop deze verklaring betrekking heeft.",
        "The fraction or party being asked, when the audience is one.": "De fractie of partij die wordt geraadpleegd, als de doelgroep er een is.",
        "The full official title, where it differs from the short one it is cited by. Optional.": "De volledige officiële titel, als die afwijkt van de korte titel waarmee het stuk wordt aangehaald. Optioneel.",
        "The governance body this configuration belongs to. One configuration per body.": "Het bestuursorgaan waar deze configuratie bij hoort. Een configuratie per orgaan.",
        "The governing document this package publishes.": "Het bestuurlijke document dat dit pakket publiceert.",
        "The gremium competent to take decisions of this type.": "Het orgaan dat bevoegd is besluiten van dit type te nemen.",
        "The gremium it is composed of.": "Het orgaan waaruit het is samengesteld.",
        "The gremium that is composed.": "Het orgaan dat wordt samengesteld.",
        "The gremium that may hold meetings of this kind.": "Het orgaan dat vergaderingen van dit soort mag houden.",
        "The gremium that owns items of this kind. Need not be the body holding the meeting. Empty means the meeting’s own body.": "Het orgaan dat punten van dit soort bezit. Dit hoeft niet het orgaan te zijn dat de vergadering houdt. Leeg betekent het orgaan van de vergadering zelf.",
        "The gremium this position exists on.": "Het orgaan waarop deze functie bestaat.",
        "The hold types valid for this position.": "De bekledingstypen die voor deze functie gelden.",
        "The identifier an external register assigned this document after publication. Stored, never minted here. Optional, and absent until first publication.": "De identificatie die een extern register aan dit document toekende na publicatie. Wordt hier alleen bewaard, nooit uitgegeven. Optioneel, en afwezig tot de eerste publicatie.",
        "The identifier of the retired record this one was copied from. Set only by the fold migration; absent on seeds and on anything a person created.": "De identificatie van het uitgefaseerde record waarvan dit is gekopieerd. Wordt alleen door de migratie gezet: niet aanwezig op voorbeeldsets of op wat iemand zelf aanmaakt.",
        "The legal instrument this body is constituted under (e.g. 'Awb 7:13', a local verordening). Free text on purpose: a closed vocabulary drawn from one country's law cannot express the instruments behind bodies in another, and a caller forced to pick from a list that does not fit will pick a wrong value.": "Het wettelijke instrument waarop dit orgaan is ingesteld (bijv. 'Awb 7:13', een gemeentelijke verordening). Bewust vrije tekst: een gesloten lijst uit het recht van één land kan de instrumenten achter organen elders niet uitdrukken, en wie moet kiezen uit een lijst die niet past, kiest een verkeerde waarde.",
        "The meeting type this meeting is an instance of.": "Het vergaderingstype waarvan deze vergadering een instantie is.",
        "The membership holding the position.": "Het lidmaatschap dat de functie bekleedt.",
        "The minutes are no longer in draft — editing is locked.": "De notulen zijn niet langer in concept — bewerken is vergrendeld.",
        "The minutes return to draft so the secretary can rework them. A comment explaining the rejection is required.": "De notulen gaan terug naar concept zodat de secretaris ze kan bijwerken. Een toelichting op de afkeuring is verplicht.",
        "The name this field's value is stored under on every agenda item of this kind. Choose it once and leave it alone: renaming it here does not move the values already recorded, so the old ones become unreachable.": "De naam waaronder de waarde van dit veld wordt opgeslagen bij elk agendapunt van deze soort. Kies hem één keer en laat hem staan: hernoemen verplaatst de al vastgelegde waarden niet, waardoor die onbereikbaar worden.",
        "The number of the version currently in force, so a list renders without resolving every row's versions. Null until a version is in force.": "Het nummer van de versie die nu geldt, zodat een lijst kan renderen zonder van elke rij de versies op te halen. Leeg tot er een versie geldt.",
        "The offboarding checklist. Required. Includes at minimum end-membership, revoke-groups (always mandatory), a personal-data note and the exit confirmation.": "De checklist voor het vertrek. Verplicht. Bevat minimaal end-membership, revoke-groups (altijd verplicht), een notitie over persoonsgegevens en de bevestiging van het vertrek.",
        "The onboarding checklist. Required. Each step carries its own status. The work behind account-linking, assign-groups and induction-pack happens elsewhere; this array records that the step exists and where it stands.": "De checklist voor het aantreden. Verplicht. Elke stap heeft een eigen status. Het werk achter account-linking, assign-groups en induction-pack gebeurt elders; deze lijst legt vast dat de stap bestaat en hoe ver hij is.",
        "The options the respondent picked, when the answer was a choice.": "De opties die de respondent koos, als de reactie een keuze was.",
        "The options to pick from, when the answer is a choice.": "De opties om uit te kiezen, als de reactie een keuze is.",
        "The organisation that answered, when the audience was organisations.": "De organisatie die reageerde, als de doelgroep organisaties waren.",
        "The person who answered, when the answer is attributed to one.": "De persoon die reageerde, als de reactie aan iemand wordt toegeschreven.",
        "The planning-cycle step this consultation belongs to.": "De stap in de planning- en controlcyclus waar deze raadpleging bij hoort.",
        "The position being held.": "De functie die wordt bekleed.",
        "The position on the composite that a representative occupies.": "De functie op het samengestelde orgaan die een afgevaardigde bekleedt.",
        "The producing app's own id for the route this one was migrated from. Paired with sourceApp it identifies the origin, so a repeated command updates this route rather than creating another.": "Het eigen id dat de bronapplicatie gebruikt voor de route waaruit deze is overgenomen. Samen met sourceApp bepaalt het de herkomst, zodat een herhaalde opdracht deze route bijwerkt in plaats van een tweede aanmaakt.",
        "The published predicate could not be set on this OpenRegister version — anonymous read is not yet available.": "Het openbaar-predicaat kon op deze OpenRegister-versie niet worden gezet — anonieme weergave is nog niet beschikbaar.",
        "The reaction is retained for audit but never counts toward the consultation. A reason is required.": "De reactie wordt bewaard voor audit maar telt nooit mee voor de consultatie. Een reden is verplicht.",
        "The referenced motion ({id}) could not be loaded.": "De gerefereerde motie ({id}) kon niet worden geladen.",
        "The requested board could not be loaded.": "Het gevraagde bestuur kon niet worden geladen.",
        "The requested board meeting could not be loaded.": "De gevraagde bestuursvergadering kon niet worden geladen.",
        "The requested resolution could not be loaded.": "Het gevraagde besluit kon niet worden geladen.",
        "The required majority for this category per the splitsingsakte (mirrors the VotingRound voteThreshold enum). Resolves into the existing round-open defaults; never a second threshold enum.": "The required majority for this category per the splitsingsakte (mirrors the VotingRound voteThreshold enum). Resolves into the existing round-open defaults; never a second threshold enum.",
        "The sentence the reader sees.": "De zin die de lezer ziet.",
        "The series is capped at 52 instances.": "De reeks is begrensd op 52 exemplaren.",
        "The server rejected this relation.": "De server heeft deze relatie geweigerd.",
        "The sign-off route could not be read.": "De paraferingsroute kon niet worden gelezen.",
        "The single body being asked, when only one is.": "Het enige orgaan dat wordt geraadpleegd, als het er een is.",
        "The state a new meeting of this kind opens in.": "De status waarin een nieuwe vergadering van dit soort opent.",
        "The state an item of this kind opens in.": "De status waarin een punt van dit soort opent.",
        "The statutory notice deadline ({deadline}) has already passed.": "De wettelijke oproepingstermijn ({deadline}) is al verstreken.",
        "The statutory notice deadline ({deadline}) is {n} day(s) away.": "De wettelijke oproepingstermijn ({deadline}) is over {n} dag(en).",
        "The summarised outcome, once the answers are in.": "De samengevatte uitkomst, zodra de reacties binnen zijn.",
        "The support threshold value.": "De waarde van de steundrempel.",
        "The template category this override applies to.": "De sjablooncategorie waarvoor deze afwijking geldt.",
        "The term on this sign-off passed without an answer, and the step declares that silence escalates, so it is now with your manager.": "De termijn van deze parafering verstreek zonder antwoord, en de stap legt vast dat stilte escaleert, dus hij ligt nu bij je leidinggevende.",
        "The term on this sign-off passed without an answer. The step declares that silence refuses, so the route has been concluded as rejected.": "De termijn van deze parafering verstreek zonder antwoord. De stap legt vast dat stilte afwijst, dus de route is als afgewezen afgesloten.",
        "The term passed without an answer. Silence approves here, so the system recorded it as approved and the next step is live.": "De termijn van deze parafering verstreek zonder antwoord. De stap legt vast dat stilte goedkeurt, dus hij is door het systeem als akkoord vastgelegd en de volgende stap staat nu open.",
        "The territorial or subject-matter competence of this body.": "De territoriale of inhoudelijke bevoegdheid van dit orgaan.",
        "The total this body’s weighted votes are expressed over. Leave empty when every member has one equal vote.": "Het totaal waarover de gewogen stemmen van dit orgaan worden uitgedrukt. Laat leeg als elk lid een gelijke stem heeft.",
        "The version of the model regulation this body follows, as plain text.": "De versie van het modelreglement dat dit orgaan volgt, als vrije tekst.",
        "The view given.": "De gegeven zienswijze.",
        "The vote is tied. As chair you must resolve it with a casting vote.": "De stemmen staken. Als voorzitter moet u dit beslechten met een beslissende stem.",
        "The vote is tied. The round may be reopened once for a revote.": "De stemmen staken. De ronde mag eenmalig worden heropend voor een herstemming.",
        "Theme session": "Themabijeenkomst",
        "There is no text to compare yet.": "Er is nog geen tekst om te vergelijken.",
        "This amendment has no proposed replacement text; the amendment text itself is compared against the motion text.": "Dit amendement heeft geen voorgestelde vervangende tekst; de amendementstekst zelf wordt vergeleken met de motietekst.",
        "This amendment is not linked to a motion, so there is no original text to compare against.": "Dit amendement is niet gekoppeld aan een motie, dus er is geen oorspronkelijke tekst om mee te vergelijken.",
        "This amendment is not linked to a motion.": "Dit amendement is niet gekoppeld aan een motie.",
        "This app needs OpenRegister to store and manage data. Please install OpenRegister from the app store to get started.": "Deze app heeft OpenRegister nodig om gegevens op te slaan en te beheren. Installeer OpenRegister via de app store om te beginnen.",
        "This body does not participate in any shared bodies yet.": "Dit orgaan neemt nog niet deel aan gemeenschappelijke organen.",
        "This decision has been enacted and the governance body is configured to prompt for publication. Publishing makes a derived public record available through OpenCatalogi. You can also publish later from the Publication tab.": "Dit besluit is bekrachtigd en het bestuursorgaan is ingesteld om te vragen om publicatie. Publiceren maakt een afgeleid openbaar record beschikbaar via OpenCatalogi. U kunt ook later publiceren via het tabblad Publicatie.",
        "This decision has no confidentiality restriction.": "Op dit besluit rust geen geheimhouding.",
        "This decision has no staged route. A stageless decision is valid.": "Dit besluit heeft geen gefaseerde route. Een besluit zonder fasen is geldig.",
        "This decision has no typed links to other decisions yet.": "Dit besluit heeft nog geen getypeerde koppelingen met andere besluiten.",
        "This decision is not a shared body's vaststellingsbesluit for any zienswijzeronde.": "Dit besluit is voor geen enkele zienswijzeronde het vaststellingsbesluit van een gemeenschappelijk orgaan.",
        "This decision was repealed{by}.": "Dit besluit is ingetrokken{by}.",
        "This decision was superseded{by}.": "Dit besluit is vervangen{by}.",
        "This delegation/mandate has no parent or sub-mandates.": "Deze delegatie/dit mandaat heeft geen bovenliggend mandaat of ondermandaten.",
        "This draft was generated by AI from the transcript. Review every section before it enters the minutes. AI never approves or publishes minutes — the normal approval workflow is unchanged.": "Dit concept is door AI gegenereerd uit het transcript. Controleer elke sectie voordat deze in de notulen terechtkomt. AI keurt nooit notulen goed of publiceert deze — de normale goedkeuringsworkflow blijft ongewijzigd.",
        "This general assembly agenda is missing legally required items:": "Op deze algemene-ledenvergaderingsagenda ontbreken wettelijk verplichte punten:",
        "This motion has no amendments to order.": "Deze motie heeft geen amendementen om te ordenen.",
        "This motion has produced no commitments yet.": "Deze motie heeft nog geen toezeggingen opgeleverd.",
        "This page is backed by the pluggable integration registry. The \"Articles\" tab is provided by the xWiki integration via OpenConnector — linked wiki pages render with their breadcrumb and a text preview.": "Deze pagina wordt ondersteund door het pluggable integratieregister. Het tabblad \"Artikelen\" wordt aangeboden door de xWiki-integratie via OpenConnector — gekoppelde wikipagina's worden weergegeven met hun broodkruimelpad en een tekstvoorvertoning.",
        "This page is backed by the pluggable integration registry. The Discussion tab is provided by the Talk integration leaf — messages posted there are linked to this motion object and visible to all participants.": "Deze pagina wordt ondersteund door het pluggable integratieregister. Het tabblad Discussie wordt aangeboden door de Talk-integratieleaf — berichten die daar worden geplaatst zijn gekoppeld aan dit agendapuntobject en zichtbaar voor alle deelnemers.",
        "This page is backed by the pluggable integration registry. When the Email integration is installed, an \"Email\" tab lets you link emails to this agenda item — the link is held by the registry, not an in-app email-link store.": "Deze pagina wordt aangedreven door het pluggable integratieregister. Wanneer de e-mailintegratie is geïnstalleerd, kunt u via een tabblad \"E-mail\" e-mails aan dit agendapunt koppelen — de koppeling wordt door het register beheerd, niet door een interne e-mailkoppelingsopslag.",
        "This page is backed by the pluggable integration registry. When the Email integration is installed, an \"Email\" tab lets you link emails to this decision dossier — the link is held by the registry, not an in-app email-link store.": "Deze pagina wordt aangedreven door het pluggable integratieregister. Wanneer de e-mailintegratie is geïnstalleerd, kunt u via een tabblad \"E-mail\" e-mails aan dit besluitdossier koppelen — de koppeling wordt door het register beheerd, niet door een interne e-mailkoppelingsopslag.",
        "This pattern creates {n} meeting(s).": "Dit patroon maakt {n} vergadering(en) aan.",
        "This respondent's own deadline, when it differs from the consultation's.": "De eigen termijn van deze respondent, als die afwijkt van die van de raadpleging.",
        "This round is the single permitted revote of the tied round.": "Deze ronde is de enige toegestane herstemming van de gestaakte ronde.",
        "This sign-off is part-way through its term, so your substitute has been asked too. You can still sign it yourself.": "Deze parafering is halverwege haar termijn, dus je vervanger is er ook bij gevraagd. Je kunt zelf gewoon tekenen.",
        "This step has nobody assigned yet.": "Deze stap heeft nog niemand toegewezen.",
        "This will set all {n} consent agenda items to \"Adopted\" (afgerond). Continue?": "Dit stelt alle {n} hamerstukken in op “Afgerond”. Doorgaan?",
        "Threshold": "Drempel",
        "Threshold a vote must clear by default.": "Drempel die een stemming standaard moet halen.",
        "Tick this to make the field compulsory: an agenda item of this kind cannot be saved while it is left empty.": "Vink dit aan om het veld verplicht te maken: een agendapunt van deze soort kan niet worden opgeslagen zolang het leeg is.",
        "Tie resolved by the chair's casting vote: {value}": "Staking beslecht door de beslissende stem van de voorzitter: {value}",
        "Tie-break rule": "Regel bij staking van stemmen",
        "Tie: chair decides": "Staking: voorzitter beslist",
        "Tie: motion fails": "Staking: motie verworpen",
        "Tie: revote (once)": "Staking: herstemming (eenmalig)",
        "Time allocation accuracy": "Nauwkeurigheid tijdsverdeling",
        "Time remaining for {title}": "Resterende tijd voor {title}",
        "Timezone": "Tijdzone",
        "Title": "Titel",
        "To": "Naar",
        "Today": "Vandaag",
        "Toetreding/uittreding": "Toetreding of uittreding",
        "Topics suggested": "Onderwerpen voorgesteld",
        "Total budget": "Totale begroting",
        "Total duration: {min} min": "Totale duur: {min} min",
        "Total votes cast: {n}": "Totaal uitgebrachte stemmen: {n}",
        "Total: {total} · Average per meeting: {average}": "Totaal: {total} · Gemiddeld per vergadering: {average}",
        "Transcribe": "Transcriberen",
        "Transcription": "Transcriptie",
        "Transcription error": "Transcriptiefout",
        "Transcription unavailable": "Transcriptie niet beschikbaar",
        "Transferred": "Overgedragen",
        "Transitie mislukt": "Transitie mislukt",
        "Transition failed": "Overgang mislukt",
        "Transition failed.": "Overgang mislukt.",
        "Transition rejected": "Overgang geweigerd",
        "Transitions": "Overgangen",
        "Treasurer": "Penningmeester",
        "Treated": "Behandeld",
        "Trigger": "Aanleiding",
        "Type": "Type",
        "Type-specific field values": "Typespecifieke veldwaarden",
        "Type-specific fields": "Typespecifieke velden",
        "Type: {type}": "Type: {type}",
        "U stemt namens: {name}": "U stemt namens: {name}",
        "Uitgebracht: {cast} / {total}": "Uitgebracht: {cast} / {total}",
        "Uitslag:": "Uitslag:",
        "Uitstaand": "Uitstaand",
        "Unanimous": "Unaniem",
        "Unassigned": "Niet toegewezen",
        "Undecided": "Onbeslist",
        "Under discussion": "In bespreking",
        "Unknown role": "Onbekende rol",
        "Unsigned": "Ongetekend",
        "Until": "Tot",
        "Until (inclusive)": "Tot en met",
        "Untitled": "Zonder titel",
        "Untitled decision": "Besluit zonder titel",
        "Upcoming meetings": "Aankomende vergaderingen",
        "Upload a CSV file with the columns: name, email, role.": "Upload een CSV-bestand met de kolommen: name, email, role.",
        "Urgent": "Urgent",
        "Urgent decision": "Spoedbesluit",
        "Urgent decisions": "Spoedbesluiten",
        "User settings will appear here in a future update.": "Gebruikersinstellingen verschijnen hier in een toekomstige update.",
        "Uuid of the OpenRegister task currently asking for this stage's sign-off. Set while the stage is active and a task surface exists; cleared when the stage stops waiting. Empty on an instance without the task surface, which changes where the ask is seen and nothing else.": "Uuid van de OpenRegister-taak die om de aftekening van deze fase vraagt. Gezet zolang de fase actief is en er een takenoppervlak bestaat; gewist zodra de fase niet meer wacht. Leeg op een omgeving zonder takenoppervlak, wat alleen verandert waar de vraag zichtbaar is.",
        "Uw stem is geregistreerd.": "Uw stem is geregistreerd.",
        "Valid from": "Geldig vanaf",
        "Valid until": "Geldig tot",
        "Value (EUR)": "Waarde (EUR)",
        "Values for the fields declared by this item’s type.": "Waarden voor de velden die door het type van dit punt zijn gedeclareerd.",
        "Verantwoordelijke organisatie": "Verantwoordelijke organisatie",
        "Verdict": "Oordeel",
        "Vergadering niet gekoppeld — stemronde kan niet worden geopend": "Vergadering niet gekoppeld — stemronde kan niet worden geopend",
        "Verlenen": "Verlenen",
        "Verordening": "Verordening",
        "Version": "Versie",
        "Version Information": "Versie-informatie",
        "Version history": "Versiegeschiedenis",
        "Version timeline": "Versietijdlijn",
        "Version {n}": "Versie {n}",
        "Verslag": "Verslag",
        "Verwerkt": "Verwerkt",
        "Verworpen": "Verworpen",
        "Verzenddatum": "Verzenddatum",
        "Vice-chair": "Vicevoorzitter",
        "View": "Bekijken",
        "View agenda item": "Agendapunt bekijken",
        "View decision": "Besluit bekijken",
        "View motion": "Motie bekijken",
        "Volmacht": "Volmacht",
        "Volmacht intrekken": "Volmacht intrekken",
        "Volmacht verlenen": "Volmacht verlenen",
        "Volmacht verlenen aan": "Volmacht verlenen aan",
        "Voor": "Voor",
        "Voor / Tegen / Onthouding": "Voor / Tegen / Onthouding",
        "Voor: {for} — Tegen: {against} — Onthouding: {abstain}": "Voor: {for} — Tegen: {against} — Onthouding: {abstain}",
        "Votable": "Stembaar",
        "Vote": "Stem",
        "Vote against": "Stem tegen",
        "Vote for": "Stem voor",
        "Vote now": "Nu stemmen",
        "Vote tally": "Stemtelling",
        "Vote threshold": "Stemdrempel",
        "Vote type": "Stemtype",
        "Vote weight denominator": "Noemer stemgewicht",
        "Voter": "Stemmer",
        "Votes": "Stemmen",
        "Votes abstain": "Onthoudingen",
        "Votes against": "Stemmen tegen",
        "Votes cast": "Uitgebracht",
        "Votes for": "Stemmen voor",
        "Voting": "Stemming",
        "Voting Default": "Standaardstemming",
        "Voting Method": "Stemmethode",
        "Voting Round": "Stemronde",
        "Voting Rounds": "Stemronden",
        "Voting Weight": "Stemgewicht",
        "Voting method": "Stemmethode",
        "Voting method this kind of meeting defaults to.": "Stemmethode die dit soort vergadering standaard gebruikt.",
        "Voting opened on \"%1$s\"": "Stemming geopend over \"%1$s\"",
        "Voting opened on {object}": "Stemming geopend over {object}",
        "Voting order": "Stemvolgorde",
        "Voting phase": "Stemfase",
        "Voting results": "Stemresultaten",
        "Voting round": "Stemronde",
        "Voting weight": "Stemgewicht",
        "VvE configuration": "VvE-configuratie",
        "VvE decision template": "VvE-besluitsjabloon",
        "VvE decision templates": "VvE-besluitsjablonen",
        "WOR article": "WOR-artikel",
        "Waiting": "Wacht",
        "Waiting on {actor}": "Wacht op {actor}",
        "Weight a vote cast by the holder of this position carries.": "Het gewicht van een stem uitgebracht door de bekleder van deze functie.",
        "Weight the component’s representation carries in votes.": "Het gewicht dat de afvaardiging van het samenstellende orgaan heeft bij stemmingen.",
        "Weighted": "Gewogen",
        "Weighted vote": "Gewogen stemming",
        "Welcome": "Welkom",
        "Welcome to Decidiq": "Welkom bij Decidiq",
        "Welcome to Decidiq!": "Welkom bij Decidiq!",
        "Welcome to Decidiq! Get started by setting up your first governing body in Settings.": "Welkom bij Decidiq! Begin door je eerste bestuursorgaan in te stellen via Instellingen.",
        "Welcome to Decidiq! Get started by setting up your first governing body.": "Welkom bij Decidiq! Begin met het instellen van je eerste bestuursorgaan.",
        "What": "Wat",
        "What a user picks when adding an agenda item.": "Wat een gebruiker kiest bij het toevoegen van een agendapunt.",
        "What a user picks when creating a meeting of this kind.": "Wat een gebruiker kiest bij het aanmaken van een vergadering van dit soort.",
        "What exactly the parties are being asked. Optional when the subject says it.": "Wat er precies aan de partijen wordt gevraagd. Optioneel als het onderwerp dat al zegt.",
        "What is being asked about, in one line.": "Waarover wordt geraadpleegd, in een regel.",
        "What is being consulted on, at more length than the subject line allows.": "Waarover wordt geraadpleegd, uitgebreider dan in de onderwerpregel past.",
        "What items of this kind carry and how they are handled.": "Wat punten van dit soort bevatten en hoe ze worden behandeld.",
        "What kind of answer is expected.": "Wat voor soort reactie wordt verwacht.",
        "What kind of answer the field takes, which decides how it is shown and what is accepted: a single line of text, a longer text, a number, a yes/no tick, a date, a pick from the list of choices below, or a link to another record.": "Wat voor antwoord het veld aanneemt, wat bepaalt hoe het wordt getoond en wat wordt geaccepteerd: één regel tekst, een langere tekst, een getal, een ja/nee-vinkje, een datum, een keuze uit de onderstaande lijst, of een verwijzing naar een ander record.",
        "What kind of governing document this is.": "Wat voor bestuurlijk document dit is.",
        "What kind of party owns the item, in whatever terms the organisation uses.": "Wat voor partij eigenaar is van het punt, in de termen die de organisatie zelf gebruikt.",
        "What kind of step this is. `custom` is a step the organisation defined itself.": "Wat voor stap dit is. `custom` is een stap die de organisatie zelf heeft bepaald.",
        "What kind of step this is. `personal-data-note` points at the author export rather than repeating it; `custom` is a step the organisation defined itself.": "Wat voor stap dit is. `personal-data-note` verwijst naar de export van de auteur in plaats van die te herhalen; `custom` is een stap die de organisatie zelf heeft bepaald.",
        "What kind of thing is being consulted on. Free vocabulary: an organisation uses its own words.": "Waar de raadpleging over gaat. Vrije woordenschat: een organisatie gebruikt haar eigen woorden.",
        "What kind of thing the body expects, in whatever terms it uses itself.": "Wat voor stuk het orgaan verwacht, in de termen die het zelf gebruikt.",
        "What meetings of this kind are for.": "Waar vergaderingen van dit soort voor bedoeld zijn.",
        "What set this off: one person joining, a whole intake at once, or someone stepping in mid-term. Free text, because what an organisation calls its intake is its own vocabulary.": "Wat hiertoe aanleiding gaf: één persoon die toetreedt, een hele instroom tegelijk, of iemand die tussentijds instapt. Vrije tekst, want hoe een organisatie haar instroom noemt is haar eigen woordenschat.",
        "What set this off: one person leaving, or a whole turnover at once. Free text, because what an organisation calls its turnover is its own vocabulary.": "Wat hiertoe aanleiding gaf: één persoon die vertrekt, of een hele wisseling tegelijk. Vrije tekst, want hoe een organisatie haar wisseling noemt is haar eigen woordenschat.",
        "What the asking body did with the answers, and why.": "Wat het vragende orgaan met de reacties heeft gedaan, en waarom.",
        "What the asking body did with this answer.": "Wat het vragende orgaan met deze reactie heeft gedaan.",
        "What the field is called on screen: the caption above the input when someone fills in an agenda item, and the column header where the field is listed.": "Hoe het veld op het scherm heet — het opschrift boven het invoerveld bij het invullen van een agendapunt, en de kolomkop waar het veld in een lijst staat.",
        "What the position is responsible for.": "Waar de functie verantwoordelijk voor is.",
        "What this document derives its authority from. A statute for a public body, the articles of association for a company rule, the deed of division for a VvE regulation.": "Waaraan dit document zijn grondslag ontleent. Een wet bij een overheidsorgaan, de statuten bij een bedrijfsreglement, de splitsingsakte bij een VvE-reglement.",
        "What was decided, how it was framed, and what reached the public.": "Wat er besloten is, hoe het geformuleerd was, en wat het publiek bereikte.",
        "What was discussed…": "Wat is er besproken…",
        "When": "Wanneer",
        "When answering opens. Empty means it is open already.": "Wanneer reageren opengaat. Leeg betekent dat het al open is.",
        "When the answer was given.": "Wanneer de reactie is gegeven.",
        "When the asking body accounted for the answers.": "Wanneer het vragende orgaan verantwoording aflegde over de reacties.",
        "When the component joined the composite.": "Wanneer het samenstellende orgaan is toegetreden.",
        "When the component left. Empty means it is still composing.": "Wanneer het samenstellende orgaan is uitgetreden. Leeg betekent dat het nog deelneemt.",
        "When the exit was confirmed. Optional until confirmed.": "Wanneer het vertrek is bevestigd. Optioneel tot het bevestigd is.",
        "When this answer becomes public.": "Wanneer deze reactie openbaar wordt.",
        "When this answer stops being public.": "Wanneer deze reactie niet langer openbaar is.",
        "When this consultation becomes public.": "Wanneer deze raadpleging openbaar wordt.",
        "When this consultation stops being public.": "Wanneer deze raadpleging niet langer openbaar is.",
        "When this hold began.": "Wanneer deze bekleding is begonnen.",
        "When this hold ends. Empty means open-ended.": "Wanneer deze bekleding eindigt. Leeg betekent onbepaald.",
        "Where Decidiq sends governance communications such as convocations, minutes and reminders.": "Waar Decidiq bestuurscommunicatie zoals convocaties, notulen en herinneringen naartoe stuurt.",
        "Where meetings stand, how they are held, and who turned up.": "Waar vergaderingen staan, hoe ze gehouden worden, en wie er kwam.",
        "Where the automation lives": "Waar de automatisering zit",
        "Where the consultation is in its course.": "Waar de raadpleging staat in haar verloop.",
        "Where the offboarding stands. Declared in x-openregister-lifecycle (ADR-031): started, in-execution, then completed or lapsed. It cannot reach completed while the always-mandatory revoke-groups step is not completed.": "Hoe ver het vertrek is. Vastgelegd in x-openregister-lifecycle (ADR-031): started, in-execution, en daarna completed of lapsed. Het kan completed niet bereiken zolang de altijd verplichte stap revoke-groups niet completed is.",
        "Where the onboarding stands. Declared in x-openregister-lifecycle (ADR-031): started, in-execution, then completed or lapsed.": "Hoe ver het aantreden is. Vastgelegd in x-openregister-lifecycle (ADR-031): started, in-execution, en daarna completed of lapsed.",
        "Where the step stands. A step may only be `skipped` when mandatory is false, enforced at save time.": "Hoe ver de stap is. Een stap mag alleen `skipped` zijn wanneer mandatory false is, en dat wordt bij opslaan afgedwongen.",
        "Where the step stands. A step may only be `skipped` when mandatory is false, enforced at save time. The revoke-groups step is always mandatory and can never be skipped.": "Hoe ver de stap is. Een stap mag alleen `skipped` zijn wanneer mandatory false is, en dat wordt bij opslaan afgedwongen. De stap revoke-groups is altijd verplicht en kan nooit worden overgeslagen.",
        "Where this answer is in its course.": "Waar deze reactie staat in haar verloop.",
        "Where this document is in its lifecycle.": "Waar dit document staat in zijn levensloop.",
        "Whether a holder may be reappointed at the end of a term.": "Of een bekleder aan het einde van een termijn herbenoemd mag worden.",
        "Whether an item of this kind can be put to a vote.": "Of een punt van dit soort in stemming gebracht kan worden.",
        "Whether answers are recorded without naming who gave them.": "Of reacties worden vastgelegd zonder te vermelden wie ze gaf.",
        "Whether every stage of this decision's route is decided or skipped. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq and never edited by hand.": "Of elke fase van de route van dit besluit is genomen of overgeslagen. Wordt door OpenRegister bij het opslaan gematerialiseerd uit x-openregister-calculations. Wordt nooit door decidiq geschreven en nooit met de hand aangepast.",
        "Whether meetings of this kind are public by default.": "Of vergaderingen van dit soort standaard openbaar zijn.",
        "Whether the answers bind the asking body, or only inform it.": "Of de reacties het vragende orgaan binden, of het alleen informeren.",
        "Whether the component’s members join directly, or send representatives.": "Of de leden van het samenstellende orgaan rechtstreeks meedoen, of afgevaardigden sturen.",
        "Whether the meeting met its quorum, true when no quorum is required. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Of de vergadering het quorum haalde; waar zodra er geen quorum vereist is. Wordt door OpenRegister bij het opslaan gematerialiseerd uit x-openregister-calculations. Wordt nooit door decidiq geschreven.",
        "Whether the position currently exists on the body.": "Of de functie momenteel op het orgaan bestaat.",
        "Whether the step is mandatory. A mandatory step blocks the offboarding from reaching `completed` until it is `completed` itself; revoke-groups is always mandatory.": "Of de stap verplicht is. Een verplichte stap houdt het vertrek tegen op `completed` totdat de stap zelf `completed` is; revoke-groups is altijd verplicht.",
        "Whether the step is mandatory. A mandatory step blocks the onboarding from reaching `completed` until it is `completed` itself.": "Of de stap verplicht is. Een verplichte stap houdt het aantreden tegen op `completed` totdat de stap zelf `completed` is.",
        "Whether the support threshold is a fraction of the body or an absolute count.": "Of de steundrempel een fractie van het orgaan is of een absoluut aantal.",
        "Whether this body may be assigned new work. Independent of termStart/termEnd: a body can sit inside its term and be suspended, or be past its term and still finishing open cases, so availability cannot be derived from the dates. Unrelated to the schema-level x-openregister.active flag, which says nothing about an individual body.": "Of dit orgaan nieuw werk toegewezen mag krijgen. Los van termStart/termEnd: een orgaan kan binnen zijn zittingstermijn geschorst zijn, of na afloop nog lopende zaken afronden, dus beschikbaarheid volgt niet uit de data. Niet te verwarren met de schema-brede vlag x-openregister.active, die niets zegt over een individueel orgaan.",
        "Whether this holder may be reappointed.": "Of deze bekleder herbenoemd mag worden.",
        "Whether this meeting still relies on the legacy meeting type list.": "Of deze vergadering nog steunt op de verouderde lijst met vergaderingstypen.",
        "Whether this member sits from OUTSIDE the administrative organ: i.e. is not employed by it. Awb 7:13(2) requires this of the chair of an objection advisory committee. NOT the same question as `independenceStatus`, which encodes corporate-governance independence and cannot carry an employment relationship.": "Of dit lid van BUITEN het bestuursorgaan komt — dus niet bij het orgaan in dienst is. Awb 7:13 lid 2 vereist dit van de voorzitter van een bezwaaradviescommissie. NIET dezelfde vraag als `independenceStatus`, dat onafhankelijkheid in corporate-governance-zin uitdrukt en geen dienstverband kan vastleggen.",
        "Whether this position seats a representative delegated by a composing body.": "Of deze functie een afgevaardigde plaatst die door een samenstellend orgaan is aangewezen.",
        "Whether this stage must be taken. COPIED from the ApprovalRoute step at instantiation, deliberately: reading it from the route at decision time would give a different answer whenever the template changed after a subject started travelling it.": "Of deze stap gezet moet worden. Bij het instantiëren GEKOPIEERD uit de stap van de ApprovalRoute, en dat is bewust: uitlezen bij de beslissing zou een ander antwoord geven zodra het sjabloon wijzigt nadat een onderwerp de route al doorloopt.",
        "Whether this type is offered when creating a meeting.": "Of dit type wordt aangeboden bij het aanmaken van een vergadering.",
        "Whether this type is offered.": "Of dit type wordt aangeboden.",
        "Which consecutive term in this position this hold is.": "De hoeveelste aaneengesloten termijn in deze functie deze bekleding is.",
        "Which kind of organisation is this for?": "Voor wat voor organisatie is dit?",
        "Who": "Wie",
        "Who holds it": "Wie de functie vervult",
        "Who spoke, for how long, and how engaged the room was.": "Wie er sprak, hoe lang, en hoe betrokken de zaal was.",
        "Why the asking body handled the answer that way.": "Waarom het vragende orgaan de reactie zo heeft behandeld.",
        "Why the hold ended.": "Waarom de bekleding is geëindigd.",
        "Why the membership ends. Free text: end of term, resignation, relocation, death, or whatever else an organisation records.": "Waarom het lidmaatschap eindigt. Vrije tekst: einde termijn, ontslag op eigen verzoek, verhuizing, overlijden, of wat een organisatie verder vastlegt.",
        "Wijziging regeling": "Wijziging regeling",
        "Will be imported": "Wordt geïmporteerd",
        "Wire buttons here to create records, open lists, or deep links. Use the sidebar for Settings and Documentation.": "Koppel hier knoppen aan het aanmaken van records, lijsten of deep links. Gebruik de zijbalk voor Instellingen en Documentatie.",
        "With reservation": "Onder voorbehoud",
        "Withdraw": "Intrekken",
        "Withdraw Motion": "Motie intrekken",
        "Withdraw publication": "Publicatie intrekken",
        "Withdraw reason": "Reden van intrekking",
        "Withdrawing removes the published record from the public surface and retracts the OpenCatalogi publication. A reason is required and recorded in the audit trail (WOO correction duty).": "Intrekken verwijdert het gepubliceerde record van het openbare oppervlak en trekt de OpenCatalogi-publicatie in. Een reden is verplicht en wordt vastgelegd in het auditspoor (WOO-correctieplicht).",
        "Withdrawn": "Ingetrokken",
        "Withdraw…": "Intrekken…",
        "Woo / DiWoo mappings": "Woo/DiWoo-koppelingen",
        "Woo absolute": "Woo absoluut",
        "Woo category mapping": "Woo-categoriekoppeling",
        "Woo category mappings": "Woo-categoriekoppelingen",
        "Woo decision": "Woo-besluit",
        "Woo governing bodies": "Woo-bestuursorganen",
        "Woo governing body": "Woo-bestuursorgaan",
        "Woo relative": "Woo relatief",
        "Works council": "Ondernemingsraad",
        "Works council (WOR)": "Ondernemingsraad (WOR)",
        "Works-council consultation": "OR-adviesaanvraag",
        "Works-council consultations": "OR-adviesaanvragen",
        "Workspace": "Werkruimte",
        "Workspace name": "Naam werkruimte",
        "Workspace type": "Type werkruimte",
        "Workspaces": "Werkruimtes",
        "Year": "Jaar",
        "Yes": "Ja",
        "You are all caught up": "U bent helemaal bij",
        "You are voting on behalf of": "U stemt namens",
        "You are voting on behalf of: {name}": "U stemt namens: {name}",
        "You have been asked to stand in on a sign-off that is part-way through its term. Whoever acts first closes it.": "Je bent gevraagd in te vallen bij een parafering die halverwege haar termijn is. Wie het eerst tekent, sluit de stap.",
        "Your Nextcloud account email": "Het e-mailadres van je Nextcloud-account",
        "Your first meeting is scheduled": "Je eerste vergadering staat gepland",
        "Your next meeting": "Uw volgende vergadering",
        "Your reaction": "Uw reactie",
        "Your reaction was submitted for moderation": "Uw reactie is ingediend voor moderatie",
        "Your response is anonymous. It cannot be traced back to you, even by an administrator.": "Uw reactie is anoniem. Deze kan niet naar u worden herleid, zelfs niet door een beheerder.",
        "Your sign-off has moved up": "Jouw parafering is doorgeschoven",
        "Your stand-in has been asked as well": "Je vervanger is er ook bij gevraagd",
        "Your vote has been recorded": "Uw stem is geregistreerd",
        "Your vote has been recorded.": "Uw stem is geregistreerd.",
        "Your vote was recorded": "Uw stem is geregistreerd",
        "Zienswijze rounds": "Zienswijzerondes",
        "Zienswijzen": "Zienswijzen",
        "Zienswijzen & references": "Zienswijzen en verwijzingen",
        "Zienswijzeronde": "Zienswijzeronde",
        "Zienswijzerondes": "Zienswijzerondes",
        "Zoek op motietitel…": "Zoek op motietitel…",
        "actions": "acties",
        "active": "actief",
        "adopted": "aangenomen",
        "advice": "advies",
        "advised": "geadviseerd",
        "advisory": "adviserend",
        "against": "tegen",
        "another app": "een andere app",
        "avg {actual} min actual vs {estimated} min allocated": "gem. {actual} min werkelijk t.o.v. {estimated} min toegewezen",
        "chair only": "alleen voorzitter",
        "chair register": "voorzittersregistratie",
        "concept": "concept",
        "consultationType=tender: public tender reference/notice number.": "consultationType=tender: het openbare aanbestedings- of publicatienummer.",
        "consultationType=tender: the winning party recorded when the award decision is made (status=awarded). Decidiq owns publish/manage/award only; authoring is procest and bidding is pipelinq.": "consultationType=tender: de winnende partij, vastgelegd zodra het gunningsbesluit is genomen (status=awarded). Decidiq beheert alleen publiceren, beheren en gunnen; het opstellen gebeurt in procest en het inschrijven in pipelinq.",
        "decided": "besloten",
        "decisions": "besluiten",
        "decisive": "besluitvormend",
        "deferred": "uitgesteld",
        "due {date}": "uiterlijk {date}",
        "e.g. 1": "bijv. 1",
        "e.g. 10": "bijv. 10",
        "e.g. 3650": "bijv. 3650",
        "e.g. ALV Statute Amendment": "bijv. ALV statutenwijziging",
        "e.g. Attendance list incomplete": "bijv. Presentielijst onvolledig",
        "e.g. Contained an error, corrected version follows": "bijv. Bevatte een fout, gecorrigeerde versie volgt",
        "e.g. Corrected vote totals": "bijv. Gecorrigeerde stemtotalen",
        "e.g. GroenLinks": "bijv. GroenLinks",
        "e.g. Off-topic or abusive": "bijv. Off-topic of beledigend",
        "e.g. Prepare budget proposal": "bijv. Begrotingsvoorstel voorbereiden",
        "e.g. Roos de Vries": "bijv. Roos de Vries",
        "e.g. The vote count for item 5 should read 12 in favour": "bijv. De stemtelling bij punt 5 moet 12 voor zijn",
        "e.g. Vereniging De Harmonie": "bijv. Vereniging De Harmonie",
        "for": "voor",
        "https://example.org/logo.png": "https://example.org/logo.png",
        "in force": "in werking",
        "lapsed": "vervallen",
        "linked to recorded outcome": "gekoppeld aan vastgelegde uitkomst",
        "manual": "handmatig",
        "meetings": "vergaderingen",
        "min": "min",
        "name@example.org": "naam@voorbeeld.nl",
        "on {date}": "op {date}",
        "pending": "in afwachting",
        "preparatory": "voorbereidend",
        "ratifying": "bekrachtigend",
        "rejected": "afgewezen",
        "replaced": "vervangen",
        "sample": "voorbeeld",
        "scheduled": "gepland",
        "schema.org annotation matching the ORI type (ChooseAction / Event / CreativeWork).": "schema.org-annotatie die hoort bij het ORI-type (ChooseAction / Event / CreativeWork).",
        "seq {n}": "volgnr {n}",
        "signature": "ondertekening",
        "skipped": "overgeslagen",
        "today": "vandaag",
        "tomorrow": "morgen",
        "total": "totaal",
        "until {date}": "tot {date}",
        "unverified — no recorded outcome": "niet geverifieerd — geen vastgelegde uitkomst",
        "vote": "stemming",
        "votes": "stemmen",
        "v{version}": "v{version}",
        "was due {date}": "moest uiterlijk {date}",
        "{actor}, with {substitute} standing in": "{actor}, met {substitute} als vervanger",
        "{completed} of {total} agenda items completed ({percent}%)": "{completed} van {total} agendapunten afgerond ({percent}%)",
        "{count} attendees × {rate}/h": "{count} aanwezigen × {rate}/u",
        "{count} decision": "{count} besluit",
        "{count} decisions": "{count} besluiten",
        "{count} members imported.": "{count} leden geïmporteerd.",
        "{decided} of {total} stages decided": "{decided} van {total} fasen besloten",
        "{level} signed at {when}": "{level} ondertekend op {when}",
        "{m} min": "{m} min",
        "{n} agenda items": "{n} agendapunten",
        "{n} already on Deck": "{n} al op Deck",
        "{n} attachment(s)": "{n} bijlage(n)",
        "{n} card(s) created": "{n} kaart(en) aangemaakt",
        "{n} conflict of interest declaration(s)": "{n} verklaring(en) van belangenverstrengeling",
        "{n} item(s) could not be projected.": "{n} item(s) konden niet worden geprojecteerd.",
        "{n} meeting(s) exceeded the scheduled time": "{n} vergadering(en) overschreden de geplande tijd",
        "{n} open action items": "{n} openstaande actiepunten",
        "{responded} of {invited} responded": "{responded} van {invited} gereageerd",
        "{states} states, {transitions} transitions": "{states} statussen, {transitions} overgangen",
        "{verb} by {actor}": "{verb} door {actor}",
        "{verb} by {actor}, on behalf of {onBehalfOf}": "{verb} door {actor}, namens {onBehalfOf}"
    },
    "nplurals=2; plural=(n != 1);"
)
