OC.L10N.register(
    "decidiq",
    {
        "#": "#",
        "+ Action item": "+ Action item",
        "+ Action item for {title}": "+ Action item for {title}",
        "1 hour before": "1 hour before",
        "1 week before": "1 week before",
        "24 hours before": "24 hours before",
        "4 hours before": "4 hours before",
        "48 hours before": "48 hours before",
        "A council with committees and factions. Motions, commitments, information letters and a regulations register.": "A council with committees and factions. Motions, commitments, information letters and a regulations register.",
        "A delegation needs an end date — it expires automatically.": "A delegation needs an end date — it expires automatically.",
        "A draft type cannot back new agenda items.": "A draft type cannot back new agenda items.",
        "A draft type cannot back new meetings.": "A draft type cannot back new meetings.",
        "A governance event (decision, meeting, vote or resolution) happened in Decidiq": "A governance event (decision, meeting, vote or resolution) happened in Decidiq",
        "A members' meeting with a board. House rules, weighted votes, audit statements and annual accounts.": "A members' meeting with a board. House rules, weighted votes, audit statements and annual accounts.",
        "A rule that resolves to a person when the step becomes live, instead of a person named now. A step carries `actor` or `actorRule` and never both: a step carrying both has two answers to the same question and whichever the engine happens to read first wins, which is a bug nobody can reproduce. Resolved at ACTIVATION rather than at instantiation, so a manager who changes next week reaches the steps that have not been asked yet and leaves the ones already signed alone (REQ-AR-012).": "A rule that resolves to a person when the step becomes live, instead of a person named now. A step carries `actor` or `actorRule` and never both: a step carrying both has two answers to the same question and whichever the engine happens to read first wins, which is a bug nobody can reproduce. Resolved at ACTIVATION rather than at instantiation, so a manager who changes next week reaches the steps that have not been asked yet and leaves the ones already signed alone (REQ-AR-012).",
        "A sign-off has escalated to you": "A sign-off has escalated to you",
        "A sign-off is waiting, on behalf of a colleague": "A sign-off is waiting, on behalf of a colleague",
        "A sign-off passed its term and was taken as approved": "A sign-off passed its term and was taken as approved",
        "A sign-off passed its term and was taken as refused": "A sign-off passed its term and was taken as refused",
        "A step your colleague was asked to sign passed its term without an answer. Silence escalates there, so it is now yours, with a new term.": "A step your colleague was asked to sign passed its term without an answer. Silence escalates there, so it is now yours, with a new term.",
        "A supervisory board, an executive board and a management team. Terms, conflicts of interest and evaluations.": "A supervisory board, an executive board and a management team. Terms, conflicts of interest and evaluations.",
        "A works council inside a company. Advice requests, consent requests and staff consultation.": "A works council inside a company. Advice requests, consent requests and staff consultation.",
        "AI": "AI",
        "AI generated": "AI generated",
        "AI-generated draft": "AI-generated draft",
        "Aangenomen": "Adopted",
        "Aanlever-deadline": "Aanlever-deadline",
        "Aanlever-deadline default": "Aanlever-deadline default",
        "Absent from": "Absent from",
        "Absent until (delegation expires automatically)": "Absent until (delegation expires automatically)",
        "Abstain": "Abstain",
        "Abstention handling": "Abstention handling",
        "Abstentions": "Abstentions",
        "Abstentions count toward base": "Abstentions count toward base",
        "Abstentions excluded from base": "Abstentions excluded from base",
        "Accept": "Accept",
        "Accept correction": "Accept correction",
        "Accepted": "Accepted",
        "Access level": "Access level",
        "Accession date": "Accession date",
        "Accession withdrawal": "Accession withdrawal",
        "Account": "Account",
        "Account id of the person who confirmed the exit. Optional until confirmed.": "Account id of the person who confirmed the exit. Optional until confirmed.",
        "Account linking": "Account linking",
        "Account matching failed (admin access required).": "Account matching failed (admin access required).",
        "Account matching failed.": "Account matching failed.",
        "Accountability": "Accountability",
        "Accountability date": "Accountability date",
        "Accounted for": "Accounted for",
        "Achterbanraadpleging": "Achterbanraadpleging",
        "Action": "Action",
        "Action Item Completion Rate": "Action Item Completion Rate",
        "Action Items": "Action Items",
        "Action item assigned": "Action item assigned",
        "Action item completion %": "Action item completion %",
        "Action item completion rate": "Action item completion rate",
        "Action item title": "Action item title",
        "Action items": "Action items",
        "Action taken": "Action taken",
        "Actions": "Actions",
        "Activate agenda item": "Activate agenda item",
        "Activate item": "Activate item",
        "Activate {title}": "Activate {title}",
        "Activated at": "Activated at",
        "Active": "Active",
        "Active agenda item": "Active agenda item",
        "Active decisions": "Active decisions",
        "Active {title}": "Active {title}",
        "Active: {title}": "Active: {title}",
        "Actor": "Actor",
        "Actor resolved at": "Actor resolved at",
        "Actor resolved by": "Actor resolved by",
        "Actor rule": "Actor rule",
        "Actor rule subject": "Actor rule subject",
        "Actor type": "Actor type",
        "Actual Duration": "Actual Duration",
        "Actual duration": "Actual duration",
        "Actual notice (hours)": "Actual notice (hours)",
        "Add a sub-item under \"{title}\".": "Add a sub-item under \"{title}\".",
        "Add action item": "Add action item",
        "Add action item for {title}": "Add action item for {title}",
        "Add agenda item": "Add agenda item",
        "Add co-author": "Add co-author",
        "Add comment": "Add comment",
        "Add integration": "Add integration",
        "Add member": "Add member",
        "Add motion": "Add motion",
        "Add participant": "Add participant",
        "Add recurring items": "Add recurring items",
        "Add related decision": "Add related decision",
        "Add relation": "Add relation",
        "Add selected": "Add selected",
        "Add signer": "Add signer",
        "Add speaker to queue": "Add speaker to queue",
        "Add state": "Add state",
        "Add sub-item": "Add sub-item",
        "Add sub-item under {title}": "Add sub-item under {title}",
        "Add to queue": "Add to queue",
        "Add transition": "Add transition",
        "Added text": "Added text",
        "Adding…": "Adding…",
        "Additional fields items of this kind carry.": "Additional fields items of this kind carry.",
        "Additional templates": "Additional templates",
        "Adjourned": "Adjourned",
        "Admitted": "Admitted",
        "Adopt all consent agenda items": "Adopt all consent agenda items",
        "Adopt consent agenda": "Adopt consent agenda",
        "Adopted": "Adopted",
        "Adopting body": "Adopting body",
        "Adoption date": "Adoption date",
        "Advance phase": "Advance phase",
        "Advance to next BOB phase for {title}": "Advance to next BOB phase for {title}",
        "Adverse": "Adverse",
        "Advice": "Advice",
        "Advice date": "Advice date",
        "Advice document": "Advice document",
        "Advice issued": "Advice issued",
        "Advice with conditions": "Advice with conditions",
        "Advisory": "Advisory",
        "Advisory body": "Advisory body",
        "Advisory request": "Advisory request",
        "Advisory stage": "Advisory stage",
        "Afdoening evidence": "Afdoening evidence",
        "Afdoening note": "Afdoening note",
        "Affirmation": "Affirmation",
        "Against": "Against",
        "Agenda": "Agenda",
        "Agenda Item": "Agenda Item",
        "Agenda Items": "Agenda Items",
        "Agenda builder": "Agenda builder",
        "Agenda completion": "Agenda completion",
        "Agenda item": "Agenda item",
        "Agenda item integrations": "Agenda item integrations",
        "Agenda item title": "Agenda item title",
        "Agenda item type": "Agenda item type",
        "Agenda item types": "Agenda item types",
        "Agenda item types admitted on this kind of meeting.": "Agenda item types admitted on this kind of meeting.",
        "Agenda item {n}: {title}": "Agenda item {n}: {title}",
        "Agenda items": "Agenda items",
        "Agenda items ({n})": "Agenda items ({n})",
        "Agenda items, drag to reorder": "Agenda items, drag to reorder",
        "Agenda policy": "Agenda policy",
        "Agenda-item notes": "Agenda-item notes",
        "Agendised": "Agendised",
        "Akte article": "Akte article",
        "Aligned at": "Aligned at",
        "All changes saved": "All changes saved",
        "All connections": "All connections",
        "All participants already added as signers.": "All participants already added as signers.",
        "All statuses": "All statuses",
        "Allocated time (minutes)": "Allocated time (minutes)",
        "Allow decide without vote": "Allow decide without vote",
        "Allowed agenda item types": "Allowed agenda item types",
        "Allowed hold types": "Allowed hold types",
        "Allowed trigger roles": "Allowed trigger roles",
        "Already a member — skipped": "Already a member — skipped",
        "Amended by": "Amended by",
        "Amendement indienen": "Submit amendment",
        "Amendementen": "Amendments",
        "Amendment": "Amendment",
        "Amendment internal regulations": "Amendment internal regulations",
        "Amendment of arrangement": "Amendment of arrangement",
        "Amendment text": "Amendment text",
        "Amendments": "Amendments",
        "Amendments are voted before the main motion, most far-reaching first. Only the chair can save the order.": "Amendments are voted before the main motion, most far-reaching first. Only the chair can save the order.",
        "Amends": "Amends",
        "Amount Delta (€)": "Amount Delta (€)",
        "An existing person with this email will be reused; otherwise a new one is created.": "An existing person with this email will be reused; otherwise a new one is created.",
        "An opaque identifier for the respondent, used when answers are anonymous or the respondent is not a recorded person.": "An opaque identifier for the respondent, used when answers are anonymous or the respondent is not a recorded person.",
        "Ancillary positions intake": "Ancillary positions intake",
        "And after. The pair is the record REQ-DWP-002 asks for: a step whose outcome changed without an action to point at is a step nobody can explain.": "And after. The pair is the record REQ-DWP-002 asks for: a step whose outcome changed without an action to point at is a step nobody can explain.",
        "Annual accounts": "Annual accounts",
        "Annual report": "Annual report",
        "Annuleren": "Cancel",
        "Anonymous intake rate limit (per hour)": "Anonymous intake rate limit (per hour)",
        "Anonymous reactions allowed": "Anonymous reactions allowed",
        "Anonymous responses": "Anonymous responses",
        "Answer": "Answer",
        "Answer summary": "Answer summary",
        "Answered": "Answered",
        "Answered by": "Answered by",
        "Answered on": "Answered on",
        "Answers": "Answers",
        "Any background the parties need to answer.": "Any background the parties need to answer.",
        "Any other business": "Any other business",
        "Any qualification or remark the committee recorded.": "Any qualification or remark the committee recorded.",
        "App id of the fleet app this route was held on behalf of (e.g. dossiq). Empty for routes authored in decidiq itself. Half of the idempotency key; never an authorization input.": "App id of the fleet app this route was held on behalf of (e.g. dossiq). Empty for routes authored in decidiq itself. Half of the idempotency key; never an authorization input.",
        "Appointed": "Appointed",
        "Appointed by": "Appointed by",
        "Appointed memberships": "Appointed memberships",
        "Appointment": "Appointment",
        "Approval basis": "Approval basis",
        "Approval of previous minutes": "Approval of previous minutes",
        "Approval workflow": "Approval workflow",
        "Approval workflow error": "Approval workflow error",
        "Approve": "Approve",
        "Approve or reject citizen reactions before they count toward a consultation.": "Approve or reject citizen reactions before they count toward a consultation.",
        "Approve proposal {title}": "Approve proposal {title}",
        "Approve reaction": "Approve reaction",
        "Approved": "Approved",
        "Approved at": "Approved at",
        "Approved at {date} by {names}": "Approved at {date} by {names}",
        "Approving": "Approving",
        "Approving counts this reaction toward the consultation and allows it to be published.": "Approving counts this reaction toward the consultation and allows it to be published.",
        "Archival retention period (days)": "Archival retention period (days)",
        "Archive": "Archive",
        "Archived": "Archived",
        "Arrangement": "Arrangement",
        "Article": "Article",
        "Articles of association": "Articles of association",
        "Ask substitute after": "Ask substitute after",
        "Asked by": "Asked by",
        "Asked on": "Asked on",
        "Asking body": "Asking body",
        "Assemble meeting package": "Assemble meeting package",
        "Assembles convocation, quorum, voting results, and the adopted decision texts into a tamper-evident package in the meeting folder.": "Assembles convocation, quorum, voting results, and the adopted decision texts into a tamper-evident package in the meeting folder.",
        "Assembling…": "Assembling…",
        "Assign a role to {name}.": "Assign a role to {name}.",
        "Assign groups": "Assign groups",
        "Assign spokesperson": "Assign spokesperson",
        "Assign spokesperson for {title}": "Assign spokesperson for {title}",
        "Assigned body": "Assigned body",
        "Assigned person": "Assigned person",
        "Assignee": "Assignee",
        "Association (assoc)": "Association (assoc)",
        "Association or VvE": "Association or VvE",
        "At most {max} rows can be imported at once.": "At most {max} rows can be imported at once.",
        "Attach recording": "Attach recording",
        "Attach with consent": "Attach with consent",
        "Attaching a recording for transcription requires confirming that all participants were informed that the meeting was recorded (AVG/GDPR). The recording and raw transcript stay restricted to this governance body and are never published.": "Attaching a recording for transcription requires confirming that all participants were informed that the meeting was recorded (AVG/GDPR). The recording and raw transcript stay restricted to this governance body and are never published.",
        "Attachments": "Attachments",
        "Attendance": "Attendance",
        "Attendance mode": "Attendance mode",
        "Attendance rate": "Attendance rate",
        "Attendance status": "Attendance status",
        "Audience": "Audience",
        "Audience body": "Audience body",
        "Audience group": "Audience group",
        "Audience size": "Audience size",
        "Audience type": "Audience type",
        "Audience user group": "Audience user group",
        "Audit statement": "Audit statement",
        "Audit statements": "Audit statements",
        "AuditStatement": "AuditStatement",
        "Author": "Author",
        "Author name": "Author name",
        "Authorisation": "Authorisation",
        "Authorisation above threshold": "Authorisation above threshold",
        "Authority delegation": "Authority delegation",
        "Authority delegations": "Authority delegations",
        "Authorization instrument": "Authorization instrument",
        "Authorizing decision": "Authorizing decision",
        "Autosave failed — retrying on next edit": "Autosave failed — retrying on next edit",
        "Availability": "Availability",
        "Available transitions": "Available transitions",
        "Average actual duration: {minutes} min": "Average actual duration: {minutes} min",
        "Award criteria": "Award criteria",
        "Awarded to": "Awarded to",
        "BOB phase": "BOB phase",
        "BOB phase for {title}": "BOB phase for {title}",
        "BOB phase progression": "BOB phase progression",
        "Back": "Back",
        "Back to agenda item": "Back to agenda item",
        "Back to boards": "Back to boards",
        "Back to decision": "Back to decision",
        "Back to meeting": "Back to meeting",
        "Back to meeting detail": "Back to meeting detail",
        "Back to meetings": "Back to meetings",
        "Back to motion": "Back to motion",
        "Back to resolutions": "Back to resolutions",
        "Background": "Background",
        "Bedrag delta": "Amount delta",
        "Beeldvorming": "Image formation",
        "Begrotingspost": "Budget line",
        "Besluitvorming": "Decision making",
        "Binding": "Binding",
        "Board election": "Board election",
        "Board elections": "Board elections",
        "Board meetings": "Board meetings",
        "Board name": "Board name",
        "Board not found": "Board not found",
        "Board self-evaluation": "Board self-evaluation",
        "Boards": "Boards",
        "Body": "Body",
        "Body composition": "Body composition",
        "Body compositions": "Body compositions",
        "Body configuration": "Body configuration",
        "Both": "Both",
        "Budget Impact": "Budget Impact",
        "Budget Line": "Budget Line",
        "Budget amendment": "Budget amendment",
        "Budget document": "Budget document",
        "Budget round updated": "Budget round updated",
        "Built-in": "Built-in",
        "Bulk turnover": "Bulk turnover",
        "By law": "By law",
        "By when an answer is expected.": "By when an answer is expected.",
        "COI ({n})": "COI ({n})",
        "CSV file": "CSV file",
        "Calendar": "Calendar",
        "Cancel": "Cancel",
        "Cannot publish: no agenda items.": "Cannot publish: no agenda items.",
        "Cast": "Cast",
        "Cast at": "Cast at",
        "Cast your vote": "Cast your vote",
        "Cast: {cast} / {total}": "Cast: {cast} / {total}",
        "Casting vote failed": "Casting vote failed",
        "Casting vote: against": "Casting vote: against",
        "Casting vote: for": "Casting vote: for",
        "Catalog id": "Catalog id",
        "Categories where this body needs a different majority than its templates use by default.": "Categories where this body needs a different majority than its templates use by default.",
        "Category": "Category",
        "Chair": "Chair",
        "Chair only": "Chair only",
        "Change it in your personal settings.": "Change it in your personal settings.",
        "Change role": "Change role",
        "Change spokesperson": "Change spokesperson",
        "Channel": "Channel",
        "Child goal achieved rate": "Child goal achieved rate",
        "Choose what happens to this body's meeting recordings and raw transcripts after the minutes are approved. The approved minutes always remain the official record.": "Choose what happens to this body's meeting recordings and raw transcripts after the minutes are approved. The approved minutes always remain the official record.",
        "Choose which Decidiq events notify you and how they are delivered.": "Choose which Decidiq events notify you and how they are delivered.",
        "Citizen participation": "Citizen participation",
        "Citizen participation defaults": "Citizen participation defaults",
        "Citizen portal (citizen)": "Citizen portal (citizen)",
        "Clear delegation": "Clear delegation",
        "Close": "Close",
        "Close At (optional)": "Close At (optional)",
        "Close Voting Round": "Close Voting Round",
        "Close agenda item": "Close agenda item",
        "Close cycle": "Close cycle",
        "Close item": "Close item",
        "Close the voting round now? Members who have not voted yet will not be counted.": "Close the voting round now? Members who have not voted yet will not be counted.",
        "Close voting round": "Close voting round",
        "Close voting round? {notVoted} of {total} members have not voted yet.": "Close voting round? {notVoted} of {total} members have not voted yet.",
        "Closed": "Closed",
        "Closing": "Closing",
        "Closing time (optional)": "Closing time (optional)",
        "Co-Signatories": "Co-Signatories",
        "Co-authors": "Co-authors",
        "Comma-separated dates, e.g. 2026-07-14, 2026-08-11": "Comma-separated dates, e.g. 2026-07-14, 2026-08-11",
        "Comment": "Comment",
        "Comments": "Comments",
        "Commitment": "Commitment",
        "Commitment settlement rate": "Commitment settlement rate",
        "Commitments": "Commitments",
        "Committee": "Committee",
        "Communication": "Communication",
        "Communication preferences": "Communication preferences",
        "Communication preferences saved.": "Communication preferences saved.",
        "Company board": "Company board",
        "Competent body": "Competent body",
        "Competent positions": "Competent positions",
        "Completed": "Completed",
        "Component body": "Component body",
        "Composite body": "Composite body",
        "Composition type": "Composition type",
        "Conclude vote": "Conclude vote",
        "Confidential": "Confidential",
        "Confidentiality ground": "Confidentiality ground",
        "Confidentiality grounds": "Confidentiality grounds",
        "Confidentiality restriction": "Confidentiality restriction",
        "Confidentiality restrictions": "Confidentiality restrictions",
        "Confidentiality status timeline": "Confidentiality status timeline",
        "Configuration": "Configuration",
        "Configure the OpenRegister schema mappings for all Decidiq object types.": "Configure the OpenRegister schema mappings for all Decidiq object types.",
        "Configure the app settings": "Configure the app settings",
        "Configure, per governance body, where adopted decisions, public agendas, and approved minutes are published. Anonymous read access is served exclusively through OpenCatalogi / OpenRegister — never an app-local public page.": "Configure, per governance body, where adopted decisions, public agendas, and approved minutes are published. Anonymous read access is served exclusively through OpenCatalogi / OpenRegister — never an app-local public page.",
        "Configured": "Configured",
        "Confirm": "Confirm",
        "Confirm adoption": "Confirm adoption",
        "Confirm recording consent": "Confirm recording consent",
        "Conflict of interest": "Conflict of interest",
        "Conflict of interest declarations": "Conflict of interest declarations",
        "Connection": "Connection",
        "Consent agenda items": "Consent agenda items",
        "Consent agenda items (hamerstukken)": "Consent agenda items (hamerstukken)",
        "Consent granted": "Consent granted",
        "Consent refused": "Consent refused",
        "Consent request": "Consent request",
        "Constituency consultation": "Constituency consultation",
        "Constitutive document": "Constitutive document",
        "Consultation response": "Consultation response",
        "Consultation responses": "Consultation responses",
        "Consultation updated": "Consultation updated",
        "Consultations": "Consultations",
        "Contact methods": "Contact methods",
        "Context": "Context",
        "Contract": "Contract",
        "Contract renewal": "Contract renewal",
        "Contrary to advice": "Contrary to advice",
        "Control how Decidiq presents itself for your account.": "Control how Decidiq presents itself for your account.",
        "Controls mode-specific labels across the app — for example what a governance body is called. Default: government.": "Controls mode-specific labels across the app — for example what a governance body is called. Default: government.",
        "Copied from the step at instantiation so the stage can resolve its own actor when it becomes live. The rule lives on the stage and not only on the route because editing a route must never change a stage already in flight (REQ-AR-012).": "Copied from the step at instantiation so the stage can resolve its own actor when it becomes live. The rule lives on the stage and not only on the route because editing a route must never change a stage already in flight (REQ-AR-012).",
        "Copied from the step at instantiation, so editing a route never changes what the silence of a stage already in flight means.": "Copied from the step at instantiation, so editing a route never changes what the silence of a stage already in flight means.",
        "Corporate (corp)": "Corporate (corp)",
        "Corporate-governance extension (beyond Popolo, MCCG): board-member independence, mode=corp. NOT the same question as `external`, which records whether the member sits from outside the administrative organ. A member can be external and non-independent, or internal and independent; neither value is derivable from the other.": "Corporate-governance extension (beyond Popolo, MCCG): board-member independence, mode=corp. NOT the same question as `external`, which records whether the member sits from outside the administrative organ. A member can be external and non-independent, or internal and independent; neither value is derivable from the other.",
        "Correction": "Correction",
        "Correction suggestions": "Correction suggestions",
        "Cost per agenda item": "Cost per agenda item",
        "Cost trend": "Cost trend",
        "Could not approve the reaction": "Could not approve the reaction",
        "Could not attach the recording.": "Could not attach the recording.",
        "Could not create decision.": "Could not create decision.",
        "Could not create minutes.": "Could not create minutes.",
        "Could not create or match a person.": "Could not create or match a person.",
        "Could not create proposal.": "Could not create proposal.",
        "Could not create the action item.": "Could not create the action item.",
        "Could not generate the draft.": "Could not generate the draft.",
        "Could not load action items": "Could not load action items",
        "Could not load agenda items": "Could not load agenda items",
        "Could not load amendments": "Could not load amendments",
        "Could not load analytics": "Could not load analytics",
        "Could not load decision-making.": "Could not load decision-making.",
        "Could not load decisions": "Could not load decisions",
        "Could not load evaluations": "Could not load evaluations",
        "Could not load group members.": "Could not load group members.",
        "Could not load groups (admin access required).": "Could not load groups (admin access required).",
        "Could not load groups.": "Could not load groups.",
        "Could not load members": "Could not load members",
        "Could not load minutes": "Could not load minutes",
        "Could not load motions": "Could not load motions",
        "Could not load parent motion": "Could not load parent motion",
        "Could not load participants": "Could not load participants",
        "Could not load participation rounds": "Could not load participation rounds",
        "Could not load related decisions": "Could not load related decisions",
        "Could not load route": "Could not load route",
        "Could not load routed documents": "Could not load routed documents",
        "Could not load signers": "Could not load signers",
        "Could not load template assignment": "Could not load template assignment",
        "Could not load the diff": "Could not load the diff",
        "Could not load the moderation queue": "Could not load the moderation queue",
        "Could not load the ondermandaat chain": "Could not load the ondermandaat chain",
        "Could not load transcription state.": "Could not load transcription state.",
        "Could not load versions": "Could not load versions",
        "Could not load votes": "Could not load votes",
        "Could not load voting overview": "Could not load voting overview",
        "Could not load voting results": "Could not load voting results",
        "Could not re-align the transcript.": "Could not re-align the transcript.",
        "Could not record your vote": "Could not record your vote",
        "Could not reject the reaction": "Could not reject the reaction",
        "Could not save retention policy": "Could not save retention policy",
        "Could not save the retention policy.": "Could not save the retention policy.",
        "Could not start transcription.": "Could not start transcription.",
        "Could not submit your proposal": "Could not submit your proposal",
        "Could not submit your reaction": "Could not submit your reaction",
        "Could not update status.": "Could not update status.",
        "Council information letter": "Council information letter",
        "Council proposal": "Council proposal",
        "Council registry": "Council registry",
        "Council turnover batch": "Council turnover batch",
        "Count": "Count",
        "Counts only": "Counts only",
        "Create": "Create",
        "Create Agenda Item": "Create Agenda Item",
        "Create Decision": "Create Decision",
        "Create Governance Body": "Create Governance Body",
        "Create Meeting": "Create Meeting",
        "Create Participant": "Create Participant",
        "Create board": "Create board",
        "Create decision": "Create decision",
        "Create meeting": "Create meeting",
        "Create minutes": "Create minutes",
        "Create process template": "Create process template",
        "Create proposal for this object": "Create proposal for this object",
        "Create template": "Create template",
        "Create your first board to get started.": "Create your first board to get started.",
        "Currency": "Currency",
        "Current": "Current",
        "Cycle step": "Cycle step",
        "Dashboard": "Dashboard",
        "Date": "Date",
        "Date after which this type may no longer be used. Empty means indefinitely.": "Date after which this type may no longer be used. Empty means indefinitely.",
        "Date format": "Date format",
        "Date from which this type may be used.": "Date from which this type may be used.",
        "Date the membership ends. The end-membership step sets Membership.endDate to this value.": "Date the membership ends. The end-membership step sets Membership.endDate to this value.",
        "Date the person was formally installed. Completing the installation step requires this, installationType and installationMeeting.": "Date the person was formally installed. Completing the installation step requires this, installationType and installationMeeting.",
        "Days after approval": "Days after approval",
        "De datum waarop het besluit is gepubliceerd. Onderscheiden van `publishedAt`, dat het moment van publiceren in dit systeem vastlegt; dit is de datum die het besluit zelf draagt. BRC `publicatiedatum`.": "De datum waarop het besluit is gepubliceerd. Onderscheiden van `publishedAt`, dat het moment van publiceren in dit systeem vastlegt; dit is de datum die het besluit zelf draagt. BRC `publicatiedatum`.",
        "De datum waarop het besluit is verzonden aan de belanghebbende. BRC `verzenddatum`.": "De datum waarop het besluit is verzonden aan de belanghebbende. BRC `verzenddatum`.",
        "De datum waarop het besluit vervalt. Onderscheiden van `effectiveDate`, die zegt wanneer het ingaat. BRC `vervaldatum`.": "De datum waarop het besluit vervalt. Onderscheiden van `effectiveDate`, die zegt wanneer het ingaat. BRC `vervaldatum`.",
        "Deadline": "Deadline",
        "Deadline {date}": "Deadline {date}",
        "Death": "Death",
        "Debat": "Debate",
        "Debat openen": "Open debate",
        "Debate": "Debate",
        "Decided": "Decided",
        "Decided by": "Decided by",
        "Decidiq": "Decidiq",
        "Decidiq governance": "Decidiq governance",
        "Decidiq personal settings": "Decidiq personal settings",
        "Decidiq settings": "Decidiq settings",
        "Decision": "Decision",
        "Decision \"%1$s\" was published": "Decision \"%1$s\" was published",
        "Decision \"%1$s\" was recorded": "Decision \"%1$s\" was recorded",
        "Decision integrations": "Decision integrations",
        "Decision list": "Decision list",
        "Decision maker": "Decision maker",
        "Decision outcome recorded when status=decided. Null when status is pending/active/skipped. `returned` is the sign-off case and behaves unlike the rest: it re-opens an EARLIER stage rather than ending this one, which is why `rejected` and `deferred` could not express it, both end a stage and neither reopens one.": "Decision outcome recorded when status=decided. Null when status is pending/active/skipped. `returned` is the sign-off case and behaves unlike the rest: it re-opens an EARLIER stage rather than ending this one, which is why `rejected` and `deferred` could not express it, both end a stage and neither reopens one.",
        "Decision policy": "Decision policy",
        "Decision published": "Decision published",
        "Decision received": "Decision received",
        "Decision requested by %1$s": "Decision requested by %1$s",
        "Decision requested by %1$s for %2$s.": "Decision requested by %1$s for %2$s.",
        "Decision requested by %1$s.": "Decision requested by %1$s.",
        "Decision templates": "Decision templates",
        "Decision type": "Decision type",
        "Decision types this position is competent to decide.": "Decision types this position is competent to decide.",
        "Decision {object} was published": "Decision {object} was published",
        "Decision {object} was recorded": "Decision {object} was recorded",
        "Decisions": "Decisions",
        "Decisions awaiting your vote": "Decisions awaiting your vote",
        "Decisions taken on this item…": "Decisions taken on this item…",
        "Deck board error": "Deck board error",
        "Declare conflict of interest": "Declare conflict of interest",
        "Declare conflict of interest for this agenda item": "Declare conflict of interest for this agenda item",
        "Deed of division": "Deed of division",
        "Deelnemer UUID": "Participant UUID",
        "Default OpenCatalogi catalog (UUID)": "Default OpenCatalogi catalog (UUID)",
        "Default duration (minutes)": "Default duration (minutes)",
        "Default language": "Default language",
        "Default moderation policy": "Default moderation policy",
        "Default process template": "Default process template",
        "Default quorum": "Default quorum",
        "Default quorum rule": "Default quorum rule",
        "Default view": "Default view",
        "Default vote threshold": "Default vote threshold",
        "Default voting rule": "Default voting rule",
        "Default: 24 hours and 1 hour before the meeting.": "Default: 24 hours and 1 hour before the meeting.",
        "Defaults applied to new consultations and budget rounds. Staff can override per round.": "Defaults applied to new consultations and budget rounds. Staff can override per round.",
        "Deferred": "Deferred",
        "Define the state machine, voting rule and quorum policy a governance body follows. Built-in templates are read-only but can be duplicated.": "Define the state machine, voting rule and quorum policy a governance body follows. Built-in templates are read-only but can be duplicated.",
        "Delegans: {who}": "Delegans: {who}",
        "Delegataris: {who}": "Delegataris: {who}",
        "Delegate": "Delegate",
        "Delegate body": "Delegate body",
        "Delegate task": "Delegate task",
        "Delegating body": "Delegating body",
        "Delegating party description": "Delegating party description",
        "Delegation": "Delegation",
        "Delegation and absence": "Delegation and absence",
        "Delegation does not include voting rights. A formal proxy (volmacht) is required for voting.": "Delegation does not include voting rights. A formal proxy (volmacht) is required for voting.",
        "Delegation saved.": "Delegation saved.",
        "Delegations": "Delegations",
        "Delegator": "Delegator",
        "Delete": "Delete",
        "Delete action item": "Delete action item",
        "Delete agenda item": "Delete agenda item",
        "Delete amendment": "Delete amendment",
        "Delete failed.": "Delete failed.",
        "Delete motion": "Delete motion",
        "Delete recording and transcript": "Delete recording and transcript",
        "Delete recording only": "Delete recording only",
        "Deliberating": "Deliberating",
        "Delivery channels": "Delivery channels",
        "Delivery method": "Delivery method",
        "Describe the agenda item": "Describe the agenda item",
        "Describe the correction you propose. The chair or secretary reviews every suggestion before approving the minutes.": "Describe the correction you propose. The chair or secretary reviews every suggestion before approving the minutes.",
        "Describe your reason for declaring a conflict of interest": "Describe your reason for declaring a conflict of interest",
        "Description": "Description",
        "Detailed rule": "Detailed rule",
        "Digital Documents": "Digital Documents",
        "Discard section": "Discard section",
        "Discharge": "Discharge",
        "Discussion": "Discussion",
        "Discussion notes": "Discussion notes",
        "Dismiss": "Dismiss",
        "Display": "Display",
        "Display position in the agenda item type picker.": "Display position in the agenda item type picker.",
        "Display position in the body’s roster.": "Display position in the body’s roster.",
        "Display preferences": "Display preferences",
        "Display preferences saved.": "Display preferences saved.",
        "Disposed": "Disposed",
        "Dissolution": "Dissolution",
        "Dissolved": "Dissolved",
        "Document format": "Document format",
        "Document generation error": "Document generation error",
        "Document stored at {path}": "Document stored at {path}",
        "Document version": "Document version",
        "Documentation": "Documentation",
        "Documents received": "Documents received",
        "Domain": "Domain",
        "Done": "Done",
        "Draft": "Draft",
        "Draft budget": "Draft budget",
        "Due": "Due",
        "Due at": "Due at",
        "Due date": "Due date",
        "Due this week": "Due this week",
        "Due {date}": "Due {date}",
        "Duplicate": "Duplicate",
        "Duplicate row — skipped": "Duplicate row — skipped",
        "Duration (min)": "Duration (min)",
        "During the configured period your delegate receives your Decidiq notifications and can follow your pending votes and action items.": "During the configured period your delegate receives your Decidiq notifications and can follow your pending votes and action items.",
        "Dutch": "Dutch",
        "E-mail stemmen": "Email voting",
        "Edit": "Edit",
        "Edit Agenda Item": "Edit Agenda Item",
        "Edit Amendment": "Edit Amendment",
        "Edit Governance Body": "Edit Governance Body",
        "Edit Meeting": "Edit Meeting",
        "Edit Motion": "Edit Motion",
        "Edit Participant": "Edit Participant",
        "Edit action item": "Edit action item",
        "Edit agenda item": "Edit agenda item",
        "Edit amendment": "Edit amendment",
        "Edit motion": "Edit motion",
        "Edit process template": "Edit process template",
        "Effective": "Effective",
        "Efficiency": "Efficiency",
        "Email": "Email",
        "Email Voting": "Email Voting",
        "Email links": "Email links",
        "Email voting": "Email voting",
        "Enable email vote reply parsing": "Enable email vote reply parsing",
        "Enable voting by email reply": "Enable voting by email reply",
        "Enact": "Enact",
        "Enacted": "Enacted",
        "End Date": "End Date",
        "End date": "End date",
        "End membership": "End membership",
        "End of council membership": "End of council membership",
        "End of term": "End of term",
        "End reason": "End reason",
        "Ended": "Ended",
        "Engagement": "Engagement",
        "Engagement records": "Engagement records",
        "Engagement score": "Engagement score",
        "English": "English",
        "Enter a valid email address.": "Enter a valid email address.",
        "Er is al een volmacht geregistreerd voor deze deelnemer in deze stemronde": "A proxy has already been registered for this participant in this voting round",
        "Error": "Error",
        "Estimated Duration": "Estimated Duration",
        "Every schema, generated values": "Every schema, generated values",
        "Every step of this route has been taken.": "Every step of this route has been taken.",
        "Example: {example}": "Example: {example}",
        "Exception dates": "Exception dates",
        "Executive board": "Executive board",
        "Executive board paper": "Executive board paper",
        "Exit confirmation": "Exit confirmation",
        "Exit date": "Exit date",
        "Expired": "Expired",
        "Export": "Export",
        "Export agenda": "Export agenda",
        "Extend 10 min": "Extend 10 min",
        "Extend 10 minutes": "Extend 10 minutes",
        "Extend 5 min": "Extend 5 min",
        "Extend 5 minutes": "Extend 5 minutes",
        "External charter": "External charter",
        "External integrations linked to this agenda item — open the sidebar to link emails, browse files, notes, tags, tasks and the audit trail.": "External integrations linked to this agenda item — open the sidebar to link emails, browse files, notes, tags, tasks and the audit trail.",
        "External integrations linked to this decision dossier — open the sidebar to link emails, browse files, notes, tags, tasks and the audit trail.": "External integrations linked to this decision dossier — open the sidebar to link emails, browse files, notes, tags, tasks and the audit trail.",
        "External integrations linked to this meeting — open the sidebar to browse linked articles, files, notes, tags, tasks and the audit trail.": "External integrations linked to this meeting — open the sidebar to browse linked articles, files, notes, tags, tasks and the audit trail.",
        "External integrations linked to this motion — open the sidebar to browse the Discussion (Talk), files, notes, tags, tasks and the audit trail.": "External integrations linked to this motion — open the sidebar to browse the Discussion (Talk), files, notes, tags, tasks and the audit trail.",
        "External member": "External member",
        "External reference": "External reference",
        "External register identifier": "External register identifier",
        "Faction": "Faction",
        "Failed": "Failed",
        "Failed to add member.": "Failed to add member.",
        "Failed to add signer.": "Failed to add signer.",
        "Failed to cast vote": "Failed to cast vote",
        "Failed to change role.": "Failed to change role.",
        "Failed to close the cycle.": "Failed to close the cycle.",
        "Failed to generate the report.": "Failed to generate the report.",
        "Failed to link participant.": "Failed to link participant.",
        "Failed to load action items.": "Failed to load action items.",
        "Failed to load agenda.": "Failed to load agenda.",
        "Failed to load amendments.": "Failed to load amendments.",
        "Failed to load analytics.": "Failed to load analytics.",
        "Failed to load decisions.": "Failed to load decisions.",
        "Failed to load evaluations.": "Failed to load evaluations.",
        "Failed to load governance bodies.": "Failed to load governance bodies.",
        "Failed to load lifecycle state.": "Failed to load lifecycle state.",
        "Failed to load members.": "Failed to load members.",
        "Failed to load minutes.": "Failed to load minutes.",
        "Failed to load motions.": "Failed to load motions.",
        "Failed to load parent motion.": "Failed to load parent motion.",
        "Failed to load participants.": "Failed to load participants.",
        "Failed to load publication state.": "Failed to load publication state.",
        "Failed to load related decisions.": "Failed to load related decisions.",
        "Failed to load route.": "Failed to load route.",
        "Failed to load routed documents.": "Failed to load routed documents.",
        "Failed to load signers.": "Failed to load signers.",
        "Failed to load the amendment diff.": "Failed to load the amendment diff.",
        "Failed to load the governance body.": "Failed to load the governance body.",
        "Failed to load the meeting.": "Failed to load the meeting.",
        "Failed to load the minutes.": "Failed to load the minutes.",
        "Failed to load the ondermandaat chain.": "Failed to load the ondermandaat chain.",
        "Failed to load the version timeline.": "Failed to load the version timeline.",
        "Failed to load votes.": "Failed to load votes.",
        "Failed to load voting overview.": "Failed to load voting overview.",
        "Failed to load voting results.": "Failed to load voting results.",
        "Failed to open voting round": "Failed to open voting round",
        "Failed to publish agenda.": "Failed to publish agenda.",
        "Failed to publish the summary.": "Failed to publish the summary.",
        "Failed to reimport register.": "Failed to reimport register.",
        "Failed to save the template assignment.": "Failed to save the template assignment.",
        "Failed to start the evaluation.": "Failed to start the evaluation.",
        "Failed to submit your response.": "Failed to submit your response.",
        "File": "File",
        "Fill in the agenda item details. The chair will approve or reject your proposal.": "Fill in the agenda item details. The chair will approve or reject your proposal.",
        "Financial statements": "Financial statements",
        "Financial year": "Financial year",
        "Flows are what happens without anyone clicking: a reminder before a deadline passes, a confirmation sent on submission. This is where you read and edit them. Nothing to build now.": "Flows are what happens without anyone clicking: a reminder before a deadline passes, a confirmation sent on submission. This is where you read and edit them. Nothing to build now.",
        "For": "For",
        "For / Against / Abstain": "For / Against / Abstain",
        "For support, contact us at": "For support, contact us at",
        "For support, contact us at {email}": "For support, contact us at {email}",
        "For: {for} — Against: {against} — Abstain: {abstain}": "For: {for} — Against: {against} — Abstain: {abstain}",
        "Format": "Format",
        "Formerly: {citation}": "Formerly: {citation}",
        "Fraction": "Fraction",
        "Framework memorandum": "Framework memorandum",
        "Free-form label linking the records created by one intake run. Optional; drives the batch filter on the index.": "Free-form label linking the records created by one intake run. Optional; drives the batch filter on the index.",
        "Free-form label linking the records created by one turnover run. Optional; drives the batch filter on the index.": "Free-form label linking the records created by one turnover run. Optional; drives the batch filter on the index.",
        "Free-text source note naming the splitsingsakte article this deviation stems from.": "Free-text source note naming the splitsingsakte article this deviation stems from.",
        "French": "French",
        "Frequency": "Frequency",
        "From": "From",
        "Functional type of this stage: determines the role it plays in the route. `endorsement` is the parafering case: a sign-off that is not itself the decision.": "Functional type of this stage: determines the role it plays in the route. `endorsement` is the parafering case: a sign-off that is not itself the decision.",
        "Geen actieve stemronde.": "No active voting round.",
        "Geen amendementen.": "No amendments.",
        "Geen moties gevonden.": "No motions found.",
        "Geheime stemming": "Secret ballot",
        "General": "General",
        "Generate document": "Generate document",
        "Generate draft minutes": "Generate draft minutes",
        "Generate meeting series": "Generate meeting series",
        "Generate proof package": "Generate proof package",
        "Generate report": "Generate report",
        "Generate series": "Generate series",
        "Generated documents": "Generated documents",
        "Generating…": "Generating…",
        "Gepubliceerd naar ORI": "Published to ORI",
        "German": "German",
        "Gewogen stemming": "Weighted vote",
        "Gift": "Gift",
        "Give floor": "Give floor",
        "Give floor to {name}": "Give floor to {name}",
        "Global search": "Global search",
        "Governance Bodies": "Governance Bodies",
        "Governance Body": "Governance Body",
        "Governance body": "Governance body",
        "Governance body composition": "Governance body composition",
        "Governance context": "Governance context",
        "Governance email": "Governance email",
        "Governance model": "Governance model",
        "Government (gov)": "Government (gov)",
        "Grant": "Grant",
        "Grant Proxy": "Grant Proxy",
        "Grant proxy": "Grant proxy",
        "Grant proxy to": "Grant proxy to",
        "Grants competence for": "Grants competence for",
        "Ground": "Ground",
        "Guest": "Guest",
        "Handled": "Handled",
        "Handopsteking": "Show of hands",
        "Handopsteking resultaat opslaan": "Save show-of-hands result",
        "Het RSIN van de organisatie die verantwoordelijk is voor het besluit. Een RSIN en niet een `$ref`: de BRC identificeert de verantwoordelijke met dit nummer, en een verwijzing die alleen binnen deze instantie oplost zou het contract breken.": "Het RSIN van de organisatie die verantwoordelijk is voor het besluit. Een RSIN en niet een `$ref`: de BRC identificeert de verantwoordelijke met dit nummer, en een verwijzing die alleen binnen deze instantie oplost zou het contract breken.",
        "Hide": "Hide",
        "Hide meeting cost": "Hide meeting cost",
        "Hold": "Hold",
        "Hold type": "Hold type",
        "How": "How",
        "How MANY members are required for a valid sitting. Not how a quorum is calculated: that is `quorumRule`. Deliberately has NO default: an unset value means 'not specified', and a default of 0 would assert that no members are needed. Awb 7:13 objection advisory committees state this as a number.": "How MANY members are required for a valid sitting. Not how a quorum is calculated: that is `quorumRule`. Deliberately has NO default: an unset value means 'not specified', and a default of 0 would assert that no members are needed. Awb 7:13 objection advisory committees state this as a number.",
        "How a quorum is CALCULATED (e.g. majority). Not how many members are required: that is `quorum`, which is a number. A calculation method cannot express a statutory minimum member count.": "How a quorum is CALCULATED (e.g. majority). Not how many members are required: that is `quorum`, which is a number. A calculation method cannot express a statutory minimum member count.",
        "How far into the step's window to ask the actor's substitute as well, as a fraction between 0 and 1. Asking BEFORE the deadline rather than after it is the whole point: a substitute asked after the term has run is a substitute asked too late to help. Unset means no substitute is asked (REQ-AR-016).": "How far into the step's window to ask the actor's substitute as well, as a fraction between 0 and 1. Asking BEFORE the deadline rather than after it is the whole point: a substitute asked after the term has run is a substitute asked too late to help. Unset means no substitute is asked (REQ-AR-016).",
        "How far into this stage's window the actor's substitute is asked as well, copied from the step.": "How far into this stage's window the actor's substitute is asked as well, copied from the step.",
        "How long before the meeting an item may still be submitted.": "How long before the meeting an item may still be submitted.",
        "How long the party has, in days.": "How long the party has, in days.",
        "How many of a step's approvers have to grant before it closes. `all` is today's behaviour and stays the default, so no stored route changes meaning.": "How many of a step's approvers have to grant before it closes. `all` is today's behaviour and stays the default, so no stored route changes meaning.",
        "How many of a step's approvers have to grant before it closes. `all` is today's behaviour and stays the default, so no stored route changes meaning. COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).": "How many of a step's approvers have to grant before it closes. `all` is today's behaviour and stays the default, so no stored route changes meaning. COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).",
        "How many people may hold this position at the same time.": "How many people may hold this position at the same time.",
        "How many representatives the component sends.": "How many representatives the component sends.",
        "How many terms one person may serve back to back.": "How many terms one person may serve back to back.",
        "How the parties who may answer are determined.": "How the parties who may answer are determined.",
        "How the person was formally installed, where the organisation marks that at all. Free text: a council takes an oath or an affirmation, a board signs a deed, an association records a vote, and none of those is every organisation's word.": "How the person was formally installed, where the organisation marks that at all. Free text: a council takes an oath or an affirmation, a board signs a deed, an association records a vote, and none of those is every organisation's word.",
        "How the position is held, for example regular or interim.": "How the position is held, for example regular or interim.",
        "How the quorum is calculated.": "How the quorum is calculated.",
        "How this ground is classified, in whatever terms the organisation's own law or statutes use.": "How this ground is classified, in whatever terms the organisation's own law or statutes use.",
        "How to contest this besluit, copied from the decision at the moment it was published. A reader was told the legal ground the besluit rests on and not how to object to it, because the allow-list carried legalBasis and not this (REQ-DWP-007). Stamped, never read live: a term that changes next year must not change what this publication told someone last year.": "How to contest this besluit, copied from the decision at the moment it was published. A reader was told the legal ground the besluit rests on and not how to object to it, because the allow-list carried legalBasis and not this (REQ-DWP-007). Stamped, never read live: a term that changes next year must not change what this publication told someone last year.",
        "I confirm participants were informed of the recording.": "I confirm participants were informed of the recording.",
        "Id of the ApprovalRoute this stage was instantiated from. Written at instantiation; empty on stages that predate the runtime absorption, which announce no cross-app conclusion.": "Id of the ApprovalRoute this stage was instantiated from. Written at instantiation; empty on stages that predate the runtime absorption, which announce no cross-app conclusion.",
        "Implemented by": "Implemented by",
        "Implements": "Implements",
        "Import failed.": "Import failed.",
        "Import from CSV": "Import from CSV",
        "Import from Nextcloud group": "Import from Nextcloud group",
        "Import members": "Import members",
        "Import {count} members": "Import {count} members",
        "Importing...": "Importing...",
        "Importing…": "Importing…",
        "Imposed": "Imposed",
        "Imposed by the chair": "Imposed by the chair",
        "Imposed by the executive board": "Imposed by the executive board",
        "Imposed by the governing body": "Imposed by the governing body",
        "In Deck": "In Deck",
        "In app": "In app",
        "In effect": "In effect",
        "In execution": "In execution",
        "In force": "In force",
        "In force from {date}": "In force from {date}",
        "In line with advice": "In line with advice",
        "In line with consent": "In line with consent",
        "In preparation": "In preparation",
        "In progress": "In progress",
        "In review": "In review",
        "In use": "In use",
        "Include with agenda item": "Include with agenda item",
        "Incoming documents": "Incoming documents",
        "Individual": "Individual",
        "Induction pack": "Induction pack",
        "Information about the current Decidiq installation": "Information about the current Decidiq installation",
        "Informational": "Informational",
        "Ingediend": "Submitted",
        "Ingetrokken": "Withdrawn",
        "Initial lifecycle state": "Initial lifecycle state",
        "Initial state": "Initial state",
        "Install OpenRegister": "Install OpenRegister",
        "Install governance configuration that other organisations have published: a council with its committees and factions, an association with its members' meeting, or the flows that move a decision through them.": "Install governance configuration that other organisations have published: a council with its committees and factions, an association with its members' meeting, or the flows that move a decision through them.",
        "Instances in series {series}": "Instances in series {series}",
        "Integrations": "Integrations",
        "Interface language follows your Nextcloud account language.": "Interface language follows your Nextcloud account language.",
        "Interim succession": "Interim succession",
        "Internal": "Internal",
        "Internal regulations": "Internal regulations",
        "Interval": "Interval",
        "Invalid email address": "Invalid email address",
        "Invalid row": "Invalid row",
        "Invitation": "Invitation",
        "Invite Co-Signatories": "Invite Co-Signatories",
        "Involved in handling": "Involved in handling",
        "Italian": "Italian",
        "Item": "Item",
        "Item closed ({minutes} min)": "Item closed ({minutes} min)",
        "Items of this kind": "Items of this kind",
        "Items per page": "Items per page",
        "Joined": "Joined",
        "Jurisdiction": "Jurisdiction",
        "Kascommissie report": "Kascommissie report",
        "Keep at least one delivery channel enabled. Use the per-event switches to mute specific notifications.": "Keep at least one delivery channel enabled. Use the per-event switches to mute specific notifications.",
        "Keep everything": "Keep everything",
        "Key": "Key",
        "Kind": "Kind",
        "Koppelen": "Link",
        "Label": "Label",
        "Laden…": "Loading…",
        "Language": "Language",
        "Lapsed": "Lapsed",
        "Lapsed at": "Lapsed at",
        "Last checked": "Last checked",
        "Latest meeting: {title}": "Latest meeting: {title}",
        "Leave empty to skip catalog routing": "Leave empty to skip catalog routing",
        "Leave empty to use your Nextcloud account email.": "Leave empty to use your Nextcloud account email.",
        "Left": "Left",
        "Legal remedies": "Legal remedies",
        "Legal remedy clause": "Legal remedy clause",
        "Length of one term in this position, in months.": "Length of one term in this position, in months.",
        "Less than 24 hours remaining": "Less than 24 hours remaining",
        "Letter organisation": "Letter organisation",
        "Letter resident": "Letter resident",
        "Lifecycle": "Lifecycle",
        "Lifecycle states": "Lifecycle states",
        "Lifecycle unavailable": "Lifecycle unavailable",
        "Limited": "Limited",
        "Line": "Line",
        "Link a motion to this agenda item": "Link a motion to this agenda item",
        "Link email": "Link email",
        "Link motion": "Link motion",
        "Linked Meeting": "Linked Meeting",
        "Linked emails": "Linked emails",
        "Linked motions": "Linked motions",
        "Live meeting": "Live meeting",
        "Live meeting view": "Live meeting view",
        "Loading action items…": "Loading action items…",
        "Loading agenda…": "Loading agenda…",
        "Loading amendments…": "Loading amendments…",
        "Loading analytics…": "Loading analytics…",
        "Loading chain…": "Loading chain…",
        "Loading decisions…": "Loading decisions…",
        "Loading group members…": "Loading group members…",
        "Loading members…": "Loading members…",
        "Loading minutes…": "Loading minutes…",
        "Loading motions…": "Loading motions…",
        "Loading participants…": "Loading participants…",
        "Loading related decisions…": "Loading related decisions…",
        "Loading routed documents…": "Loading routed documents…",
        "Loading route…": "Loading route…",
        "Loading series instances…": "Loading series instances…",
        "Loading signers…": "Loading signers…",
        "Loading template assignment…": "Loading template assignment…",
        "Loading versions…": "Loading versions…",
        "Loading votes…": "Loading votes…",
        "Loading voting overview…": "Loading voting overview…",
        "Loading voting results…": "Loading voting results…",
        "Loading…": "Loading…",
        "Location": "Location",
        "Logo URL": "Logo URL",
        "Majority overrides": "Majority overrides",
        "Majority threshold": "Majority threshold",
        "Manage the OpenRegister configuration for Decidiq.": "Manage the OpenRegister configuration for Decidiq.",
        "Management charter": "Management charter",
        "Management point": "Management point",
        "Mandate": "Mandate",
        "Mandatory": "Mandatory",
        "Manual only": "Manual only",
        "Markdown": "Markdown",
        "Markdown fallback": "Markdown fallback",
        "Max consecutive terms": "Max consecutive terms",
        "Maximum consecutive terms": "Maximum consecutive terms",
        "Medeondertekenaars": "Co-signatories",
        "Medeondertekenaars uitnodigen": "Invite co-signatories",
        "Meeting": "Meeting",
        "Meeting \"%1$s\" moved to \"%2$s\"": "Meeting \"%1$s\" moved to \"%2$s\"",
        "Meeting cost": "Meeting cost",
        "Meeting date": "Meeting date",
        "Meeting duration": "Meeting duration",
        "Meeting integrations": "Meeting integrations",
        "Meeting not found": "Meeting not found",
        "Meeting outcome": "Meeting outcome",
        "Meeting package assembled": "Meeting package assembled",
        "Meeting reminder": "Meeting reminder",
        "Meeting reminder timing": "Meeting reminder timing",
        "Meeting scheduled": "Meeting scheduled",
        "Meeting status distribution": "Meeting status distribution",
        "Meeting type": "Meeting type",
        "Meeting type (deprecated enum)": "Meeting type (deprecated enum)",
        "Meeting types": "Meeting types",
        "Meeting view": "Meeting view",
        "Meeting {object} moved to \"%1$s\"": "Meeting {object} moved to \"%1$s\"",
        "Meetings": "Meetings",
        "Meetings ({n})": "Meetings ({n})",
        "Meetings without a date": "Meetings without a date",
        "Member": "Member",
        "Member offboarding": "Member offboarding",
        "Member onboarding": "Member onboarding",
        "Members": "Members",
        "Members ({n})": "Members ({n})",
        "Membership": "Membership",
        "Mention": "Mention",
        "Mentioned in a comment": "Mentioned in a comment",
        "Minimum attendees for a valid sitting.": "Minimum attendees for a valid sitting.",
        "Minute taking": "Minute taking",
        "Minutes": "Minutes",
        "Minutes (live)": "Minutes (live)",
        "Minutes ({n})": "Minutes ({n})",
        "Minutes attendance rendering": "Minutes attendance rendering",
        "Minutes lifecycle": "Minutes lifecycle",
        "Minutes policy": "Minutes policy",
        "Missing name": "Missing name",
        "Missing statutory ALV agenda items": "Missing statutory ALV agenda items",
        "Mjop adoption": "Mjop adoption",
        "Mode": "Mode",
        "Moderation note (optional)": "Moderation note (optional)",
        "Moderation queue": "Moderation queue",
        "Mogelijk conflict": "Possible conflict",
        "Mogelijk conflict met ander amendement — raadpleeg de griffier": "Possible conflict with another amendment — consult the clerk",
        "Monetary Amounts": "Monetary Amounts",
        "Motie intrekken": "Withdraw motion",
        "Motie koppelen": "Link motion",
        "Motion": "Motion",
        "Motion Details": "Motion Details",
        "Motion integrations": "Motion integrations",
        "Motion text": "Motion text",
        "Motions": "Motions",
        "Move action item to another status": "Move action item to another status",
        "Move amendment down": "Move amendment down",
        "Move amendment up": "Move amendment up",
        "Move {name} down": "Move {name} down",
        "Move {name} up": "Move {name} up",
        "Move {title} down": "Move {title} down",
        "Move {title} up": "Move {title} up",
        "Municipalities act": "Municipalities act",
        "Municipality": "Municipality",
        "Name": "Name",
        "Name of the position as the organisation calls it.": "Name of the position as the organisation calls it.",
        "Names of role-holders": "Names of role-holders",
        "Natural person": "Natural person",
        "Negative": "Negative",
        "Negative advice": "Negative advice",
        "New board": "New board",
        "New meeting": "New meeting",
        "New member": "New member",
        "Next meeting": "Next meeting",
        "Next month": "Next month",
        "Next phase": "Next phase",
        "Nextcloud group": "Nextcloud group",
        "Nextcloud locale (default)": "Nextcloud locale (default)",
        "Nextcloud notification": "Nextcloud notification",
        "Nextcloud user id linked to the person by the account-linking step. Optional until linked; the induction-pack step depends on it.": "Nextcloud user id linked to the person by the account-linking step. Optional until linked; the induction-pack step depends on it.",
        "No": "No",
        "No account — manual linking needed": "No account — manual linking needed",
        "No action items assigned to you": "No action items assigned to you",
        "No action items spawned by this decision yet.": "No action items spawned by this decision yet.",
        "No active budget rounds": "No active budget rounds",
        "No active motions": "No active motions",
        "No active voting round.": "No active voting round.",
        "No advice": "No advice",
        "No agenda item types configured yet. An example set seeds some, or add your own.": "No agenda item types configured yet. An example set seeds some, or add your own.",
        "No agenda items use this kind yet.": "No agenda items use this kind yet.",
        "No agenda items yet for this meeting.": "No agenda items yet for this meeting.",
        "No agenda items.": "No agenda items.",
        "No amendments": "No amendments",
        "No amendments for this motion yet.": "No amendments for this motion yet.",
        "No amendments for this motion.": "No amendments for this motion.",
        "No answer within the term": "No answer within the term",
        "No audio files found in this meeting's folder. Upload a recording to the meeting folder, then refresh.": "No audio files found in this meeting's folder. Upload a recording to the meeting folder, then refresh.",
        "No board meetings yet": "No board meetings yet",
        "No boards yet": "No boards yet",
        "No body compositions configured yet.": "No body compositions configured yet.",
        "No co-signatories yet.": "No co-signatories yet.",
        "No consultation responses yet.": "No consultation responses yet.",
        "No consultations were held on this decision.": "No consultations were held on this decision.",
        "No consultations yet.": "No consultations yet.",
        "No corrections suggested.": "No corrections suggested.",
        "No decision-making linked to this object yet.": "No decision-making linked to this object yet.",
        "No decisions yet": "No decisions yet",
        "No decisions yet for this meeting.": "No decisions yet for this meeting.",
        "No documents generated yet.": "No documents generated yet.",
        "No draft minutes exist for this meeting yet.": "No draft minutes exist for this meeting yet.",
        "No governance bodies found.": "No governance bodies found.",
        "No hourly rate configured on this governance body — set one to see the running cost.": "No hourly rate configured on this governance body — set one to see the running cost.",
        "No incoming documents routed to this meeting yet.": "No incoming documents routed to this meeting yet.",
        "No linked governance body.": "No linked governance body.",
        "No linked meeting.": "No linked meeting.",
        "No linked motion.": "No linked motion.",
        "No linked motions.": "No linked motions.",
        "No meeting is linked to these minutes — the proof package needs a meeting.": "No meeting is linked to these minutes — the proof package needs a meeting.",
        "No meeting linked — the voting round cannot be opened": "No meeting linked — the voting round cannot be opened",
        "No meeting types configured yet. An example set seeds some, or add your own.": "No meeting types configured yet. An example set seeds some, or add your own.",
        "No meetings": "No meetings",
        "No meetings found. Create a meeting to see status distribution.": "No meetings found. Create a meeting to see status distribution.",
        "No meetings recorded for this body yet.": "No meetings recorded for this body yet.",
        "No members linked to this body yet.": "No members linked to this body yet.",
        "No minutes yet for this meeting.": "No minutes yet for this meeting.",
        "No more participants available to link.": "No more participants available to link.",
        "No motion is linked to this decision, so there are no voting results.": "No motion is linked to this decision, so there are no voting results.",
        "No motion linked to this decision item.": "No motion linked to this decision item.",
        "No motions for this agenda item yet.": "No motions for this agenda item yet.",
        "No motions for this agenda item.": "No motions for this agenda item.",
        "No motions found for this meeting.": "No motions found for this meeting.",
        "No object store available": "No object store available",
        "No open consultations": "No open consultations",
        "No other meetings in this series yet.": "No other meetings in this series yet.",
        "No parent motion": "No parent motion",
        "No parent motion linked.": "No parent motion linked.",
        "No participants found.": "No participants found.",
        "No participants linked to this meeting yet.": "No participants linked to this meeting yet.",
        "No pending votes": "No pending votes",
        "No position types configured yet. An example set seeds some, or add your own.": "No position types configured yet. An example set seeds some, or add your own.",
        "No positions configured for this body yet.": "No positions configured for this body yet.",
        "No proposals to vote on yet": "No proposals to vote on yet",
        "No proposed text": "No proposed text",
        "No reactions awaiting moderation": "No reactions awaiting moderation",
        "No recurring agenda items found.": "No recurring agenda items found.",
        "No related decisions": "No related decisions",
        "No related meetings.": "No related meetings.",
        "No related participants.": "No related participants.",
        "No resolutions yet": "No resolutions yet",
        "No responses recorded yet.": "No responses recorded yet.",
        "No responses were recorded for this decision.": "No responses were recorded for this decision.",
        "No results found": "No results found",
        "No self-evaluation cycles yet for this body.": "No self-evaluation cycles yet for this body.",
        "No settings available yet": "No settings available yet",
        "No sign-off route on this object yet.": "No sign-off route on this object yet.",
        "No signatures collected yet.": "No signatures collected yet.",
        "No signers added yet.": "No signers added yet.",
        "No speakers in the queue.": "No speakers in the queue.",
        "No speech-to-text provider is installed on this instance. You can still attach a recording and record consent; transcription becomes available once a provider (e.g. a local Whisper app) is installed.": "No speech-to-text provider is installed on this instance. You can still attach a recording and record consent; transcription becomes available once a provider (e.g. a local Whisper app) is installed.",
        "No spokesperson assigned.": "No spokesperson assigned.",
        "No staged route configured": "No staged route configured",
        "No stand-in was found for your sign-off": "No stand-in was found for your sign-off",
        "No term set": "No term set",
        "No time allocated — elapsed time is tracked for analytics.": "No time allocated — elapsed time is tracked for analytics.",
        "No transitions available from this state.": "No transitions available from this state.",
        "No unassigned participants available.": "No unassigned participants available.",
        "No upcoming meetings": "No upcoming meetings",
        "No versions have been recorded for this record yet.": "No versions have been recorded for this record yet.",
        "No versions yet": "No versions yet",
        "No view": "No view",
        "No votes recorded for this decision yet.": "No votes recorded for this decision yet.",
        "No votes recorded for this motion yet.": "No votes recorded for this motion yet.",
        "No voting recorded for this meeting": "No voting recorded for this meeting",
        "No voting recorded for this meeting.": "No voting recorded for this meeting.",
        "No voting round for this item.": "No voting round for this item.",
        "Nobody assigned": "Nobody assigned",
        "Nobody holds a position on this body yet.": "Nobody holds a position on this body yet.",
        "Nobody holds a position yet.": "Nobody holds a position yet.",
        "Nobody holds this position yet.": "Nobody holds this position yet.",
        "Nobody is recorded as your substitute, so this sign-off is still waiting on you. Its deadline has not moved.": "Nobody is recorded as your substitute, so this sign-off is still waiting on you. Its deadline has not moved.",
        "Nog geen medeondertekenaars.": "No co-signatories yet.",
        "None": "None",
        "None, I will set this up myself": "None, I will set this up myself",
        "Not adopted": "Not adopted",
        "Not appointed": "Not appointed",
        "Not available": "Not available",
        "Not configured": "Not configured",
        "Not enough data": "Not enough data",
        "Not handled": "Not handled",
        "Not issued": "Not issued",
        "Not now": "Not now",
        "Not published.": "Not published.",
        "Not pursued": "Not pursued",
        "Not submitted": "Not submitted",
        "Not yet asked": "Not yet asked",
        "Notarial proof package": "Notarial proof package",
        "Noted for information": "Noted for information",
        "Nothing to project.": "Nothing to project.",
        "Notice deliveries ({n})": "Notice deliveries ({n})",
        "Notification preferences": "Notification preferences",
        "Notification preferences saved.": "Notification preferences saved.",
        "Notification, display, delegation and communication preferences for your account.": "Notification, display, delegation and communication preferences for your account.",
        "Notifications": "Notifications",
        "Notify me about": "Notify me about",
        "Notify on decision published": "Notify on decision published",
        "Notify on meeting scheduled": "Notify on meeting scheduled",
        "Notify on mention": "Notify on mention",
        "Notify on task assigned": "Notify on task assigned",
        "Notify on vote opened": "Notify on vote opened",
        "Number": "Number",
        "ORI API endpoint URL": "ORI API endpoint URL",
        "ORI Endpoint": "ORI Endpoint",
        "ORI endpoint": "ORI endpoint",
        "ORI endpoint URL for publishing voting results": "ORI endpoint URL for publishing voting results",
        "ORI niet geconfigureerd": "ORI not configured",
        "ORI not configured": "ORI not configured",
        "ORI-eindpunt": "ORI endpoint",
        "Oath": "Oath",
        "Objection decision": "Objection decision",
        "Observer": "Observer",
        "Offers": "Offers",
        "Official title": "Official title",
        "On a recomputation action, the share or count the step required before an administrator changed it.": "On a recomputation action, the share or count the step required before an administrator changed it.",
        "On silence": "On silence",
        "On silence policy": "On silence policy",
        "Once someone asks colleagues to sign off on this, every step appears here with what they did and why.": "Once someone asks colleagues to sign off on this, every step appears here with what they did and why.",
        "Ondermandaat chain": "Ondermandaat chain",
        "Ondermandaten": "Ondermandaten",
        "Ondersteunen": "Confirm co-signature",
        "Only the chair or secretary can open this cycle.": "Only the chair or secretary can open this cycle.",
        "Onthouding": "Abstain",
        "Onthoudingen": "Abstentions",
        "Ontvankelijkheid": "Ontvankelijkheid",
        "Oordeelsvorming": "Opinion forming",
        "Open": "Open",
        "Open Debate": "Open Debate",
        "Open Decidiq": "Open Decidiq",
        "Open Flows in the menu": "Open Flows in the menu",
        "Open Voting Round": "Open Voting Round",
        "Open consultations": "Open consultations",
        "Open consultations and participatory budget rounds for your governance body.": "Open consultations and participatory budget rounds for your governance body.",
        "Open for responses": "Open for responses",
        "Open in Deck": "Open in Deck",
        "Open in decidiq": "Open in decidiq",
        "Open items": "Open items",
        "Open live meeting view": "Open live meeting view",
        "Open package folder": "Open package folder",
        "Open parent motion": "Open parent motion",
        "Open settings": "Open settings",
        "Open the decision that replaced this one": "Open the decision that replaced this one",
        "Open vote": "Open vote",
        "Open voting": "Open voting",
        "Open voting round": "Open voting round",
        "OpenCatalogi is not installed — the record received the public predicate but was not routed to a catalog.": "OpenCatalogi is not installed — the record received the public predicate but was not routed to a catalog.",
        "OpenRegister is required": "OpenRegister is required",
        "OpenRegister register ID": "OpenRegister register ID",
        "Opened": "Opened",
        "Openen": "Open",
        "Opening": "Opening",
        "Operations (ops)": "Operations (ops)",
        "Optional display label for this composition.": "Optional display label for this composition.",
        "Optional object UUID linking what the step is about: the meeting where the person was installed, the induction pack that was delivered, the membership that was created.": "Optional object UUID linking what the step is about: the meeting where the person was installed, the induction pack that was delivered, the membership that was created.",
        "Optional quorum fraction (e.g. '2/3') required for this category per the akte. Null means no quorum beyond the body default.": "Optional quorum fraction (e.g. '2/3') required for this category per the akte. Null means no quorum beyond the body default.",
        "Order": "Order",
        "Order saved": "Order saved",
        "Ordered states an item of this kind moves through.": "Ordered states an item of this kind moves through.",
        "Orders": "Orders",
        "Organisation mode": "Organisation mode",
        "Organization": "Organization",
        "Organization defaults applied to meetings, decisions, and generated documents": "Organization defaults applied to meetings, decisions, and generated documents",
        "Organization name": "Organization name",
        "Organization settings saved": "Organization settings saved",
        "Origin": "Origin",
        "Origin commitment": "Origin commitment",
        "Other": "Other",
        "Other activities": "Other activities",
        "Outcome": "Outcome",
        "Outstanding": "Outstanding",
        "Over limit": "Over limit",
        "Over time": "Over time",
        "Overall score: {score}": "Overall score: {score}",
        "Overdue": "Overdue",
        "Overdue actions": "Overdue actions",
        "Overlapping edit conflict detected": "Overlapping edit conflict detected",
        "Owner": "Owner",
        "Owning body": "Owning body",
        "P&C cycles": "P&C cycles",
        "PDF (via Docudesk when available)": "PDF (via Docudesk when available)",
        "Package assembly failed": "Package assembly failed",
        "Package assembly failed.": "Package assembly failed.",
        "Parafering": "Parafering",
        "Parent Motion": "Parent Motion",
        "Parent motion": "Parent motion",
        "Parent motion not found": "Parent motion not found",
        "Participant": "Participant",
        "Participant UUID": "Participant UUID",
        "Participants": "Participants",
        "Participation": "Participation",
        "Participatory budget rounds": "Participatory budget rounds",
        "Participatory budgets": "Participatory budgets",
        "Partly adopted": "Partly adopted",
        "Party": "Party",
        "Party affiliation": "Party affiliation",
        "Pause": "Pause",
        "Pause timer": "Pause timer",
        "Paused": "Paused",
        "Pending": "Pending",
        "Pending vote": "Pending vote",
        "Pending votes": "Pending votes",
        "Pending votes: %s": "Pending votes: %s",
        "Per-dimension and free-text breakdowns are hidden: too few respondents to protect anonymity.": "Per-dimension and free-text breakdowns are hidden: too few respondents to protect anonymity.",
        "Personal data note": "Personal data note",
        "Personal settings": "Personal settings",
        "Petition": "Petition",
        "Phone for urgent matters": "Phone for urgent matters",
        "Photo": "Photo",
        "Pick a participant": "Pick a participant",
        "Pick a participant to link to this governance body.": "Pick a participant to link to this governance body.",
        "Pick a participant to link to this meeting.": "Pick a participant to link to this meeting.",
        "Pick a participant to request a signature from.": "Pick a participant to request a signature from.",
        "Placeholder: comment added": "Placeholder: comment added",
        "Placeholder: status changed to Review": "Placeholder: status changed to Review",
        "Placeholder: user opened a record": "Placeholder: user opened a record",
        "Planned": "Planned",
        "Planned agenda": "Planned agenda",
        "Planned agenda item": "Planned agenda item",
        "Planned length, used to prefill the end date.": "Planned length, used to prefill the end date.",
        "Planning cycle": "Planning cycle",
        "Planning cycle step": "Planning cycle step",
        "Planning cycle template": "Planning cycle template",
        "Policy": "Policy",
        "Policy rule": "Policy rule",
        "Political group assignment": "Political group assignment",
        "Portfolio holder": "Portfolio holder",
        "Position": "Position",
        "Position hold": "Position hold",
        "Position holder": "Position holder",
        "Position holders": "Position holders",
        "Position type": "Position type",
        "Position types": "Position types",
        "Positions": "Positions",
        "Positions additionally competent to take decisions of this type.": "Positions additionally competent to take decisions of this type.",
        "Positive": "Positive",
        "Positive advice": "Positive advice",
        "Positive with reservations": "Positive with reservations",
        "Possible conflict with another amendment — consult the clerk": "Possible conflict with another amendment — consult the clerk",
        "Post-moderation (auto-approve authenticated)": "Post-moderation (auto-approve authenticated)",
        "Postponed": "Postponed",
        "Power of attorney": "Power of attorney",
        "Pre-moderation (approve before counting)": "Pre-moderation (approve before counting)",
        "Preferred language for communications": "Preferred language for communications",
        "Previous month": "Previous month",
        "Private": "Private",
        "Process template": "Process template",
        "Process templates": "Process templates",
        "Processed": "Processed",
        "Processing": "Processing",
        "Processing notes": "Processing notes",
        "Products": "Products",
        "Project to Deck": "Project to Deck",
        "Projected to Deck": "Projected to Deck",
        "Projection failed.": "Projection failed.",
        "Prompt on transition": "Prompt on transition",
        "Proof package sealed (SHA-256 {hash}).": "Proof package sealed (SHA-256 {hash}).",
        "Properties": "Properties",
        "Proposal": "Proposal",
        "Proposal submitted": "Proposal submitted",
        "Proposal title": "Proposal title",
        "Proposals": "Proposals",
        "Propose": "Propose",
        "Propose agenda item": "Propose agenda item",
        "Proposed": "Proposed",
        "Proposed items": "Proposed items",
        "Proposer": "Proposer",
        "Proxies are granted per voting round from the voting panel.": "Proxies are granted per voting round from the voting panel.",
        "Public": "Public",
        "Public publication": "Public publication",
        "Publicatie in behandeling": "Publication pending",
        "Publicatiedatum": "Publicatiedatum",
        "Publication configuration saved.": "Publication configuration saved.",
        "Publication error": "Publication error",
        "Publication failed": "Publication failed",
        "Publication failed.": "Publication failed.",
        "Publication history": "Publication history",
        "Publication pending": "Publication pending",
        "Publication warning": "Publication warning",
        "Publiceren naar ORI": "Publish to ORI",
        "Publish": "Publish",
        "Publish agenda": "Publish agenda",
        "Publish allocation": "Publish allocation",
        "Publish now": "Publish now",
        "Publish results": "Publish results",
        "Publish summary": "Publish summary",
        "Publish this decision?": "Publish this decision?",
        "Publish to ORI": "Publish to ORI",
        "Published": "Published",
        "Published as {oriType} (version {version}) on {date}": "Published as {oriType} (version {version}) on {date}",
        "Published to ORI": "Published to ORI",
        "Publishing to the OpenCatalogi catalog failed.": "Publishing to the OpenCatalogi catalog failed.",
        "Put": "Put",
        "Qualified": "Qualified",
        "Qualified majority (2/3)": "Qualified majority (2/3)",
        "Qualified majority (2/3) with elevated quorum.": "Qualified majority (2/3) with elevated quorum.",
        "Qualified majority (3/4)": "Qualified majority (3/4)",
        "Questions raised": "Questions raised",
        "Quick actions": "Quick actions",
        "Quorum %": "Quorum %",
        "Quorum (members)": "Quorum (members)",
        "Quorum Met": "Quorum Met",
        "Quorum Percentage": "Quorum Percentage",
        "Quorum Required": "Quorum Required",
        "Quorum Rule": "Quorum Rule",
        "Quorum fraction": "Quorum fraction",
        "Quorum niet bereikt": "Quorum not met",
        "Quorum not reached": "Quorum not reached",
        "Quorum required": "Quorum required",
        "Quorum required before voting": "Quorum required before voting",
        "Quorum rule": "Quorum rule",
        "Ranked Choice": "Ranked Choice",
        "Ratification": "Ratification",
        "Ratified": "Ratified",
        "Rationale": "Rationale",
        "Re-align to agenda": "Re-align to agenda",
        "Reaction approved": "Reaction approved",
        "Reaction moderation queue": "Reaction moderation queue",
        "Reaction rejected": "Reaction rejected",
        "Realised": "Realised",
        "Reappointable": "Reappointable",
        "Reason for recusal": "Reason for recusal",
        "Reason for the correction (optional)": "Reason for the correction (optional)",
        "Reason, required to reject": "Reason, required to reject",
        "Reason:": "Reason:",
        "Received": "Received",
        "Recent activity": "Recent activity",
        "Recipient": "Recipient",
        "Reclaim task": "Reclaim task",
        "Reclaimed": "Reclaimed",
        "Record decision": "Record decision",
        "Recorded during minute-taking on agenda item: {title}": "Recorded during minute-taking on agenda item: {title}",
        "Recording retention": "Recording retention",
        "Recording source": "Recording source",
        "Rectification publishes a corrected new version and withdraws the current one in a single operation. The new version references the version it corrects.": "Rectification publishes a corrected new version and withdraws the current one in a single operation. The new version references the version it corrects.",
        "Rectified": "Rectified",
        "Rectify": "Rectify",
        "Rectify publication": "Rectify publication",
        "Rectify…": "Rectify…",
        "Recurring": "Recurring",
        "Recurring series": "Recurring series",
        "Reference to the Meeting where the person was installed. The meeting itself is owned elsewhere and only linked here.": "Reference to the Meeting where the person was installed. The meeting itself is owned elsewhere and only linked here.",
        "Reference to the Membership created as an outcome. Optional until created: an intake often onboards a person before the membership row exists.": "Reference to the Membership created as an outcome. Optional until created: an intake often onboards a person before the membership row exists.",
        "Reference to the Membership to be end-dated. Required. End-dating happens on the end-membership step, never as an automatic side effect.": "Reference to the Membership to be end-dated. Required. End-dating happens on the end-membership step, never as an automatic side effect.",
        "Reference to the governance body the person is joining. Required. With targetRole it derives the body-role group mapping the assign-groups step uses.": "Reference to the governance body the person is joining. Required. With targetRole it derives the body-role group mapping the assign-groups step uses.",
        "Reference to the governance body the person is leaving. Required. With the person's role it derives the body-role group mapping the revoke-groups step uses.": "Reference to the governance body the person is leaving. Required. With the person's role it derives the body-role group mapping the revoke-groups step uses.",
        "Referenced by": "Referenced by",
        "Referred to executive board for disposal": "Referred to executive board for disposal",
        "Referred to executive board for preparation": "Referred to executive board for preparation",
        "Refers to": "Refers to",
        "Refused": "Refused",
        "Register": "Register",
        "Register Configuration": "Register Configuration",
        "Register successfully reimported.": "Register successfully reimported.",
        "Registered": "Registered",
        "Regulation version": "Regulation version",
        "Regulations": "Regulations",
        "Reimport Register": "Reimport Register",
        "Reject": "Reject",
        "Reject correction": "Reject correction",
        "Reject minutes": "Reject minutes",
        "Reject proposal {title}": "Reject proposal {title}",
        "Reject reaction": "Reject reaction",
        "Rejected": "Rejected",
        "Rejection comment": "Rejection comment",
        "Rejection reason": "Rejection reason",
        "Reject…": "Reject…",
        "Related Meetings": "Related Meetings",
        "Related Participants": "Related Participants",
        "Related decisions": "Related decisions",
        "Relation rejected": "Relation rejected",
        "Relation type": "Relation type",
        "Relocation": "Relocation",
        "Reminder 3 months": "Reminder 3 months",
        "Reminder 6 months": "Reminder 6 months",
        "Remove": "Remove",
        "Remove failed.": "Remove failed.",
        "Remove from body": "Remove from body",
        "Remove from consent agenda": "Remove from consent agenda",
        "Remove from meeting": "Remove from meeting",
        "Remove member": "Remove member",
        "Remove member from workspace": "Remove member from workspace",
        "Remove relation": "Remove relation",
        "Remove signer": "Remove signer",
        "Remove spokesperson": "Remove spokesperson",
        "Remove state": "Remove state",
        "Remove transition": "Remove transition",
        "Remove {name} from queue": "Remove {name} from queue",
        "Remove {title} from consent agenda": "Remove {title} from consent agenda",
        "Removed text": "Removed text",
        "Reopen round (revote)": "Reopen round (revote)",
        "Repealed": "Repealed",
        "Repealed by": "Repealed by",
        "Repeals": "Repeals",
        "Replaced": "Replaced",
        "Reply": "Reply",
        "Report adoption": "Report adoption",
        "Reported": "Reported",
        "Reports": "Reports",
        "Representation seat": "Representation seat",
        "Representation seats": "Representation seats",
        "Requested amount": "Requested amount",
        "Reserve fund contribution": "Reserve fund contribution",
        "Resignation": "Resignation",
        "Resolution": "Resolution",
        "Resolution \"%1$s\" was adopted": "Resolution \"%1$s\" was adopted",
        "Resolution not found": "Resolution not found",
        "Resolution {object} was adopted": "Resolution {object} was adopted",
        "Resolutions": "Resolutions",
        "Resolutions ({n})": "Resolutions ({n})",
        "Resolutions are proposed from a board meeting.": "Resolutions are proposed from a board meeting.",
        "Resolve thread": "Resolve thread",
        "Respond anonymously": "Respond anonymously",
        "Respondent": "Respondent",
        "Respondent body": "Respondent body",
        "Respondent identifier": "Respondent identifier",
        "Response": "Response",
        "Responses": "Responses",
        "Restore version": "Restore version",
        "Restricted": "Restricted",
        "Restrictions": "Restrictions",
        "Result": "Result",
        "Result:": "Result:",
        "Resultaat opslaan": "Save result",
        "Results published": "Results published",
        "Results published (with a warning)": "Results published (with a warning)",
        "Resume": "Resume",
        "Resume timer": "Resume timer",
        "Retention policy": "Retention policy",
        "Retraction from the OpenCatalogi catalog failed and is pending retry — the record is no longer publicly readable but the catalog still lists it.": "Retraction from the OpenCatalogi catalog failed and is pending retry — the record is no longer publicly readable but the catalog still lists it.",
        "Retry transcription": "Retry transcription",
        "Returned to draft": "Returned to draft",
        "Revise agenda": "Revise agenda",
        "Revoke Proxy": "Revoke Proxy",
        "Revoke groups": "Revoke groups",
        "Revoke proxy": "Revoke proxy",
        "Revoked": "Revoked",
        "Role": "Role",
        "Route": "Route",
        "Route Complete": "Route Complete",
        "Routing determined": "Routing determined",
        "Rules of procedure": "Rules of procedure",
        "Rules: {threshold} · {abstentions} · {tieBreak} — base: {base}": "Rules: {threshold} · {abstentions} · {tieBreak} — base: {base}",
        "Save": "Save",
        "Save Result": "Save Result",
        "Save communication preferences": "Save communication preferences",
        "Save delegation": "Save delegation",
        "Save display preferences": "Save display preferences",
        "Save failed.": "Save failed.",
        "Save notification preferences": "Save notification preferences",
        "Save order": "Save order",
        "Save publication configuration": "Save publication configuration",
        "Save result": "Save result",
        "Save retention policy": "Save retention policy",
        "Save show-of-hands result": "Save show-of-hands result",
        "Save voting order": "Save voting order",
        "Saved": "Saved",
        "Saving failed.": "Saving failed.",
        "Saving the order failed": "Saving the order failed",
        "Saving the order failed.": "Saving the order failed.",
        "Saving …": "Saving …",
        "Saving...": "Saving...",
        "Saving…": "Saving…",
        "Schedule": "Schedule",
        "Schedule a board meeting": "Schedule a board meeting",
        "Schedule a meeting from a board's detail page.": "Schedule a meeting from a board's detail page.",
        "Schedule board meeting": "Schedule board meeting",
        "Scheduled": "Scheduled",
        "Scheduled Date": "Scheduled Date",
        "Scheduled date": "Scheduled date",
        "Search across all governance data": "Search across all governance data",
        "Search decisions…": "Search decisions…",
        "Search meetings, motions, decisions…": "Search meetings, motions, decisions…",
        "Search results": "Search results",
        "Searching…": "Searching…",
        "Seat position": "Seat position",
        "Seats": "Seats",
        "Secret Ballot": "Secret Ballot",
        "Secret ballot": "Secret ballot",
        "Secret ballot with candidate rounds.": "Secret ballot with candidate rounds.",
        "Secretary": "Secretary",
        "Section discarded — write your own text in the minutes editor.": "Section discarded — write your own text in the minutes editor.",
        "Section summary": "Section summary",
        "Select a motion from the same meeting to link.": "Select a motion from the same meeting to link.",
        "Select a participant": "Select a participant",
        "Select a relation type and a target decision.": "Select a relation type and a target decision.",
        "Self-evaluation": "Self-evaluation",
        "Send notice": "Send notice",
        "Sent": "Sent",
        "Sent at": "Sent at",
        "Series": "Series",
        "Series error": "Series error",
        "Series generated": "Series generated",
        "Series generation failed.": "Series generation failed.",
        "Set Up Body": "Set Up Body",
        "Settings": "Settings",
        "Settings error": "Settings error",
        "Settings saved successfully": "Settings saved successfully",
        "Settled commitment": "Settled commitment",
        "Share of the governance body present at this meeting, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Share of the governance body present at this meeting, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.",
        "Share of this goal's direct child goals that are achieved, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Share of this goal's direct child goals that are achieved, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.",
        "Share of this goal's linked action items that are completed, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Share of this goal's linked action items that are completed, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.",
        "Share of this goal's linked commitments that are settled, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Share of this goal's linked commitments that are settled, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.",
        "Share of this meeting's action items that are completed, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Share of this meeting's action items that are completed, as a percentage. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.",
        "Shortened debate and voting windows.": "Shortened debate and voting windows.",
        "Show": "Show",
        "Show meeting cost": "Show meeting cost",
        "Show of Hands": "Show of Hands",
        "Show of hands": "Show of hands",
        "Sign": "Sign",
        "Sign now": "Sign now",
        "Signatures": "Signatures",
        "Signed": "Signed",
        "Signers": "Signers",
        "Signing failed.": "Signing failed.",
        "Simple majority (50%+1)": "Simple majority (50%+1)",
        "Simple majority of votes cast, default quorum.": "Simple majority of votes cast, default quorum.",
        "Simulated": "Simulated",
        "Skipped": "Skipped",
        "Sluiten": "Close",
        "Sluitingstijd (optioneel)": "Closing time (optional)",
        "Source app": "Source app",
        "Spanish": "Spanish",
        "Speaker queue": "Speaker queue",
        "Speaking duration": "Speaking duration",
        "Speaking limit (min)": "Speaking limit (min)",
        "Speaking-time distribution": "Speaking-time distribution",
        "Specialized templates": "Specialized templates",
        "Specialized templates apply to specific decision types; the default applies when none is chosen.": "Specialized templates apply to specific decision types; the default applies when none is chosen.",
        "Speeches": "Speeches",
        "Spokesperson": "Spokesperson",
        "Stable short name for the position within its body.": "Stable short name for the position within its body.",
        "Standard decision": "Standard decision",
        "Start": "Start",
        "Start date": "Start date",
        "Start decision-making process": "Start decision-making process",
        "Start deliberation": "Start deliberation",
        "Start evaluation": "Start evaluation",
        "Start process": "Start process",
        "Start taking minutes": "Start taking minutes",
        "Start timer": "Start timer",
        "Started": "Started",
        "Starter overview with sample KPIs and activity placeholders. Replace this view with your own data.": "Starter overview with sample KPIs and activity placeholders. Replace this view with your own data.",
        "State": "State",
        "State machine": "State machine",
        "State name": "State name",
        "States": "States",
        "Status": "Status",
        "Status message": "Status message",
        "Statute amendment": "Statute amendment",
        "Statutory basis": "Statutory basis",
        "Stem tegen": "Vote against",
        "Stem uitbrengen mislukt": "Failed to cast vote",
        "Stem voor": "Vote for",
        "Stemmen tegen": "Votes against",
        "Stemmen voor": "Votes for",
        "Stemmethode": "Voting method",
        "Stemronde": "Voting Round",
        "Stemronde is al geopend — volmacht kan niet meer worden ingetrokken": "Voting round already opened — proxy can no longer be revoked",
        "Stemronde is gesloten": "Voting round is closed",
        "Stemronde is nog niet geopend": "Voting round has not been opened yet",
        "Stemronde openen": "Open voting round",
        "Stemronde openen mislukt": "Failed to open voting round",
        "Stemronde sluiten": "Close voting round",
        "Stemronde sluiten? {notVoted} van {total} leden hebben nog niet gestemd.": "Close voting round? {notVoted} of {total} members have not yet voted.",
        "Step kind": "Step kind",
        "Step {step}": "Step {step}",
        "Step {step} of {total}": "Step {step} of {total}",
        "Still to do: stage {seq} ({maker})": "Still to do: stage {seq} ({maker})",
        "Stop": "Stop",
        "Stop {name}": "Stop {name}",
        "Store": "Store",
        "Sub-item title": "Sub-item title",
        "Sub-item: {title}": "Sub-item: {title}",
        "Sub-items of {title}": "Sub-items of {title}",
        "Subject": "Subject",
        "Subject description": "Subject description",
        "Submission phase": "Submission phase",
        "Submission window (hours)": "Submission window (hours)",
        "Submit Amendment": "Submit Amendment",
        "Submit Motion": "Submit Motion",
        "Submit amendment": "Submit amendment",
        "Submit anonymously": "Submit anonymously",
        "Submit declaration": "Submit declaration",
        "Submit for review": "Submit for review",
        "Submit proposal": "Submit proposal",
        "Submit reaction": "Submit reaction",
        "Submit suggestion": "Submit suggestion",
        "Submitted": "Submitted",
        "Submitted At": "Submitted At",
        "Substitute": "Substitute",
        "Substitute actor": "Substitute actor",
        "Substitute asked at": "Substitute asked at",
        "Substitute searched at": "Substitute searched at",
        "Suggest a correction": "Suggest a correction",
        "Suggest order": "Suggest order",
        "Suggest order, most far-reaching first": "Suggest order, most far-reaching first",
        "Superseded": "Superseded",
        "Superseded by": "Superseded by",
        "Supersedes": "Supersedes",
        "Support": "Support",
        "Support this motion": "Support this motion",
        "Support threshold type": "Support threshold type",
        "Support threshold value": "Support threshold value",
        "Swearing in": "Swearing in",
        "Table": "Table",
        "Talk recording": "Talk recording",
        "Tallying": "Tallying",
        "Target OpenCatalogi catalog": "Target OpenCatalogi catalog",
        "Target agenda item": "Target agenda item",
        "Target decision": "Target decision",
        "Target document": "Target document",
        "Task": "Task",
        "Task group": "Task group",
        "Task status": "Task status",
        "Task title": "Task title",
        "Tasks": "Tasks",
        "Team members": "Team members",
        "Tegen": "Against",
        "Template assignment saved": "Template assignment saved",
        "Template category": "Template category",
        "Term": "Term",
        "Term (months)": "Term (months)",
        "Term End": "Term End",
        "Term Start": "Term Start",
        "Term duration (months)": "Term duration (months)",
        "Term in days": "Term in days",
        "Term number": "Term number",
        "Term passed": "Term passed",
        "Text": "Text",
        "Text changes": "Text changes",
        "That action was refused.": "That action was refused.",
        "That relation already exists.": "That relation already exists.",
        "The CSV file contains no rows.": "The CSV file contains no rows.",
        "The CSV must have a header row with name and email columns.": "The CSV must have a header row with name and email columns.",
        "The Nextcloud group being asked, when the audience is one.": "The Nextcloud group being asked, when the audience is one.",
        "The action failed.": "The action failed.",
        "The admissibility verdict, when the route began with an intake step.": "The admissibility verdict, when the route began with an intake step.",
        "The agenda item this consultation feeds.": "The agenda item this consultation feeds.",
        "The agenda item type this item is an instance of.": "The agenda item type this item is an instance of.",
        "The agenda item where the accounts were adopted.": "The agenda item where the accounts were adopted.",
        "The amendment voting order has been saved.": "The amendment voting order has been saved.",
        "The answer in the respondent's own words.": "The answer in the respondent's own words.",
        "The body putting the question.": "The body putting the question.",
        "The body the membership sits on.": "The body the membership sits on.",
        "The body whose accounts were examined.": "The body whose accounts were examined.",
        "The choices someone may pick from, one per entry. Only used when the field type is a pick from a list; leave it empty for every other field type.": "The choices someone may pick from, one per entry. Only used when the field type is a pick from a list; leave it empty for every other field type.",
        "The commitment this letter settles, when it settles one.": "The commitment this letter settles, when it settles one.",
        "The commitment this planned item follows up, when it follows one.": "The commitment this planned item follows up, when it follows one.",
        "The committee’s conclusion on the accounts for that year.": "The committee’s conclusion on the accounts for that year.",
        "The consultation being answered.": "The consultation being answered.",
        "The consultation of the constituency this request started, when one was started. Optional: a request with none keeps a usable lifecycle and a detail page with an empty reference. This capability defines no poll mechanics of its own; it only points at one.": "The consultation of the constituency this request started, when one was started. Optional: a request with none keeps a usable lifecycle and a detail page with an empty reference. This capability defines no poll mechanics of its own; it only points at one.",
        "The count, or the share between 0 and 1. Ignored when the kind is `all`.": "The count, or the share between 0 and 1. Ignored when the kind is `all`.",
        "The count, or the share between 0 and 1. Ignored when the kind is `all`. COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).": "The count, or the share between 0 and 1. Ignored when the kind is `all`. COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).",
        "The current status of this regulation. The lifecycle moves it from in preparation to adopted, in effect and lapsed.": "The current status of this regulation. The lifecycle moves it from in preparation to adopted, in effect and lapsed.",
        "The decision that adopted this answer.": "The decision that adopted this answer.",
        "The decision that made this appointment.": "The decision that made this appointment.",
        "The decision the answers inform.": "The decision the answers inform.",
        "The decision type a vote on this kind of item produces.": "The decision type a vote on this kind of item produces.",
        "The decision type this decision is an instance of.": "The decision type this decision is an instance of.",
        "The document that constitutes this body: a splitsingsakte, articles of association, statutes or a gemeenschappelijke regeling.": "The document that constitutes this body: a splitsingsakte, articles of association, statutes or a gemeenschappelijke regeling.",
        "The document the answer was given in.": "The document the answer was given in.",
        "The end date must not be before the start date.": "The end date must not be before the start date.",
        "The financial year this statement covers.": "The financial year this statement covers.",
        "The fraction or party being asked, when the audience is one.": "The fraction or party being asked, when the audience is one.",
        "The full official title, where it differs from the short one it is cited by. Optional.": "The full official title, where it differs from the short one it is cited by. Optional.",
        "The governance body this configuration belongs to. One configuration per body.": "The governance body this configuration belongs to. One configuration per body.",
        "The governing document this package publishes.": "The governing document this package publishes.",
        "The gremium competent to take decisions of this type.": "The gremium competent to take decisions of this type.",
        "The gremium it is composed of.": "The gremium it is composed of.",
        "The gremium that is composed.": "The gremium that is composed.",
        "The gremium that may hold meetings of this kind.": "The gremium that may hold meetings of this kind.",
        "The gremium that owns items of this kind. Need not be the body holding the meeting. Empty means the meeting’s own body.": "The gremium that owns items of this kind. Need not be the body holding the meeting. Empty means the meeting’s own body.",
        "The gremium this position exists on.": "The gremium this position exists on.",
        "The ground the verdict rests on. A `niet-ontvankelijk` verdict without one cannot be contested, which is why it is refused (REQ-DWP-004).": "The ground the verdict rests on. A `niet-ontvankelijk` verdict without one cannot be contested, which is why it is refused (REQ-DWP-004).",
        "The hold types valid for this position.": "The hold types valid for this position.",
        "The identifier an external register assigned this document after publication. Stored, never minted here. Optional, and absent until first publication.": "The identifier an external register assigned this document after publication. Stored, never minted here. Optional, and absent until first publication.",
        "The identifier of the retired record this one was copied from. Set only by the fold migration; absent on seeds and on anything a person created.": "The identifier of the retired record this one was copied from. Set only by the fold migration; absent on seeds and on anything a person created.",
        "The legal instrument this body is constituted under (e.g. 'Awb 7:13', a local verordening). Free text on purpose: a closed vocabulary drawn from one country's law cannot express the instruments behind bodies in another, and a caller forced to pick from a list that does not fit will pick a wrong value.": "The legal instrument this body is constituted under (e.g. 'Awb 7:13', a local verordening). Free text on purpose: a closed vocabulary drawn from one country's law cannot express the instruments behind bodies in another, and a caller forced to pick from a list that does not fit will pick a wrong value.",
        "The meeting type this meeting is an instance of.": "The meeting type this meeting is an instance of.",
        "The membership holding the position.": "The membership holding the position.",
        "The minutes are no longer in draft — editing is locked.": "The minutes are no longer in draft — editing is locked.",
        "The minutes return to draft so the secretary can rework them. A comment explaining the rejection is required.": "The minutes return to draft so the secretary can rework them. A comment explaining the rejection is required.",
        "The name this field's value is stored under on every agenda item of this kind. Choose it once and leave it alone: renaming it here does not move the values already recorded, so the old ones become unreachable.": "The name this field's value is stored under on every agenda item of this kind. Choose it once and leave it alone: renaming it here does not move the values already recorded, so the old ones become unreachable.",
        "The number of the version currently in force, so a list renders without resolving every row's versions. Null until a version is in force.": "The number of the version currently in force, so a list renders without resolving every row's versions. Null until a version is in force.",
        "The offboarding checklist. Required. Includes at minimum end-membership, revoke-groups (always mandatory), a personal-data note and the exit confirmation.": "The offboarding checklist. Required. Includes at minimum end-membership, revoke-groups (always mandatory), a personal-data note and the exit confirmation.",
        "The onboarding checklist. Required. Each step carries its own status. The work behind account-linking, assign-groups and induction-pack happens elsewhere; this array records that the step exists and where it stands.": "The onboarding checklist. Required. Each step carries its own status. The work behind account-linking, assign-groups and induction-pack happens elsewhere; this array records that the step exists and where it stands.",
        "The options the respondent picked, when the answer was a choice.": "The options the respondent picked, when the answer was a choice.",
        "The options to pick from, when the answer is a choice.": "The options to pick from, when the answer is a choice.",
        "The ordered steps, with the rule, silence and substitute additions. Every addition defaults to today's behaviour, so no stored route changes meaning.": "The ordered steps, with the rule, silence and substitute additions. Every addition defaults to today's behaviour, so no stored route changes meaning.",
        "The ordered steps, with the walked-process additions. Every addition defaults to today's behaviour, so no stored route changes meaning.": "The ordered steps, with the walked-process additions. Every addition defaults to today's behaviour, so no stored route changes meaning.",
        "The organisation that answered, when the audience was organisations.": "The organisation that answered, when the audience was organisations.",
        "The person the rule is read against, for the rules that need one. `manager-of-subject-owner` reads the subject's own owner instead and needs none.": "The person the rule is read against, for the rules that need one. `manager-of-subject-owner` reads the subject's own owner instead and needs none.",
        "The person who answered, when the answer is attributed to one.": "The person who answered, when the answer is attributed to one.",
        "The planning-cycle step this consultation belongs to.": "The planning-cycle step this consultation belongs to.",
        "The position being held.": "The position being held.",
        "The position on the composite that a representative occupies.": "The position on the composite that a representative occupies.",
        "The producing app's own id for the route this one was migrated from. Paired with sourceApp it identifies the origin, so a repeated command updates this route rather than creating another.": "The producing app's own id for the route this one was migrated from. Paired with sourceApp it identifies the origin, so a repeated command updates this route rather than creating another.",
        "The property paths on the subject that this step's approvers are actually approving. When one of them changes after a grant, the grant is withdrawn and the step reopens, because an approval of a document that has since been rewritten is not an approval of the document in front of you. An EMPTY basis never withdraws anything, which is what keeps this opt-in (REQ-DWP-003).": "The property paths on the subject that this step's approvers are actually approving. When one of them changes after a grant, the grant is withdrawn and the step reopens, because an approval of a document that has since been rewritten is not an approval of the document in front of you. An EMPTY basis never withdraws anything, which is what keeps this opt-in (REQ-DWP-003).",
        "The property paths on the subject that this step's approvers are actually approving. When one of them changes after a grant, the grant is withdrawn and the step reopens, because an approval of a document that has since been rewritten is not an approval of the document in front of you. An EMPTY basis never withdraws anything, which is what keeps this opt-in (REQ-DWP-003). COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).": "The property paths on the subject that this step's approvers are actually approving. When one of them changes after a grant, the grant is withdrawn and the step reopens, because an approval of a document that has since been rewritten is not an approval of the document in front of you. An EMPTY basis never withdraws anything, which is what keeps this opt-in (REQ-DWP-003). COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).",
        "The published predicate could not be set on this OpenRegister version — anonymous read is not yet available.": "The published predicate could not be set on this OpenRegister version — anonymous read is not yet available.",
        "The reaction is retained for audit but never counts toward the consultation. A reason is required.": "The reaction is retained for audit but never counts toward the consultation. A reason is required.",
        "The referenced motion ({id}) could not be loaded.": "The referenced motion ({id}) could not be loaded.",
        "The remedies open against a decision of this type. A type published with none declared is refused: a decision that tells nobody how to contest it is the one failure this declaration exists to prevent. `geen` is a legitimate declaration and is how a type says so on purpose.": "The remedies open against a decision of this type. A type published with none declared is refused: a decision that tells nobody how to contest it is the one failure this declaration exists to prevent. `geen` is a legitimate declaration and is how a type says so on purpose.",
        "The remedy clause as it stood when this decision was taken, resolved from the decision type and STAMPED here. Resolved at decision time rather than read live, because a type whose term changes next year must not silently change what this decision told someone last year (REQ-DWP-007).": "The remedy clause as it stood when this decision was taken, resolved from the decision type and STAMPED here. Resolved at decision time rather than read live, because a type whose term changes next year must not silently change what this decision told someone last year (REQ-DWP-007).",
        "The requested board could not be loaded.": "The requested board could not be loaded.",
        "The requested board meeting could not be loaded.": "The requested board meeting could not be loaded.",
        "The requested resolution could not be loaded.": "The requested resolution could not be loaded.",
        "The required majority for this category per the splitsingsakte (mirrors the VotingRound voteThreshold enum). Resolves into the existing round-open defaults; never a second threshold enum.": "The required majority for this category per the splitsingsakte (mirrors the VotingRound voteThreshold enum). Resolves into the existing round-open defaults; never a second threshold enum.",
        "The sentence printed on the decision.": "The sentence printed on the decision.",
        "The sentence the reader sees.": "The sentence the reader sees.",
        "The series is capped at 52 instances.": "The series is capped at 52 instances.",
        "The server rejected this relation.": "The server rejected this relation.",
        "The sign-off route could not be read.": "The sign-off route could not be read.",
        "The single body being asked, when only one is.": "The single body being asked, when only one is.",
        "The state a new meeting of this kind opens in.": "The state a new meeting of this kind opens in.",
        "The state an item of this kind opens in.": "The state an item of this kind opens in.",
        "The statutory notice deadline ({deadline}) has already passed.": "The statutory notice deadline ({deadline}) has already passed.",
        "The statutory notice deadline ({deadline}) is {n} day(s) away.": "The statutory notice deadline ({deadline}) is {n} day(s) away.",
        "The summarised outcome, once the answers are in.": "The summarised outcome, once the answers are in.",
        "The support threshold value.": "The support threshold value.",
        "The template category this override applies to.": "The template category this override applies to.",
        "The term on this sign-off passed without an answer, and the step declares that silence escalates, so it is now with your manager.": "The term on this sign-off passed without an answer, and the step declares that silence escalates, so it is now with your manager.",
        "The term on this sign-off passed without an answer. The step declares that silence refuses, so the route has been concluded as rejected.": "The term on this sign-off passed without an answer. The step declares that silence refuses, so the route has been concluded as rejected.",
        "The term passed without an answer. Silence approves here, so the system recorded it as approved and the next step is live.": "The term passed without an answer. Silence approves here, so the system recorded it as approved and the next step is live.",
        "The territorial or subject-matter competence of this body.": "The territorial or subject-matter competence of this body.",
        "The total this body’s weighted votes are expressed over. Leave empty when every member has one equal vote.": "The total this body’s weighted votes are expressed over. Leave empty when every member has one equal vote.",
        "The version of the model regulation this body follows, as plain text.": "The version of the model regulation this body follows, as plain text.",
        "The view given.": "The view given.",
        "The vote is tied. As chair you must resolve it with a casting vote.": "The vote is tied. As chair you must resolve it with a casting vote.",
        "The vote is tied. The round may be reopened once for a revote.": "The vote is tied. The round may be reopened once for a revote.",
        "Theme session": "Theme session",
        "There is no text to compare yet.": "There is no text to compare yet.",
        "This amendment has no proposed replacement text; the amendment text itself is compared against the motion text.": "This amendment has no proposed replacement text; the amendment text itself is compared against the motion text.",
        "This amendment is not linked to a motion, so there is no original text to compare against.": "This amendment is not linked to a motion, so there is no original text to compare against.",
        "This amendment is not linked to a motion.": "This amendment is not linked to a motion.",
        "This app needs OpenRegister to store and manage data. Please install OpenRegister from the app store to get started.": "This app needs OpenRegister to store and manage data. Please install OpenRegister from the app store to get started.",
        "This decision has been enacted and the governance body is configured to prompt for publication. Publishing makes a derived public record available through OpenCatalogi. You can also publish later from the Publication tab.": "This decision has been enacted and the governance body is configured to prompt for publication. Publishing makes a derived public record available through OpenCatalogi. You can also publish later from the Publication tab.",
        "This decision has no staged route. A stageless decision is valid.": "This decision has no staged route. A stageless decision is valid.",
        "This decision has no typed links to other decisions yet.": "This decision has no typed links to other decisions yet.",
        "This decision was repealed{by}.": "This decision was repealed{by}.",
        "This decision was superseded{by}.": "This decision was superseded{by}.",
        "This delegation/mandate has no parent or sub-mandates.": "This delegation/mandate has no parent or sub-mandates.",
        "This draft was generated by AI from the transcript. Review every section before it enters the minutes. AI never approves or publishes minutes — the normal approval workflow is unchanged.": "This draft was generated by AI from the transcript. Review every section before it enters the minutes. AI never approves or publishes minutes — the normal approval workflow is unchanged.",
        "This general assembly agenda is missing legally required items:": "This general assembly agenda is missing legally required items:",
        "This motion has no amendments to order.": "This motion has no amendments to order.",
        "This page is backed by the pluggable integration registry. The \"Articles\" tab is provided by the xWiki integration via OpenConnector — linked wiki pages render with their breadcrumb and a text preview.": "This page is backed by the pluggable integration registry. The \"Articles\" tab is provided by the xWiki integration via OpenConnector — linked wiki pages render with their breadcrumb and a text preview.",
        "This page is backed by the pluggable integration registry. The Discussion tab is provided by the Talk integration leaf — messages posted there are linked to this motion object and visible to all participants.": "This page is backed by the pluggable integration registry. The Discussion tab is provided by the Talk integration leaf — messages posted there are linked to this motion object and visible to all participants.",
        "This page is backed by the pluggable integration registry. When the Email integration is installed, an \"Email\" tab lets you link emails to this agenda item — the link is held by the registry, not an in-app email-link store.": "This page is backed by the pluggable integration registry. When the Email integration is installed, an \"Email\" tab lets you link emails to this agenda item — the link is held by the registry, not an in-app email-link store.",
        "This page is backed by the pluggable integration registry. When the Email integration is installed, an \"Email\" tab lets you link emails to this decision dossier — the link is held by the registry, not an in-app email-link store.": "This page is backed by the pluggable integration registry. When the Email integration is installed, an \"Email\" tab lets you link emails to this decision dossier — the link is held by the registry, not an in-app email-link store.",
        "This pattern creates {n} meeting(s).": "This pattern creates {n} meeting(s).",
        "This respondent's own deadline, when it differs from the consultation's.": "This respondent's own deadline, when it differs from the consultation's.",
        "This round is the single permitted revote of the tied round.": "This round is the single permitted revote of the tied round.",
        "This sign-off is part-way through its term, so your substitute has been asked too. You can still sign it yourself.": "This sign-off is part-way through its term, so your substitute has been asked too. You can still sign it yourself.",
        "This step has nobody assigned yet.": "This step has nobody assigned yet.",
        "This will set all {n} consent agenda items to \"Adopted\" (afgerond). Continue?": "This will set all {n} consent agenda items to \"Adopted\" (afgerond). Continue?",
        "Threshold": "Threshold",
        "Threshold a vote must clear by default.": "Threshold a vote must clear by default.",
        "Threshold after": "Threshold after",
        "Threshold before": "Threshold before",
        "Threshold kind": "Threshold kind",
        "Threshold value": "Threshold value",
        "Tick this to make the field compulsory: an agenda item of this kind cannot be saved while it is left empty.": "Tick this to make the field compulsory: an agenda item of this kind cannot be saved while it is left empty.",
        "Tie resolved by the chair's casting vote: {value}": "Tie resolved by the chair's casting vote: {value}",
        "Tie-break rule": "Tie-break rule",
        "Tie: chair decides": "Tie: chair decides",
        "Tie: motion fails": "Tie: motion fails",
        "Tie: revote (once)": "Tie: revote (once)",
        "Time allocation accuracy": "Time allocation accuracy",
        "Time remaining for {title}": "Time remaining for {title}",
        "Timezone": "Timezone",
        "Title": "Title",
        "To": "To",
        "Today": "Today",
        "Topics suggested": "Topics suggested",
        "Total duration: {min} min": "Total duration: {min} min",
        "Total votes cast: {n}": "Total votes cast: {n}",
        "Total: {total} · Average per meeting: {average}": "Total: {total} · Average per meeting: {average}",
        "Transcribe": "Transcribe",
        "Transcription": "Transcription",
        "Transcription error": "Transcription error",
        "Transcription unavailable": "Transcription unavailable",
        "Transferred": "Transferred",
        "Transitie mislukt": "Transition failed",
        "Transition failed": "Transition failed",
        "Transition failed.": "Transition failed.",
        "Transition rejected": "Transition rejected",
        "Transitions": "Transitions",
        "Treasurer": "Treasurer",
        "Type": "Type",
        "Type-specific field values": "Type-specific field values",
        "Type-specific fields": "Type-specific fields",
        "Type: {type}": "Type: {type}",
        "U stemt namens: {name}": "You are voting on behalf of: {name}",
        "Uitgebracht: {cast} / {total}": "Cast: {cast} / {total}",
        "Uitslag:": "Result:",
        "Unanimous": "Unanimous",
        "Unassigned": "Unassigned",
        "Undecided": "Undecided",
        "Under discussion": "Under discussion",
        "Unknown role": "Unknown role",
        "Unsigned": "Unsigned",
        "Until": "Until",
        "Until (inclusive)": "Until (inclusive)",
        "Untitled": "Untitled",
        "Untitled decision": "Untitled decision",
        "Upcoming meetings": "Upcoming meetings",
        "Upload a CSV file with the columns: name, email, role.": "Upload a CSV file with the columns: name, email, role.",
        "Urgent": "Urgent",
        "Urgent decision": "Urgent decision",
        "User settings will appear here in a future update.": "User settings will appear here in a future update.",
        "Uuid of the OpenRegister task currently asking for this stage's sign-off. Set while the stage is active and a task surface exists; cleared when the stage stops waiting. Empty on an instance without the task surface, which changes where the ask is seen and nothing else.": "Uuid of the OpenRegister task currently asking for this stage's sign-off. Set while the stage is active and a task surface exists; cleared when the stage stops waiting. Empty on an instance without the task surface, which changes where the ask is seen and nothing else.",
        "Uw stem is geregistreerd.": "Your vote has been registered.",
        "Valid from": "Valid from",
        "Valid until": "Valid until",
        "Values for the fields declared by this item’s type.": "Values for the fields declared by this item’s type.",
        "Verantwoordelijke organisatie": "Verantwoordelijke organisatie",
        "Vergadering niet gekoppeld — stemronde kan niet worden geopend": "Meeting not linked — voting round cannot be opened",
        "Verlenen": "Grant",
        "Version": "Version",
        "Version Information": "Version Information",
        "Version history": "Version history",
        "Version timeline": "Version timeline",
        "Version {n}": "Version {n}",
        "Verworpen": "Rejected",
        "Verzenddatum": "Verzenddatum",
        "Vice-chair": "Vice-chair",
        "View agenda item": "View agenda item",
        "View decision": "View decision",
        "View motion": "View motion",
        "Volmacht intrekken": "Revoke proxy",
        "Volmacht verlenen": "Grant proxy",
        "Volmacht verlenen aan": "Grant proxy to",
        "Voor": "For",
        "Voor / Tegen / Onthouding": "For / Against / Abstain",
        "Voor: {for} — Tegen: {against} — Onthouding: {abstain}": "For: {for} — Against: {against} — Abstain: {abstain}",
        "Votable": "Votable",
        "Vote": "Vote",
        "Vote against": "Vote against",
        "Vote for": "Vote for",
        "Vote now": "Vote now",
        "Vote tally": "Vote tally",
        "Vote threshold": "Vote threshold",
        "Vote type": "Vote type",
        "Vote weight denominator": "Vote weight denominator",
        "Voter": "Voter",
        "Votes": "Votes",
        "Votes abstain": "Votes abstain",
        "Votes against": "Votes against",
        "Votes cast": "Votes cast",
        "Votes for": "Votes for",
        "Voting": "Voting",
        "Voting Default": "Voting Default",
        "Voting Method": "Voting Method",
        "Voting Round": "Voting Round",
        "Voting Rounds": "Voting Rounds",
        "Voting Weight": "Voting Weight",
        "Voting method": "Voting method",
        "Voting method this kind of meeting defaults to.": "Voting method this kind of meeting defaults to.",
        "Voting opened on \"%1$s\"": "Voting opened on \"%1$s\"",
        "Voting opened on {object}": "Voting opened on {object}",
        "Voting order": "Voting order",
        "Voting phase": "Voting phase",
        "Voting results": "Voting results",
        "Voting round": "Voting round",
        "Voting weight": "Voting weight",
        "Waiting": "Waiting",
        "Waiting on {actor}": "Waiting on {actor}",
        "Weight a vote cast by the holder of this position carries.": "Weight a vote cast by the holder of this position carries.",
        "Weight the component’s representation carries in votes.": "Weight the component’s representation carries in votes.",
        "Weighted": "Weighted",
        "Weighted vote": "Weighted vote",
        "Welcome to Decidiq!": "Welcome to Decidiq!",
        "Welcome to Decidiq! Get started by setting up your first governing body in Settings.": "Welcome to Decidiq! Get started by setting up your first governing body in Settings.",
        "Welcome to Decidiq! Get started by setting up your first governing body.": "Welcome to Decidiq! Get started by setting up your first governing body.",
        "What a user picks when adding an agenda item.": "What a user picks when adding an agenda item.",
        "What a user picks when creating a meeting of this kind.": "What a user picks when creating a meeting of this kind.",
        "What exactly the parties are being asked. Optional when the subject says it.": "What exactly the parties are being asked. Optional when the subject says it.",
        "What is being asked about, in one line.": "What is being asked about, in one line.",
        "What is being consulted on, at more length than the subject line allows.": "What is being consulted on, at more length than the subject line allows.",
        "What items of this kind carry and how they are handled.": "What items of this kind carry and how they are handled.",
        "What kind of answer is expected.": "What kind of answer is expected.",
        "What kind of answer the field takes, which decides how it is shown and what is accepted: a single line of text, a longer text, a number, a yes/no tick, a date, a pick from the list of choices below, or a link to another record.": "What kind of answer the field takes, which decides how it is shown and what is accepted: a single line of text, a longer text, a number, a yes/no tick, a date, a pick from the list of choices below, or a link to another record.",
        "What kind of governing document this is.": "What kind of governing document this is.",
        "What kind of party owns the item, in whatever terms the organisation uses.": "What kind of party owns the item, in whatever terms the organisation uses.",
        "What kind of step this is beyond its stage type. `intake` is the admissibility test: it yields a verdict rather than an approval, and a `niet-ontvankelijk` verdict ends the route where it stands (REQ-DWP-004).": "What kind of step this is beyond its stage type. `intake` is the admissibility test: it yields a verdict rather than an approval, and a `niet-ontvankelijk` verdict ends the route where it stands (REQ-DWP-004).",
        "What kind of step this is beyond its stage type. `intake` is the admissibility test: it yields a verdict rather than an approval, and a `niet-ontvankelijk` verdict ends the route where it stands (REQ-DWP-004). COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).": "What kind of step this is beyond its stage type. `intake` is the admissibility test: it yields a verdict rather than an approval, and a `niet-ontvankelijk` verdict ends the route where it stands (REQ-DWP-004). COPIED onto the stage at instantiation, not read back off the route: editing a route must never change what a stage already in flight means, which is the same rule actorRule and onSilence follow (REQ-AR-012).",
        "What kind of step this is. `custom` is a step the organisation defined itself.": "What kind of step this is. `custom` is a step the organisation defined itself.",
        "What kind of step this is. `personal-data-note` points at the author export rather than repeating it; `custom` is a step the organisation defined itself.": "What kind of step this is. `personal-data-note` points at the author export rather than repeating it; `custom` is a step the organisation defined itself.",
        "What kind of thing is being consulted on. Free vocabulary: an organisation uses its own words.": "What kind of thing is being consulted on. Free vocabulary: an organisation uses its own words.",
        "What kind of thing the body expects, in whatever terms it uses itself.": "What kind of thing the body expects, in whatever terms it uses itself.",
        "What meetings of this kind are for.": "What meetings of this kind are for.",
        "What set this off: one person joining, a whole intake at once, or someone stepping in mid-term. Free text, because what an organisation calls its intake is its own vocabulary.": "What set this off: one person joining, a whole intake at once, or someone stepping in mid-term. Free text, because what an organisation calls its intake is its own vocabulary.",
        "What set this off: one person leaving, or a whole turnover at once. Free text, because what an organisation calls its turnover is its own vocabulary.": "What set this off: one person leaving, or a whole turnover at once. Free text, because what an organisation calls its turnover is its own vocabulary.",
        "What the asking body did with the answers, and why.": "What the asking body did with the answers, and why.",
        "What the asking body did with this answer.": "What the asking body did with this answer.",
        "What the field is called on screen: the caption above the input when someone fills in an agenda item, and the column header where the field is listed.": "What the field is called on screen: the caption above the input when someone fills in an agenda item, and the column header where the field is listed.",
        "What the position is responsible for.": "What the position is responsible for.",
        "What this document derives its authority from. A statute for a public body, the articles of association for a company rule, the deed of division for a VvE regulation.": "What this document derives its authority from. A statute for a public body, the articles of association for a company rule, the deed of division for a VvE regulation.",
        "What this step's silence means once its due date has passed. `hold` is the default and is what every stored route means today: nothing happens and the stage stays active. `approve` is a signature nobody gave, so only an administrator may set it and a route carrying it says so on its own surface (REQ-AR-015).": "What this step's silence means once its due date has passed. `hold` is the default and is what every stored route means today: nothing happens and the stage stays active. `approve` is a signature nobody gave, so only an administrator may set it and a route carrying it says so on its own surface (REQ-AR-015).",
        "What was discussed…": "What was discussed…",
        "When": "When",
        "When a lapse policy was applied to this stage. Written once, and the sweep skips a stage that already carries it, which is what makes running the sweep twice harmless (REQ-AR-017).": "When a lapse policy was applied to this stage. Written once, and the sweep skips a stage that already carries it, which is what makes running the sweep twice harmless (REQ-AR-017).",
        "When a substitute was looked for and none was found. A NOTE on the stage rather than a refusal: no substitute is a normal state of affairs and must not stop the original actor from signing (REQ-AR-016).": "When a substitute was looked for and none was found. A NOTE on the stage rather than a refusal: no substitute is a normal state of affairs and must not stop the original actor from signing (REQ-AR-016).",
        "When answering opens. Empty means it is open already.": "When answering opens. Empty means it is open already.",
        "When it is due. Shown on the assignee's task list beside the subject and the step.": "When it is due. Shown on the assignee's task list beside the subject and the step.",
        "When it was withdrawn.": "When it was withdrawn.",
        "When the answer was given.": "When the answer was given.",
        "When the asking body accounted for the answers.": "When the asking body accounted for the answers.",
        "When the component joined the composite.": "When the component joined the composite.",
        "When the component left. Empty means it is still composing.": "When the component left. Empty means it is still composing.",
        "When the exit was confirmed. Optional until confirmed.": "When the exit was confirmed. Optional until confirmed.",
        "When the rule was resolved.": "When the rule was resolved.",
        "When the substitute was also asked.": "When the substitute was also asked.",
        "When this answer becomes public.": "When this answer becomes public.",
        "When this answer stops being public.": "When this answer stops being public.",
        "When this consultation becomes public.": "When this consultation becomes public.",
        "When this consultation stops being public.": "When this consultation stops being public.",
        "When this hold began.": "When this hold began.",
        "When this hold ends. Empty means open-ended.": "When this hold ends. Empty means open-ended.",
        "When this stage became live. The start of its window, which is what the substitute ask point is a fraction of (REQ-AR-016).": "When this stage became live. The start of its window, which is what the substitute ask point is a fraction of (REQ-AR-016).",
        "When this stage is due. REQ-AR-009 owns the arithmetic that splits one route deadline over the stages; this declares the field the lapse sweep reads, because silence can only be measured against a deadline. A stage with no due date never lapses, whatever its onSilence says (REQ-AR-015).": "When this stage is due. REQ-AR-009 owns the arithmetic that splits one route deadline over the stages; this declares the field the lapse sweep reads, because silence can only be measured against a deadline. A stage with no due date never lapses, whatever its onSilence says (REQ-AR-015).",
        "When this step is due. One deadline for the whole route is divided over its steps in working days, the last step landing exactly on the deadline, because the deadline is the promise made to whoever asked for the review. A step past this date with nothing recorded renders as overdue; that is a computed view and not a stored state, so nothing has to unset it when the step is signed (REQ-AR-009).": "When this step is due. One deadline for the whole route is divided over its steps in working days, the last step landing exactly on the deadline, because the deadline is the promise made to whoever asked for the review. A step past this date with nothing recorded renders as overdue; that is a computed view and not a stored state, so nothing has to unset it when the step is signed (REQ-AR-009).",
        "When.": "When.",
        "Where Decidiq sends governance communications such as convocations, minutes and reminders.": "Where Decidiq sends governance communications such as convocations, minutes and reminders.",
        "Where it is lodged.": "Where it is lodged.",
        "Where the action stands. `open` is the work item; the other three are its ends.": "Where the action stands. `open` is the work item; the other three are its ends.",
        "Where the automation lives": "Where the automation lives",
        "Where the consultation is in its course.": "Where the consultation is in its course.",
        "Where the offboarding stands. Declared in x-openregister-lifecycle (ADR-031): started, in-execution, then completed or lapsed. It cannot reach completed while the always-mandatory revoke-groups step is not completed.": "Where the offboarding stands. Declared in x-openregister-lifecycle (ADR-031): started, in-execution, then completed or lapsed. It cannot reach completed while the always-mandatory revoke-groups step is not completed.",
        "Where the onboarding stands. Declared in x-openregister-lifecycle (ADR-031): started, in-execution, then completed or lapsed.": "Where the onboarding stands. Declared in x-openregister-lifecycle (ADR-031): started, in-execution, then completed or lapsed.",
        "Where the route came from. `adhoc` means it was held from a list of named people with no stored ApprovalRoute behind it, which is the everyday review: a clerk, a document and three colleagues. Without this a surface that finds no route row cannot tell an ad-hoc route from a template somebody deleted under a route still in flight (REQ-AR-008).": "Where the route came from. `adhoc` means it was held from a list of named people with no stored ApprovalRoute behind it, which is the everyday review: a clerk, a document and three colleagues. Without this a surface that finds no route row cannot tell an ad-hoc route from a template somebody deleted under a route still in flight (REQ-AR-008).",
        "Where the step stands. A step may only be `skipped` when mandatory is false, enforced at save time.": "Where the step stands. A step may only be `skipped` when mandatory is false, enforced at save time.",
        "Where the step stands. A step may only be `skipped` when mandatory is false, enforced at save time. The revoke-groups step is always mandatory and can never be skipped.": "Where the step stands. A step may only be `skipped` when mandatory is false, enforced at save time. The revoke-groups step is always mandatory and can never be skipped.",
        "Where this answer is in its course.": "Where this answer is in its course.",
        "Where this document is in its lifecycle.": "Where this document is in its lifecycle.",
        "Whether a holder may be reappointed at the end of a term.": "Whether a holder may be reappointed at the end of a term.",
        "Whether an item of this kind can be put to a vote.": "Whether an item of this kind can be put to a vote.",
        "Whether answers are recorded without naming who gave them.": "Whether answers are recorded without naming who gave them.",
        "Whether every stage of this decision's route is decided or skipped. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq and never edited by hand.": "Whether every stage of this decision's route is decided or skipped. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq and never edited by hand.",
        "Whether meetings of this kind are public by default.": "Whether meetings of this kind are public by default.",
        "Whether the answers bind the asking body, or only inform it.": "Whether the answers bind the asking body, or only inform it.",
        "Whether the component’s members join directly, or send representatives.": "Whether the component’s members join directly, or send representatives.",
        "Whether the decision has been withdrawn. The original outcome is NOT cleared.": "Whether the decision has been withdrawn. The original outcome is NOT cleared.",
        "Whether the meeting met its quorum, true when no quorum is required. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.": "Whether the meeting met its quorum, true when no quorum is required. Materialised by OpenRegister on save from x-openregister-calculations. Never written by decidiq.",
        "Whether the position currently exists on the body.": "Whether the position currently exists on the body.",
        "Whether the step is mandatory. A mandatory step blocks the offboarding from reaching `completed` until it is `completed` itself; revoke-groups is always mandatory.": "Whether the step is mandatory. A mandatory step blocks the offboarding from reaching `completed` until it is `completed` itself; revoke-groups is always mandatory.",
        "Whether the step is mandatory. A mandatory step blocks the onboarding from reaching `completed` until it is `completed` itself.": "Whether the step is mandatory. A mandatory step blocks the onboarding from reaching `completed` until it is `completed` itself.",
        "Whether the support threshold is a fraction of the body or an absolute count.": "Whether the support threshold is a fraction of the body or an absolute count.",
        "Whether this body may be assigned new work. Independent of termStart/termEnd: a body can sit inside its term and be suspended, or be past its term and still finishing open cases, so availability cannot be derived from the dates. Unrelated to the schema-level x-openregister.active flag, which says nothing about an individual body.": "Whether this body may be assigned new work. Independent of termStart/termEnd: a body can sit inside its term and be suspended, or be past its term and still finishing open cases, so availability cannot be derived from the dates. Unrelated to the schema-level x-openregister.active flag, which says nothing about an individual body.",
        "Whether this holder may be reappointed.": "Whether this holder may be reappointed.",
        "Whether this meeting still relies on the legacy meeting type list.": "Whether this meeting still relies on the legacy meeting type list.",
        "Whether this member sits from OUTSIDE the administrative organ: i.e. is not employed by it. Awb 7:13(2) requires this of the chair of an objection advisory committee. NOT the same question as `independenceStatus`, which encodes corporate-governance independence and cannot carry an employment relationship.": "Whether this member sits from OUTSIDE the administrative organ: i.e. is not employed by it. Awb 7:13(2) requires this of the chair of an objection advisory committee. NOT the same question as `independenceStatus`, which encodes corporate-governance independence and cannot carry an employment relationship.",
        "Whether this position seats a representative delegated by a composing body.": "Whether this position seats a representative delegated by a composing body.",
        "Whether this route has to conclude before its subject counts as cleared. Defaults to true, because a route somebody bothered to start is a route somebody is waiting on, and the safe default for a clearance question is the one that blocks (REQ-AR-014).": "Whether this route has to conclude before its subject counts as cleared. Defaults to true, because a route somebody bothered to start is a route somebody is waiting on, and the safe default for a clearance question is the one that blocks (REQ-AR-014).",
        "Whether this stage must be taken. COPIED from the ApprovalRoute step at instantiation, deliberately: reading it from the route at decision time would give a different answer whenever the template changed after a subject started travelling it.": "Whether this stage must be taken. COPIED from the ApprovalRoute step at instantiation, deliberately: reading it from the route at decision time would give a different answer whenever the template changed after a subject started travelling it.",
        "Whether this type is offered when creating a meeting.": "Whether this type is offered when creating a meeting.",
        "Whether this type is offered.": "Whether this type is offered.",
        "Which consecutive term in this position this hold is.": "Which consecutive term in this position this hold is.",
        "Which declared meaning of silence produced this action. An undeclared property is DROPPED by the importer without an error, so a lapse row would otherwise be unable to say why it happened (REQ-AR-017).": "Which declared meaning of silence produced this action. An undeclared property is DROPPED by the importer without an error, so a lapse row would otherwise be unable to say why it happened (REQ-AR-017).",
        "Which remedy is open.": "Which remedy is open.",
        "Which rule produced this stage's actor, when a rule did. Kept so the stage can answer why this person was asked, which a resolved name on its own cannot (REQ-AR-012).": "Which rule produced this stage's actor, when a rule did. Kept so the stage can answer why this person was asked, which a resolved name on its own cannot (REQ-AR-012).",
        "Who gave the verdict.": "Who gave the verdict.",
        "Who holds it": "Who holds it",
        "Who the rule is read against, copied from the step.": "Who the rule is read against, copied from the step.",
        "Who the substitute is. Both they and the original actor may act, and whichever acts first closes the stage.": "Who the substitute is. Both they and the original actor may act, and whichever acts first closes the stage.",
        "Who this approval is waiting on. An open action with no assignee is an approval nobody has been asked for, which is how a route sits still for a week with nothing to show for it.": "Who this approval is waiting on. An open action with no assignee is an approval nobody has been asked for, which is how a route sits still for a week with nothing to show for it.",
        "Who withdrew it. A withdrawal by the deciding body and a withdrawal by the party who asked are different acts with different consequences, and a record that does not say which is not a record (REQ-DWP-005).": "Who withdrew it. A withdrawal by the deciding body and a withdrawal by the party who asked are different acts with different consequences, and a record that does not say which is not a record (REQ-DWP-005).",
        "Why a granted approval is no longer good. `basis-changed` is the system's own: what was approved was edited afterwards. `by-actor` is the person taking it back. A history that cannot tell the two apart cannot answer who is responsible for the reopened step.": "Why a granted approval is no longer good. `basis-changed` is the system's own: what was approved was edited afterwards. `by-actor` is the person taking it back. A history that cannot tell the two apart cannot answer who is responsible for the reopened step.",
        "Why the asking body handled the answer that way.": "Why the asking body handled the answer that way.",
        "Why the hold ended.": "Why the hold ended.",
        "Why the membership ends. Free text: end of term, resignation, relocation, death, or whatever else an organisation records.": "Why the membership ends. Free text: end of term, resignation, relocation, death, or whatever else an organisation records.",
        "Why. Read by the person who receives the withdrawal, so it is written for them.": "Why. Read by the person who receives the withdrawal, so it is written for them.",
        "Widened with `lapsed`, the verb for a step that moved because its deadline passed rather than because somebody acted. Without it the row fails validation and the lapse leaves no trace (REQ-AR-017).": "Widened with `lapsed`, the verb for a step that moved because its deadline passed rather than because somebody acted. Without it the row fails validation and the lapse leaves no trace (REQ-AR-017).",
        "Widened with `system`. A lapse is recorded as an action by the system and never under the silent actor's name, because an action carrying their name reads exactly like one they took. Fragments union list values, so this ADDS to the enum rather than replacing it (REQ-AR-017).": "Widened with `system`. A lapse is recorded as an action by the system and never under the silent actor's name, because an action carrying their name reads exactly like one they took. Fragments union list values, so this ADDS to the enum rather than replacing it (REQ-AR-017).",
        "Will be imported": "Will be imported",
        "Wire buttons here to create records, open lists, or deep links. Use the sidebar for Settings and Documentation.": "Wire buttons here to create records, open lists, or deep links. Use the sidebar for Settings and Documentation.",
        "Withdraw": "Withdraw",
        "Withdraw Motion": "Withdraw Motion",
        "Withdraw publication": "Withdraw publication",
        "Withdraw reason": "Withdraw reason",
        "Withdrawing removes the published record from the public surface and retracts the OpenCatalogi publication. A reason is required and recorded in the audit trail (WOO correction duty).": "Withdrawing removes the published record from the public surface and retracts the OpenCatalogi publication. A reason is required and recorded in the audit trail (WOO correction duty).",
        "Withdrawn": "Withdrawn",
        "Withdrawn reason": "Withdrawn reason",
        "Withdraw…": "Withdraw…",
        "Woo absolute": "Woo absolute",
        "Woo decision": "Woo decision",
        "Woo relative": "Woo relative",
        "Works council": "Works council",
        "Workspace": "Workspace",
        "Workspace name": "Workspace name",
        "Workspace type": "Workspace type",
        "Workspaces": "Workspaces",
        "Yes": "Yes",
        "You are all caught up": "You are all caught up",
        "You are voting on behalf of": "You are voting on behalf of",
        "You are voting on behalf of: {name}": "You are voting on behalf of: {name}",
        "You have been asked to stand in on a sign-off that is part-way through its term. Whoever acts first closes it.": "You have been asked to stand in on a sign-off that is part-way through its term. Whoever acts first closes it.",
        "Your Nextcloud account email": "Your Nextcloud account email",
        "Your next meeting": "Your next meeting",
        "Your reaction": "Your reaction",
        "Your reaction was submitted for moderation": "Your reaction was submitted for moderation",
        "Your response is anonymous. It cannot be traced back to you, even by an administrator.": "Your response is anonymous. It cannot be traced back to you, even by an administrator.",
        "Your sign-off has moved up": "Your sign-off has moved up",
        "Your stand-in has been asked as well": "Your stand-in has been asked as well",
        "Your vote has been recorded": "Your vote has been recorded",
        "Your vote has been recorded.": "Your vote has been recorded.",
        "Your vote was recorded": "Your vote was recorded",
        "Zoek op motietitel…": "Search by motion title…",
        "actions": "actions",
        "active": "active",
        "adopted": "adopted",
        "advice": "advice",
        "advised": "advised",
        "advisory": "advisory",
        "against": "against",
        "another app": "another app",
        "avg {actual} min actual vs {estimated} min allocated": "avg {actual} min actual vs {estimated} min allocated",
        "chair only": "chair only",
        "chair register": "chair register",
        "concept": "concept",
        "consultationType=tender: public tender reference/notice number.": "consultationType=tender: public tender reference/notice number.",
        "consultationType=tender: the winning party recorded when the award decision is made (status=awarded). Decidiq owns publish/manage/award only; authoring is procest and bidding is pipelinq.": "consultationType=tender: the winning party recorded when the award decision is made (status=awarded). Decidiq owns publish/manage/award only; authoring is procest and bidding is pipelinq.",
        "decided": "decided",
        "decisions": "decisions",
        "decisive": "decisive",
        "deferred": "deferred",
        "due {date}": "due {date}",
        "e.g. 1": "e.g. 1",
        "e.g. 10": "e.g. 10",
        "e.g. 3650": "e.g. 3650",
        "e.g. ALV Statute Amendment": "e.g. ALV Statute Amendment",
        "e.g. Attendance list incomplete": "e.g. Attendance list incomplete",
        "e.g. Contained an error, corrected version follows": "e.g. Contained an error, corrected version follows",
        "e.g. Corrected vote totals": "e.g. Corrected vote totals",
        "e.g. GroenLinks": "e.g. GroenLinks",
        "e.g. Off-topic or abusive": "e.g. Off-topic or abusive",
        "e.g. Prepare budget proposal": "e.g. Prepare budget proposal",
        "e.g. Roos de Vries": "e.g. Roos de Vries",
        "e.g. The vote count for item 5 should read 12 in favour": "e.g. The vote count for item 5 should read 12 in favour",
        "e.g. Vereniging De Harmonie": "e.g. Vereniging De Harmonie",
        "for": "for",
        "https://example.org/logo.png": "https://example.org/logo.png",
        "in force": "in force",
        "lapsed": "lapsed",
        "linked to recorded outcome": "linked to recorded outcome",
        "manual": "manual",
        "meetings": "meetings",
        "min": "min",
        "name@example.org": "name@example.org",
        "on {date}": "on {date}",
        "pending": "pending",
        "preparatory": "preparatory",
        "ratifying": "ratifying",
        "rejected": "rejected",
        "replaced": "replaced",
        "sample": "sample",
        "scheduled": "scheduled",
        "schema.org annotation matching the ORI type (ChooseAction / Event / CreativeWork).": "schema.org annotation matching the ORI type (ChooseAction / Event / CreativeWork).",
        "seq {n}": "seq {n}",
        "signature": "signature",
        "skipped": "skipped",
        "today": "today",
        "tomorrow": "tomorrow",
        "total": "total",
        "until {date}": "until {date}",
        "unverified — no recorded outcome": "unverified — no recorded outcome",
        "vote": "vote",
        "votes": "votes",
        "v{version}": "v{version}",
        "was due {date}": "was due {date}",
        "{actor}, with {substitute} standing in": "{actor}, with {substitute} standing in",
        "{completed} of {total} agenda items completed ({percent}%)": "{completed} of {total} agenda items completed ({percent}%)",
        "{count} attendees × {rate}/h": "{count} attendees × {rate}/h",
        "{count} decision": "{count} decision",
        "{count} decisions": "{count} decisions",
        "{count} members imported.": "{count} members imported.",
        "{decided} of {total} stages decided": "{decided} of {total} stages decided",
        "{level} signed at {when}": "{level} signed at {when}",
        "{m} min": "{m} min",
        "{n} agenda items": "{n} agenda items",
        "{n} already on Deck": "{n} already on Deck",
        "{n} attachment(s)": "{n} attachment(s)",
        "{n} card(s) created": "{n} card(s) created",
        "{n} conflict of interest declaration(s)": "{n} conflict of interest declaration(s)",
        "{n} item(s) could not be projected.": "{n} item(s) could not be projected.",
        "{n} meeting(s) exceeded the scheduled time": "{n} meeting(s) exceeded the scheduled time",
        "{n} open action items": "{n} open action items",
        "{responded} of {invited} responded": "{responded} of {invited} responded",
        "{states} states, {transitions} transitions": "{states} states, {transitions} transitions",
        "{verb} by {actor}": "{verb} by {actor}",
        "{verb} by {actor}, on behalf of {onBehalfOf}": "{verb} by {actor}, on behalf of {onBehalfOf}"
    },
    "nplurals=2; plural=(n != 1);"
)
