...
import { useReportStore } from '../../store/modules/report.js'
...
export default {
	...
	computed: {
		...
		subtitle() { return this.object?.reportType || '' },
		detailData() {
			if (!this.object) return []
			return [
				{ label: t('decidesk', 'Name'), value: this.object.name },
				{ label: t('decidesk', 'Report Type'), value: this.object.reportType },
				{ label: t('decidesk', 'Period'), value: this.object.period },
				{ label: t('decidesk', 'Generated At'), value: this.object.generatedAt },
			]
		},
	},
	async created() {
	...
	methods: {
		async loadObject() {
			...
			const objects = await store.fetchObjects('report')
