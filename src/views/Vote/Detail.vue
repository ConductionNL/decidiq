...
import { useVoteStore } from '../../store/modules/vote.js'
...
export default {
	...
	computed: {
		...
		subtitle() { return this.object?.castAt || '' },
		detailData() {
			if (!this.object) return []
			return [
				{ label: t('decidesk', 'Value'), value: this.object.value },
				{ label: t('decidesk', 'Weight'), value: this.object.weight },
				{ label: t('decidesk', 'Is Proxy'), value: this.object.isProxy },
				{ label: t('decidesk', 'Cast At'), value: this.object.castAt },
			]
		},
	},
	async created() {
	...
	methods: {
		async loadObject() {
			...
			const objects = await store.fetchObjects('vote')
