	...
	<CnDetailPage
		...
		<template #properties>
			<CnDetailCard :title="t('decidesk', 'Eigenschappen')">
				...
				<div v-if="object.taskStatus !== 'completed'" class="decidesk-transitions">
					<NcButton
...
export default {
	...
	computed: {
		...
		isOverdue() {
			if (!this.object.dueDate || this.object.taskStatus === 'completed') return false
			return new Date(this.object.dueDate) < new Date() || this.object.taskStatus === 'overdue'
		},
		propertyItems() {
			const statusLabel = {
			...
			return [
				{ label: this.t('decidesk', 'Status'), value: this.isOverdue ? this.t('decidesk', 'Verlopen') : (statusLabel[this.object.taskStatus] || this.object.taskStatus) },
	...
	methods: {
		...
		async startItem() {
			await this.objectStore.saveObject('action-item', { ...this.object, taskStatus: 'in-progress' })
