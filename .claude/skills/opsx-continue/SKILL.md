...
- **design.md**: Document technical decisions, architecture, and implementation approach. **MUST include a Seed Data section** (ADR-016) with realistic objects per schema — general organization data that works for municipalities, consultancies, or travel agencies. Research realistic field values. The apply agent generates `_registers.json` entries from this section.
