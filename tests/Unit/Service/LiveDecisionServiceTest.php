...
class LiveDecisionServiceTest extends TestCase {
	 ...
	 *
	 * ADR-084: the service takes OpenRegister's published contract directly,
	 * so the double is handed in rather than routed through the container.
	 *
	 * @param ObjectServiceInterface $objectService The object-service double.
	 *
	 * @return LiveDecisionService
	 */
	private function service(ObjectServiceInterface $objectService): LiveDecisionService {
	...
	public function testRecordDecisionCreatesDecisionAndLinksToMeeting(): void {
		...
		$result = $service->recordDecision('meeting-1', $decisionData, 'user-1');
	...
	public function testRecordDecisionThrows409ForNonOpenedMeeting(): void {
		...
		$service->recordDecision('meeting-1', $decisionData, 'user-1');
	}
