 ...
 *
 * @spec openspec/specs/register-seed-data/spec.md
 */
...
final class RegisterFragmentMergeTest extends TestCase
{
    ...
    public function testDisjointFragmentsUnionSchemasAndPaths(): void
    {
        ...
        $base = $this->merge(
                $base,
                [
                    'components' => ['schemas' => ['AlphaMotion' => ['type' => 'object']]],
                    'paths'      => ['/alpha' => ['get' => []]],
                ]
                );
        $base = $this->merge(
                $base,
                [
                    'components' => ['schemas' => ['BetaDecision' => ['type' => 'object']]],
                    'paths'      => ['/beta' => ['post' => []]],
                ]
                );

        $this->assertArrayHasKey('Existing', $base['components']['schemas']);
        $this->assertArrayHasKey('AlphaMotion', $base['components']['schemas']);
        $this->assertArrayHasKey('BetaDecision', $base['components']['schemas']);
