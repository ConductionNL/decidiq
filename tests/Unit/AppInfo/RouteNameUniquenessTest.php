 ...
 * declared=147 registered=147 lost=0. This test is what keeps it that way.
 *
 ...
 *
 * The two route contract tests beside this one cannot see it either.
 * `RouteDeclarationOrderContractTest` builds a `$positions` map keyed by name,
 * which collapses a duplicate pair into one entry before it is compared, and
 * `CanonicalRouteMethodContractTest` asks whether each canonical name is
 * present, which a surviving duplicate answers yes to.
 *
 ...
 * openregister is absent. Either way the entries being checked for collisions
 * are this app's own, which is what a `postfix` decision here can affect.
 */
