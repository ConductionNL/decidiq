...
use OCA\Decidiq\Repair\MigrateRegisterSlug;
use OCA\Decidiq\Repair\MigrateRegisterSlugDecisions;
use OCP\DB\IResult;
use OCP\IAppConfig;
 ...
 *
 * @covers \OCA\Decidiq\Repair\MigrateRegisterSlug
 * @covers \OCA\Decidiq\Repair\MigrateRegisterSlugDecisions
 *
 * @spec exclude No canonical spec covers the `decidesk` -> `decidiq` register-slug
...
final class MigrateRegisterSlugTest extends TestCase {
	...
	public function testRenamesARegisterThatIsPresent(): void {
		$db = $this->dbReturning([['slug' => 'decidesk']]);
		...
		self::assertSame([['decidiq', 'decidesk']], $written);
	...
	public function testIsIdempotent(): void {
		$db = $this->dbReturning([['slug' => 'decidiq']]);
	...
	public function testRefusesWhenTheTargetSlugAlreadyExists(): void {
		$db = $this->dbReturning([['slug' => 'decidesk'], ['slug' => 'decidiq']]);
		$db->expects(self::never())->method('executeStatement');
	...
	public function testSlugsToReadCoversBothSidesOfTheMap(): void {
		...
		self::assertContains('decidesk', $slugs);
		self::assertContains('decidiq', $slugs);
	...
	public function testSlugsFromToleratesMissingAndNullSlugs(): void {
		self::assertSame(
			['decidesk', '', ''],
			$this->decisions->slugsFrom(rows: [['slug' => 'decidesk'], ['slug' => null], []])
		);
	...
	public function testAStoredConfigValueOnTheOldSlugIsRePointed(): void {
		...
		$step = new MigrateRegisterSlug(
			$db,
			$this->appConfig('decidesk', $written),
		...
		self::assertSame([['register', 'decidiq']], $written);
	...
	public function testAConfigWriteFailureIsLoggedNotThrown(): void {
		$cfg = $this->createMock(IAppConfig::class);
		$cfg->method('getValueString')->willReturn('decidesk');
