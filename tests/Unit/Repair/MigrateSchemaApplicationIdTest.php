...
use OCA\Decidiq\Repair\MigrateSchemaApplicationId;
use OCP\DB\IResult;
 ...
 *
 * @covers \OCA\Decidiq\Repair\MigrateSchemaApplicationId
 *
...
final class MigrateSchemaApplicationIdTest extends TestCase {
	...
	public function testMovesASchemaThatHasNoTwin(): void {
		...
		self::assertSame(
			[['decidiq', 'decidesk', 'case'], ['decidiq', 'decidesk', 'task']],
			$written
		);
	...
	public function testRefusesASlugThatWouldCollide(): void {
		...
		$output = $this->createMock(IOutput::class);
		$output->expects(self::once())->method('info')->with(self::stringContains('1 schema(s) moved to decidiq, 1 refused'));

		$step->run($output);

		self::assertSame([['decidiq', 'decidesk', 'task']], $written);
