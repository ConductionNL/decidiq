<?php

/**
 * Unit tests for MigratePlanningCycles.
 *
 * @category Test
 * @package  OCA\Decidiq\Tests\Unit\Migration
 *
 * @author    Conduction Development Team <info@conduction.nl>
 * @copyright 2026 Conduction B.V.
 * @license   EUPL-1.2 https://joinup.ec.europa.eu/collection/eupl/eupl-text-eupl-12
 *
 * @link https://conduction.nl
 */

declare(strict_types=1);

namespace OCA\Decidiq\Tests\Unit\Migration;

use OCA\Decidiq\Migration\MigratePlanningCycles;
use OCA\Decidiq\Service\SettingsService;
use OCP\Migration\IOutput;
use PHPUnit\Framework\TestCase;
use Psr\Container\ContainerInterface;
use Psr\Log\LoggerInterface;

/**
 * The rename of the planning and control cycle into plain words.
 *
 * @spec openspec/changes/planning-cycle-in-plain-words/specs/planning-cycle-in-plain-words/spec.md
 */
class MigratePlanningCyclesTest extends TestCase {

	private SettingsService $settingsService;

	private ContainerInterface $container;

	private LoggerInterface $logger;

	private IOutput $output;

	private MigratePlanningCycles $migration;

	/**
	 * Set up test fixtures.
	 *
	 * @return void
	 */
	protected function setUp(): void {
		$this->settingsService = $this->createMock(originalClassName: SettingsService::class);
		$this->container       = $this->createMock(originalClassName: ContainerInterface::class);
		$this->logger          = $this->createMock(originalClassName: LoggerInterface::class);
		$this->output          = $this->createMock(originalClassName: IOutput::class);

		$this->migration = new MigratePlanningCycles(
			$this->settingsService,
			$this->container,
			$this->logger,
		);

	}//end setUp()

	/**
	 * A reference follows the record it points at.
	 *
	 * 🔴 THIS IS THE ONE THAT MATTERS. A step copied across still held its old
	 * cycle's identifier. Written unchanged it would live on the NEW schema while
	 * pointing at the RETIRED record: readable, plausible, and joined to the wrong
	 * side of the migration, with nothing to show for it in any list.
	 *
	 * @return void
	 *
	 * @spec openspec/changes/planning-cycle-in-plain-words/specs/planning-cycle-in-plain-words/spec.md#requirement-existing-cycles-are-carried-across
	 */
	public function testAStepPointsAtTheCopiedCycleNotTheRetiredOne(): void {
		$this->settingsService->method('isOpenRegisterAvailable')->willReturn(true);

		$service = $this->makeObjectService(
			templates: [['id' => 'ct-1', 'name' => 'Municipal', 'steps' => [['label' => 'Kadernota']]]],
			cycles: [['id' => 'pc-1', 'name' => 'Cyclus 2027', 'year' => 2027, 'template' => 'ct-1']],
			steps: [['id' => 'cs-1', 'cyclus' => 'pc-1', 'sequence' => 1, 'label' => 'Kadernota', 'status' => 'planned']],
		);
		$this->container->method('get')->willReturn($service);

		$this->migration->run(output: $this->output);

		$cycles = $service->savedFor('planning-cycle');
		$steps  = $service->savedFor('planning-cycle-step');
		self::assertCount(expectedCount: 1, haystack: $cycles);
		self::assertCount(expectedCount: 1, haystack: $steps);

		// 🔑 `cyclus` became `cycle`, the one property that was itself Dutch, and
		// it names the COPY rather than the source.
		self::assertArrayNotHasKey(key: 'cyclus', array: $steps[0]);
		self::assertSame(expected: $cycles[0]['id'], actual: $steps[0]['cycle']);
		self::assertNotSame(expected: 'pc-1', actual: $steps[0]['cycle']);

		// The same holds one level up: a cycle points at the copied template.
		self::assertSame(
			expected: $service->savedFor('planning-cycle-template')[0]['id'],
			actual: $cycles[0]['template'],
		);

	}//end testAStepPointsAtTheCopiedCycleNotTheRetiredOne()

	/**
	 * Every value survives the rename.
	 *
	 * @return void
	 *
	 * @spec openspec/changes/planning-cycle-in-plain-words/specs/planning-cycle-in-plain-words/spec.md#requirement-existing-cycles-are-carried-across
	 */
	public function testTheValuesSurviveTheRename(): void {
		$this->settingsService->method('isOpenRegisterAvailable')->willReturn(true);

		$service = $this->makeObjectService(
			cycles: [
				[
					'id' => 'pc-1',
					'name' => 'Cyclus 2027',
					'year' => 2027,
					'governanceBody' => 'gemeenteraad',
					'stepCount' => 6,
					'overdueStepCount' => 1,
				],
			],
		);
		$this->container->method('get')->willReturn($service);

		$this->migration->run(output: $this->output);

		$cycle = $service->savedFor('planning-cycle')[0];
		self::assertSame(expected: 'Cyclus 2027', actual: $cycle['name']);
		self::assertSame(expected: 2027, actual: $cycle['year']);
		self::assertSame(expected: 6, actual: $cycle['stepCount']);
		self::assertSame(expected: 1, actual: $cycle['overdueStepCount']);
		// The body reference is resolved, because the target declares format uuid
		// and a seeded row holds a slug.
		self::assertSame(expected: 'uuid-of-gemeenteraad', actual: $cycle['governanceBody']);
		self::assertSame(expected: 'pc-1', actual: $cycle['migratedFromObject']);

	}//end testTheValuesSurviveTheRename()

	/**
	 * A second run copies nothing.
	 *
	 * @return void
	 *
	 * @spec openspec/changes/planning-cycle-in-plain-words/specs/planning-cycle-in-plain-words/spec.md#requirement-existing-cycles-are-carried-across
	 */
	public function testASecondRunCopiesNothing(): void {
		$this->settingsService->method('isOpenRegisterAvailable')->willReturn(true);

		$service = $this->makeObjectService(
			cycles: [['id' => 'pc-1', 'name' => 'Al gedaan']],
			existing: ['planning-cycle' => [['id' => 'plc-1', 'migratedFromObject' => 'pc-1']]],
		);
		$this->container->method('get')->willReturn($service);

		$this->migration->run(output: $this->output);

		self::assertCount(expectedCount: 0, haystack: $service->savedFor('planning-cycle'));

	}//end testASecondRunCopiesNothing()

	/**
	 * Nothing runs when OpenRegister is unavailable.
	 *
	 * @return void
	 *
	 * @spec exclude Guard clause; asserts the migration is inert without OpenRegister.
	 */
	public function testNothingRunsWithoutOpenRegister(): void {
		$this->settingsService->method('isOpenRegisterAvailable')->willReturn(false);
		$this->container->expects(self::never())->method('get');

		$this->migration->run(output: $this->output);

	}//end testNothingRunsWithoutOpenRegister()

	/**
	 * A fake ObjectService recording what the migration writes.
	 *
	 * @param array<int,array<string,mixed>>              $templates Legacy templates.
	 * @param array<int,array<string,mixed>>              $cycles    Legacy cycles.
	 * @param array<int,array<string,mixed>>              $steps     Legacy steps.
	 * @param array<string,array<int,array<string,mixed>>> $existing Rows already copied, by schema.
	 *
	 * @return object The fake.
	 */
	private function makeObjectService(
		array $templates = [],
		array $cycles = [],
		array $steps = [],
		array $existing = [],
	): object {
		return new class($templates, $cycles, $steps, $existing) {
			/**
			 * The schema currently selected.
			 *
			 * @var string
			 */
			private string $currentSchema = '';

			/**
			 * Saves, as [schema, payload, id] triples.
			 *
			 * @var array<int,array{0:string,1:array<string,mixed>,2:string}>
			 */
			public array $saves = [];

			/**
			 * Constructor.
			 *
			 * @param array<int,array<string,mixed>>               $templates Legacy templates.
			 * @param array<int,array<string,mixed>>               $cycles    Legacy cycles.
			 * @param array<int,array<string,mixed>>               $steps     Legacy steps.
			 * @param array<string,array<int,array<string,mixed>>> $existing  Rows already copied.
			 *
			 * @return void
			 */
			public function __construct(
				private array $templates,
				private array $cycles,
				private array $steps,
				private array $existing,
			) {
			}//end __construct()

			/**
			 * Payloads saved for one schema, each carrying the id it was given.
			 *
			 * @param string $schema The schema slug.
			 *
			 * @return array<int,array<string,mixed>> The payloads.
			 */
			public function savedFor(string $schema): array {
				$out = [];
				foreach ($this->saves as $save) {
					if ($save[0] === $schema) {
						$out[] = ($save[1] + ['id' => $save[2]]);
					}
				}

				return $out;

			}//end savedFor()

			/**
			 * Run an operation as the system user.
			 *
			 * @param callable $operation The operation to run.
			 *
			 * @return mixed The operation's result.
			 */
			public function runAsSystem(callable $operation): mixed {
				return $operation();

			}//end runAsSystem()

			/**
			 * Select the register.
			 *
			 * @param string $register The register slug.
			 *
			 * @return void
			 */
			public function setRegister(string $register): void {
			}//end setRegister()

			/**
			 * Select the schema.
			 *
			 * @param string $schema The schema slug.
			 *
			 * @return void
			 */
			public function setSchema(string $schema): void {
				$this->currentSchema = $schema;

			}//end setSchema()

			/**
			 * Return the rows for the selected schema.
			 *
			 * @param array<string,mixed> $filters Slug lookups.
			 *
			 * @return array<int,array<string,mixed>> The rows.
			 */
			public function findAll(array $filters = []): array {
				$slug = (string)($filters['filters']['@self']['slug'] ?? '');
				if ($slug !== '') {
					return [['id' => 'uuid-of-' . $slug]];
				}

				return match ($this->currentSchema) {
					'cyclus-template' => $this->templates,
					'pc-cyclus' => $this->cycles,
					'cyclus-stap' => $this->steps,
					default => array_merge(
						($this->existing[$this->currentSchema] ?? []),
						$this->savedFor($this->currentSchema)
					),
				};

			}//end findAll()

			/**
			 * Record a save, and hand back an object carrying a NEW id.
			 *
			 * The id deliberately does not look like the source id, so a test can
			 * tell a retargeted reference from one that was copied verbatim.
			 *
			 * @param string              $register The register slug.
			 * @param string              $schema   The schema slug.
			 * @param array<string,mixed> $object   The payload.
			 *
			 * @return array<string,mixed> The saved object.
			 */
			public function saveObject(string $register, string $schema, array $object): array {
				$id = 'new-' . $schema . '-' . (count($this->saves) + 1);
				$this->saves[] = [$schema, $object, $id];

				return ($object + ['id' => $id]);

			}//end saveObject()
		};

	}//end makeObjectService()
}//end class
