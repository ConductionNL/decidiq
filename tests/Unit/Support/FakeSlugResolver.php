 ...
 * the tests that use it. What they check is the DIFFERENCE between a migrated
 * and an unmigrated instance, and that difference IS the resolver's behaviour. A
 * mock stubbed to return one slug behaves identically on both, so every test
 * built on one passes whether or not the code under test uses the answer.
 *
 * Only the register decidiq actually reads is listed. openregister owns the full
 * fleet map in `lib/Support/RegisterSlugAliases.php`, which is not published to
 * consumers; copying all ten here would put a second copy of OpenRegister's
 * truth in a repository that does not own it.
 */
final class FakeSlugResolver implements RegisterSlugResolverInterface {

	/**
	 * The slug history of the one register decidiq reads.
	 *
	 * @var array<string, list<string>>
	 */
	private const CANDIDATES = ['decidiq' => ['decidiq', 'decidesk']];
