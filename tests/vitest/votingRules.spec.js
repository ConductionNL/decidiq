...
describe('computeBase', () => {
	it('excludes abstentions by default (spec worked example: 14/5/1 -> 19)', () => {
		expect(computeBase({ votesFor: 14, votesAgainst: 5, votesAbstain: 1 })).toBe(19)
	})

	it('excludes abstentions when abstentionHandling is exclude', () => {
		expect(computeBase({ votesFor: 14, votesAgainst: 5, votesAbstain: 1, abstentionHandling: 'exclude' })).toBe(19)
	})

	it('counts abstentions toward the base when abstentionHandling is count', () => {
		expect(computeBase({ votesFor: 14, votesAgainst: 5, votesAbstain: 1, abstentionHandling: 'count' })).toBe(20)
	})
	...
	it('falls back to exclude for unknown abstention modes', () => {
		expect(computeBase({ votesFor: 2, votesAgainst: 1, votesAbstain: 5, abstentionHandling: 'bogus' })).toBe(3)
	})
})
...
describe('ruleLabels', () => {
	it('provides a label for every enum value through the translate function', () => {
		...
		VOTE_THRESHOLDS.forEach((value) => expect(labels.voteThreshold[value]).toBeTruthy())
		ABSTENTION_MODES.forEach((value) => expect(labels.abstentionHandling[value]).toBeTruthy())
		TIE_BREAK_RULES.forEach((value) => expect(labels.tieBreakRule[value]).toBeTruthy())
		// Every label went through t() so the strings are translatable.
		expect(seen.length).toBe(VOTE_THRESHOLDS.length + ABSTENTION_MODES.length + TIE_BREAK_RULES.length)
	})
})
