...
async function openProcessTemplates(page: Page): Promise<boolean> {
	...
	return section.waitFor({ state: 'visible', timeout: 15_000 }).then(() => true).catch(() => false)
}

// @e2e openspec/specs/process-configuration/spec.md#built-in-templates-are-read-only
test('Template list shows built-in templates as read-only (duplicate, no delete)', async ({ page }) => {
	test.skip(!(await openProcessTemplates(page)), 'process-templates admin section not deployed on this instance')

	const list = page.locator('[data-testid="process-template-list"]')
	await expect(list).toBeVisible({ timeout: 8_000 })

	const builtIn = page.locator('[data-testid="process-template-item"]').filter({
		has: page.locator('[data-testid="process-template-builtin"]'),
	}).first()
	test.skip(!(await builtIn.isVisible().catch(() => false)), 'no built-in templates seeded on this instance')

	// A built-in row offers Duplicate but neither Edit nor Delete.
	await expect(builtIn.locator('[data-testid="process-template-duplicate"]')).toBeVisible()
	await expect(builtIn.locator('[data-testid="process-template-edit"]')).toHaveCount(0)
	await expect(builtIn.locator('[data-testid="process-template-delete"]')).toHaveCount(0)
})

// @e2e openspec/specs/process-configuration/spec.md#create-a-process-template-for-alv-decisions
test('Create modal accepts a well-formed template and enables Save', async ({ page }) => {
	test.skip(!(await openProcessTemplates(page)), 'process-templates admin section not deployed on this instance')
	...
	await modal.locator('[data-testid="process-template-name"]').fill('E2E ALV Standard Decision')
	...
	await expect(modal.locator('[data-testid="process-template-errors"]')).toHaveCount(0)
	await expect(modal.locator('[data-testid="process-template-save"]')).toBeEnabled()
})

// @e2e openspec/specs/process-configuration/spec.md#reject-an-invalid-transition-graph
test('Create modal blocks Save when the transition graph is invalid', async ({ page }) => {
	test.skip(!(await openProcessTemplates(page)), 'process-templates admin section not deployed on this instance')
	...
	await modal.locator('[data-testid="process-template-name"]').fill('E2E Invalid Graph')
	...
	await expect(modal.locator('[data-testid="process-template-errors"]')).toBeVisible({ timeout: 5_000 })
	await expect(modal.locator('[data-testid="process-template-save"]')).toBeDisabled()
})
...
test('Duplicate action is available on every template row', async ({ page }) => {
	...
	test.skip(!(await firstRow.isVisible().catch(() => false)), 'no templates listed on this instance')

	await expect(firstRow.locator('[data-testid="process-template-duplicate"]')).toBeVisible()
})
