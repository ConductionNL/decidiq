...
- [ ] 1.1 Add `lib/Event/DecisionRequestedEvent.php` (`OCA\Decidesk\Event`, extends
...
- [ ] 1.2 Add `lib/Event/DecisionConcludedEvent.php` (`OCA\Decidesk\Event`, extends `Event`):
...
- [ ] 2.1 Add `lib/Listener/DecisionRequestedListener.php` (`OCA\Decidesk\Listener`, implements
...
- [ ] 2.2 Register `DecisionIntegrationService` as a DI service and `DecisionRequestedListener` as a
...
- [ ] 3.1 Inject `DecisionIntegrationService` + `OCP\EventDispatcher\IEventDispatcher` into
  `DecisionLifecycleService` (constructor + the `registerService` closure in `Application.php`).
- [ ] 3.2 In `DecisionLifecycleService::transition()`, after the successful `saveObject`, when the
...
- [ ] 4.1 `php -l` every new/changed PHP file; `openspec validate decidesk-decision-events --strict`
  passes.
- [ ] 4.2 Run the hydra mechanical gates (spdx, forbidden-patterns, stub-scan, spec-coverage on
...
- [ ] 4.3 Confirm the two event FQCNs + payload fields match the spec contract verbatim so the
