# Design — spec-anchor-repair (decidesk)

## Category breakdown (base `origin/development`)

1,199 broken `@spec` anchors, classified by recoverability:

| Category | Count | Disposition |
|---|---|---|
| **(c) archived change → canonical spec recoverable** | **519** | **auto-repointed** |
| &nbsp;&nbsp;↳ exact requirement-heading match | 62 | `specs/<cap>/spec.md#requirement-<slug>` |
| &nbsp;&nbsp;↳ capability proven, heading not matched | 457 | `specs/<cap>/spec.md` (file-level) |
| **(d)**  non-tasks.md ref (decimal task / design.md / proposal.md / re-headed specs anchor) | 500 | flag |
| **(d)**  change uses non-annotate tasks.md (no `task-N: cap#REQ` line) — needs spec-delta read | 172 | flag |
| **(d)**  archived change dir not located | 6 | flag |
| **(d)**  no-fragment tasks.md ref, slug not a canonical capability | 2 | flag |
| **Total flagged for human triage** | **680** | `residual-dangling.md` |

Auto-repointed **43%**; the residual 680 stay broken *on purpose*
...
4. **Post-condition gate** — every proposed target is re-checked with gate-46
   ...
   and the anchor stays dangling. **Observed on decidesk: 0 rejects.**
...
519 anchors is infeasible and more error-prone, and the safety property
...
The residual 680 anchors are **not** force-removed or re-pointed on a
