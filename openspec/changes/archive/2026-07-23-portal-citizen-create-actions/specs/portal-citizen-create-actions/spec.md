# portal-citizen-create-actions Specification (delta)
...
Decidesk lets an accountless **citizen** react to an open consultation and
propose a participatory-budget item from **portaliq** (hydra ADR-046,
contribution contract v2.2), through `type: create` actions whose scope is
server-stamped from the resolved subject. Today the `citizen` manifest ships
`actions: []` (`REQ-DKPORT-006`); this change declares the two primary citizen
creates with client-writable whitelists, server-stamped scope + intake state,
and an open-parent constraint — so a citizen can only ever create a submission
owned by themselves into a process that is actually open, and moderation stays
internal. This closes the write-IDOR class filed as portaliq#16.
