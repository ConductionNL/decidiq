...
- **Create the new `public-publication` capability**: staff-driven, per-object publication of adopted decisions, finalized meeting agendas, and approved minutes through the OpenRegister RBAC published-predicate (`publicatiedatum` + a public-group `authorization.read` rule on the published schema) plus routing into a configured OpenCatalogi catalog per governance body. No app-local anonymous pages or read endpoints — anonymous read access happens exclusively through OpenCatalogi's / OR's existing publication surface (identical posture to `citizen-participation`).
...
- **Backend**: thin publish/withdraw/rectify endpoints plus a `PublicationPayloadService` (eligibility check, payload derivation, PII strip, ORI mapping) and a `PublicationService` that sets `publicatiedatum`/`depublicatiedatum` on the derived payload and routes to the catalog. Plain CRUD stays on `useObjectStore` → OR object API per ADR-022.
...
- **Dependency**: OpenCatalogi (publication surface), OpenRegister RBAC published-predicate (`publicatiedatum` + a public-group `authorization.read` rule). No magic-mapper constraint applies: decidesk payloads are register-owned objects on the normal RBAC save path, so `publicatiedatum` is settable via the standard OR object API. (`@self.published` is deprecated/removed; the earlier magic-mapper-gap concern was a misdiagnosis.)
