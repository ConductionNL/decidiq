# Spec: Faction workspace via Collectives leaf

This file contains delta specifications for the migrate-workspaces-to-collectives-leaf change.
...
### Requirement: Collaboration workspace via in-app WorkspaceService
**Reason:** Superseded by the collectives integration leaf (REQ-WS-COLL-001). The in-app `WorkspaceService` and `CollaborationWorkspace` schema duplicated the ADR-019 collectives abstraction, a review-blocking anti-pattern under ADR-022.
**Migration:** Existing workspaces are migrated to collectives and archived (not purged) per REQ-WS-COLL-003.
