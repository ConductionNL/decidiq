...
- **Performance:** No requirement in this spec SHALL introduce an additional unbounded (no `_limit`) collection fetch.
- **Accessibility:** Reference-resolved labels (REQ-001) improve WCAG 2.2 AA SC 2.4.6 (Headings and Labels) / general comprehensibility by replacing opaque UUIDs with human-readable text; no regression to keyboard/screen-reader access is introduced by any requirement here.
- **Internationalization:** No new user-facing strings beyond existing column labels; Dutch/English parity is unaffected (ADR-007).

## Acceptance Criteria

- [ ] Every column identified by the item-1 sweep resolves a name instead of a raw id, for both UUID- and slug-stored references.
- [ ] P&C cycles Year and Kascommissie Boekjaar render as plain 4-digit years.
- [ ] Every column identified by the item-3 sweep renders through `NcDateTime` instead of a raw string.
- [ ] Delegations & mandates and Proxy authorizations reach a table or empty state within a bounded time on every load.
- [ ] The identified malformed governing-document seed object no longer shows "—" for its title (corrected or removed).
- [ ] At least the meeting/body/consultation/governing-document e2e spec files that directly caused the observed pollution create objects with the documented marker and clean them up.

## Notes

- **REQ-001, REQ-002, REQ-003, REQ-004** rely on capabilities (`fkResolve` widget, declarative `format` column hint, `config.subscribe`) already present in the pinned `@conduction/nextcloud-vue@2.3.0`. No library change is required for these four requirements.
- **REQ-004**'s root cause is NOT owned by this capability: it traces to `@conduction/nextcloud-vue`'s `liveUpdatesPlugin` (`store/plugins/liveUpdates.js`) dispatching a bare, unbounded `fetchCollection(type, {})` before the first real fetch stashes its params, combined with OpenRegister's `_facets=extend` (no `_limit`) query hanging server-side rather than completing or failing fast. `config.subscribe: false` is a decidesk-side mitigation of the *symptom* (the page reaching a terminal state), not a fix of the *cause* — the cause is filed against the two owning repos (see `ux-debt-rendering`'s design.md).
- **REQ-005** could not be reproduced against the current build (word-break/overflow-wrap are `normal`, ample container width at every tested viewport). The acceptance bar for this requirement is the regression guard (a Playwright assertion), not a demonstrated CSS fix — there is currently no defect to fix.
- **REQ-008** intentionally does not require retrofitting cleanup onto every one of the 33 spec files under `tests/e2e/spec-coverage/` — only the direct contributors to the observed pollution (meeting/body/consultation/governing-document creators) are required by `ux-debt-rendering`'s tasks; the convention itself applies going forward to all new specs.
