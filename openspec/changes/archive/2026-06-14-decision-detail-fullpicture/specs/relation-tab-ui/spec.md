...
This file contains delta specifications for the `decision-detail-fullpicture` change against the existing `relation-tab-ui` capability. The existing requirement covers parent-owned child CRUD tabs; this adds the complementary pattern for typed links between peer objects. This delta SUBSUMES the `relation-tab-ui` delta of the active `decision-relations` change.
