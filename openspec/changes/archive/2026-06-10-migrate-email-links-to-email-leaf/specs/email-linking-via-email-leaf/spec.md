# Spec: Email linking via Email leaf

This file contains delta specifications for the migrate-email-links-to-email-leaf change.
...
### Requirement: Email integration via in-app EmailLinkService
**Reason:** Superseded by the email integration leaf (REQ-EMAIL-LEAF-001). The in-app `EmailLink` store is the exact "parallel `{app}_email_links` link table" anti-pattern ADR-022 names as review-blocking, duplicating the ADR-019 email abstraction.
**Migration:** Existing `EmailLink` objects are relinked through the registry and archived (not purged) per REQ-EMAIL-LEAF-003. Vote-by-email casting is retained in the statutory voting path per REQ-EMAIL-LEAF-002.
