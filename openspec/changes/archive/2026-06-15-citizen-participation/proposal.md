...
- **Result publication via OpenCatalogi / the OR RBAC published-predicate**: when a round reaches `results-published`, the result summary objects get `publicatiedatum` set (and a public-group `authorization.read` rule on the schema makes them anonymously readable) and are routed to the configured OpenCatalogi catalog. **No app-local anonymous portal pages or `/api/citizens/*` read endpoints** — anonymous read access happens through OpenCatalogi's existing publication surface.
...
- **Dependency**: OpenCatalogi (publication surface); the OpenRegister RBAC published-predicate (`publicatiedatum` + a public-group `authorization.read` rule). Graceful degradation: without OpenCatalogi, `results-published` still sets `publicatiedatum`; the catalog routing step is skipped with a staff-visible warning.
