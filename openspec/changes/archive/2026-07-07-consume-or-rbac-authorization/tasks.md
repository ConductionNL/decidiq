...
- [x] Implement an idempotent per-body reconcile that maps the current chair/secretary/vice-chair roster to the two OR scopes, driven by a Participant/Membership write listener.
- [x] Test: chair→both scopes; secretary→signatory-only; removal reconciles; re-run no-op; empty scope denies.
...
- [x] Implement an idempotent repair step invoking the projector for every GovernanceBody.
- [x] Test: backfill populates scopes; re-run is a no-op.
...
- [x] Declare the write-gating RBAC rule on the signing-request property (minutes + resolution decision) keyed on `decidesk:body:{bodyId}:signatory`.
- [x] Route the controller's signing-request write through `ObjectService`; remove the `canInitiateSigning()` call; delete the service + registration.
- [x] Test: signatory allowed, non-signatory 403; gate reports clean.
...
- [x] Declare the write-gating RBAC rule on `meeting.lifecycle` keyed on `decidesk:body:{bodyId}:chair` for the `chairOnlyTransitions` set.
- [x] Remove the chair-auth actor branch from `MeetingService`; keep `allowPause`/`allowAdjourn`/`isQuorumRequired` and the quorum guard.
- [x] Test: chair vs non-chair; domain policy still forbids independent of actor.
...
- [x] Implement one shared admin guard consuming `PropertyRbacHandler::isAdmin()` / the OR admin decision; replace the four private copies.
- [x] Test: non-admin denied and admin allowed on all four surfaces; single implementation.
...
- [x] Add unit + e2e assertions for the deny-on-ambiguity paths; confirm no unsafe-auth-resolver pattern.
- [x] Test (UI): non-signatory blocked from signing; non-chair blocked from a chair-only transition; non-admin blocked on the four admin surfaces.
