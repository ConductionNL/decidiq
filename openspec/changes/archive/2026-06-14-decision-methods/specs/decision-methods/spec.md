# Spec: Decision Methods

## Purpose

A decision method is **how a `DecisionStage` reaches its outcome**. C4 (`decision-route-and-stages`) gave each stage a `method` enum placeholder; this capability makes each method real by wiring the stage to its resolution mechanism and deriving the stage `outcome` from it. The methods are: `vote` (resolved by a `VotingRound`; sub-variants via the round's `isSecret`/`votingMethod`), `chair-register` (the chair records the outcome directly), `signature` (eIDAS signing on a signed document, available to ANY decision per ADR-006), `advice` (non-binding advisory outcome), and `manual` (the default fallback). HOW the route is rendered and the per-stage resolve actions are owned by the later Cycle-2 UI change; this capability is register configuration plus a single eIDAS stage-resolution code seam per ADR-031/ADR-032.

**Standards**: eIDAS (QES signing), Popolo (`Person`, `Organization`), OpenRaadsinformatie (besluit *behandeltraject*), Akoma Ntoso (workflow steps), Schema.org (`VoteAction`)
