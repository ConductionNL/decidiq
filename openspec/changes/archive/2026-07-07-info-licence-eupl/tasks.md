...
- [x] Change `<licence>agpl</licence>` to `<licence>EUPL-1.2</licence>` (mirroring pipelinq at `min-version="28"`); do not change `min-version`/`max-version`.
- [x] Verify no other occurrence of `agpl` remains in `appinfo/info.xml`.
...
- [x] Bump `<version>` per the repository's versioning convention.
