...
**Depends on:** decidesk `decision-management` (ADR-005), `decision-route`, `decision-methods`
(eIDAS `method=signature`); hydra ADR-019 (integration-registry), ADR-022 (apps-consume-or-abstractions),
ADR-031 (schema-declarative-business-logic); docudesk signing capability (`signingRequest`/`signingSession`,
cross-app contract #2). Counterpart of `procest-delegate-contract-decision` and `shillinq-delegate-signing`.

## Purpose

Formalize decidesk as the fleet **contract-decision + sign-off hub**: a generic delegation target that
finance/case apps consume via the ADR-019 integration registry to raise a governance Decision, drive a
document signature (delegated to docudesk), and read back the outcome. This spec adds the integration
surface and a small additive schema delta ON TOP of the existing Decision model; it does not rebuild
the Decision supertype, routes, methods, or eIDAS wiring (those already exist — see Phase 0).
