...
This file contains delta specifications for the meeting-transcription-ai-minutes change.

**Entities (new):** Transcript (`schema:CreativeWork`, segments as `schema:Clip`-like timed text)
**Entities (existing):** Meeting, AgendaItem, Minutes (handoff only — see the resolution-minutes delta)
**Conventions:** storage/RBAC/notifications via OpenRegister (ADR-031 dialect); audio + transcript files in NC Files (meeting folder); transcription via the Nextcloud SpeechToText provider abstraction; draft generation via the Nextcloud TaskProcessing/AI provider abstraction; no app-local STT/LLM pipeline; recordings and transcripts never publicly published
