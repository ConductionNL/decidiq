...
on a mass scale — **1,199 in decidesk** on base `origin/development`, out of
1,945 total `@spec` tags (62%). A broken anchor is code *claiming* to
...
- **519 anchors repointed** across 80 files
  (62 to an exact requirement heading, 457 downgraded to
  capability-level file granularity).
- **680 anchors left untouched** and filed for human review
  (`residual-dangling.md` + umbrella issue) — never guessed.
- **gate-46 (`spec-anchor-existence`): 1,199 → 680 broken.**
- The diff is **`@spec`-comment-lines only**: 1,038 changed lines, **0**
...
- Gate-46 noise drops by 43%, so a *newly* broken anchor in a PR is now
  visible instead of drowning in 1,199 pre-existing failures.
