# Tasks — spec-anchor-repair (decidesk)

- [x] task-1: Measure broken `@spec` anchors repo-wide with gate-46 resolution logic (1,199 on base `origin/development`).
...
- [x] task-7: Apply the repointer — 519 anchors repointed across 80 files (62 anchor-level, 457 file-level).
- [x] task-8: Verify 0 repoint candidates rejected by the gate-46 post-condition check.
- [x] task-9: Comment-only proof — 0 non-`@spec` changed lines out of 1,038; 0 files with asymmetric insertions/deletions; 0 files touched outside `lib/`/`src/`.
- [x] task-10: Reconcile the tool's count against gate-46 exactly (`before − repointed == after`).
- [x] task-11: Gate-46 re-verify — broken **1,199 → 680**.
- [x] task-12: File the 680 residual-dangling anchors for human triage (`residual-dangling.md` + umbrella issue).
