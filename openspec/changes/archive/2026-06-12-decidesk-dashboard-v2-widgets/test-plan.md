...
| TC-003c | Outcome badge mapping (adopted/rejected/undecided) + publication badge (internal/public/confidential) | Functional (unit) | vitest | Real schema enums |
...
| TC | Scenario | Type | Command | Notes |
|----|----------|------|---------|-------|
| TC-004a | Chart renders two live series with >= 2 data points | Functional (browser) | `/test-functional` | Seed 2 meetings with materialized fields |
...
| TC-009e | No participant record for current user ⇒ count 0 | Functional (unit) | vitest | User decision: not a voting member, never over-count |
...
| TC-012b | Empty state renders "No pending votes" + checkmark | Functional (browser) | `/test-functional` | Also covers the no-participant-record case |
...
| TC | Scenario | Type | Command | Notes |
|----|----------|------|---------|-------|
| TC-013a | Count decisions with outcome == null | Functional (unit) | vitest | Mix of adopted/rejected/null outcomes |
| TC-013b | Click navigates to Decisions view | Functional (browser) | `/test-functional` | |
...
| REQ-009 Pending Votes KPI | TC-009a–c, TC-009e | TC-009d | — | Covered |
...
| REQ-013 Active Decisions KPI | TC-013a | TC-013b | — | Covered |
