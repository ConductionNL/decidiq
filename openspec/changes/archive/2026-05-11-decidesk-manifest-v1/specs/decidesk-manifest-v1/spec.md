...
status: draft
---
# Decidesk manifest v1 — JSON manifest renderer migration
...
Pages that render schema-backed list views — `GovernanceBodies`,
`Meetings`, `Participants`, `AgendaItems`, `Motions`, `Minutes`,
`Decisions`, `ActionItems` — MUST declare `type: "index"` (not
...
`GovernanceBodyDetail`, `MeetingDetail`, `ParticipantDetail`,
`AgendaItemDetail`, `MotionDetail`, `AmendmentDetail`,
`MinutesDetail`, `DecisionDetail`, `ActionItemDetail` — MUST declare
...
The per-page Vue files for the 18 migrated pages
(`Dashboard.vue` through `ActionItemDetail.vue`, excluding
`LiveMeeting.vue` and `SettingsView.vue`) MUST stay in place in
