# Context Brief: Motion and Voting — Other T1
...
**Spec:** p2-motion-and-voting-other-t1
...
**demand: 226** | Category: general
Discovered from GitHub issue in traefik/traefik

### Conduct secret ballot for board election
**demand: 126** (42 tender mentions) | Category: other

### Motion Status Management with Custom Metadata Configuration
**demand: 119** (39 tender mentions) | Category: other

### proxy-protocol-udp
**demand: 100** | Category: general
Discovered from GitHub issue in traefik/traefik

### Voter and Independent Vote Review and Recount
**demand: 58** (15 tender mentions) | Category: other

### Mobile Voter Authentication and Vote Submission
**demand: 40** (13 tender mentions) | Category: other

### Real Time Ballot Distribution
**demand: 40** (1 tender mentions) | Category: other

### Preferential Ballot
**demand: 26** | Category: other

### Internet Voting Platform for Online Vote Casting
**demand: 25** (8 tender mentions) | Category: other

### conflict-resolution
**demand: 21** | Category: general
Discovered from GitHub issue in payloadcms/payload

### Deliberative Polls
**demand: 17** | Category: other

### Proposals & Amendments
**demand: 16** | Category: other

### Feature request: LDAP vs Active Directory (Proxy Addresses)
**demand: 11** (3 tender mentions) | Category: other

### person-resolution
**demand: 10** | Category: general
Discovered from GitHub issue in PostHog/posthog

### Proxy Handling
**demand: 9** | Category: other

### Conduct formal votes with configurable majority rules
**demand: 9** (3 tender mentions) | Category: other

### reverse-proxy
**demand: 8** | Category: general
Discovered from GitHub issue in Stirling-Tools/Stirling-PDF
...
### reduced-motion
**demand: 7** | Category: general
Discovered from GitHub issue in nextcloud/server

### Custom Ballot Design
**demand: 7** (1 tender mentions) | Category: other

### proxy-resource-type
**demand: 7** | Category: general
Discovered from GitHub issue in appwrite/appwrite

### Voter Anonymity With Authorization Control
**demand: 7** (2 tender mentions) | Category: other

### Participants > Group rights: New and improved group rights for managing polls and ballots.
**demand: 5** (1 tender mentions) | Category: other
...
## User Stories (13 linked)

### Story 1: Anonymous Submissions
**Priority:** wont
As a council clerk, I want to have anonymous submissions capabilities, so that the platform meets diverse organizational needs.

### Story 2: Formal Verification
**Priority:** wont
As a council clerk, I want to have formal verification capabilities, so that the platform meets diverse organizational needs.

### Story 3: Conflict of Interest Rules
**Priority:** wont
As a compliance officer, I want to have conflict of interest rules capabilities, so that organizational risks are identified and managed proactively.

### Story 4: Provide auditor access
**Priority:** should-have
As an accountant, I want to give the auditor read-only access, so that they can review independently

### Story 5: Conduct live voting at AGM
**Priority:** must
...
### Story 6: Set up hybrid/digital AGM
**Priority:** must
As a board secretary, I want to configure a hybrid or fully digital AGM with identity verification, live voting, and Q&A, so that all shareholders can participate equally regardless of location.

**Acceptance Criteria:**
["Video conferencing integration", "Digital identity verification for attendees", "Real-time Q&A queue management", "Equal speaking rights enforcement"]

### Story 7: Conduct formal votes with configurable majority rules
**Priority:** must
As a council clerk or board secretary, I want to conduct formal votes (for/against/abstain) with configurable majority rules (simple, absolute, qualified 2/3), real-time vote counting, and an auditable record of all votes, so that decisions are legally valid and transparent.

**Acceptance Criteria:**
["Vote types: for/against/abstain/blank", "Majority rules configurable (simple, absolute, 2/3, unanimous)", "Real-time vote count and result display", "Secret ballot option available", "Vote record auditable and exportable", "Quorum check before vote enabled", "Works for physical, hybrid, and remote meetings"]

### Story 8: Configure required DigiD assurance level
**Priority:** must
As an iam-specialist, I want to configure the minimum DigiD assurance level (Basis, Midden, Substantieel) required for a service, so that citizens are forced to authenticate at the appropriate level.

**Acceptance Criteria:**
GIVEN a DigiD connector is active for a service
WHEN I set the minimum assurance level to 'Substantieel'
THEN the AuthnRequest sent to DigiD includes the correct LOA URI, and any authentication response with a lower level causes the connector to return an authorisation denied response

### Story 9: Provision user access from LDAP or AD group
**Priority:** must
As an IAM administrator, I want to map an LDAP or Active Directory group to a register role, so that users are automatically provisioned when they join the group in the directory.

**Acceptance Criteria:**
GIVEN an LDAP group is mapped to a register role
WHEN a new user is added to that LDAP group
THEN at their next login to Nextcloud, they are automatically granted the mapped register role

GIVEN a user is removed from the LDAP group
WHEN they next log in
THEN their register role is revoked and they can no longer access the register

### Story 10: Send test email before campaign launch
**Priority:** must
As an email marketer, I want to send a test email to myself or a small group before launching the full campaign, so that I can verify content, links, and rendering.

**Acceptance Criteria:**
Test send to one or multiple addresses; merge tags populated with sample data; test emails clearly marked as test; link validation report included

### Story 11: Conduct open vote on agenda item
**Priority:** critical
As chair, I want to conduct an open vote (for/against/abstain) on an agenda item and see results in real-time so that I can announce the outcome immediately.

**Acceptance Criteria:**
["Start/stop voting for specific item", "Real-time vote count", "Include proxy votes", "Calculate simple majority", "Display result to attendees"]

### Story 12: Use Nextcloud auth
**Priority:** critical
As an IT admin, I want CRM to use Nextcloud authentication for SSO

**Acceptance Criteria:**
Nextcloud auth, user provisioning, group mapping

### Story 13: Rate-limit and monitor portfolio API usage
**Priority:** could
As an ict-architect, I want API rate limiting and usage monitoring in place, so that excessive consumption by one client does not degrade planiq performance for other users.

**Acceptance Criteria:**
GIVEN a client exceeds the configured request rate limit WHEN the limit is breached THEN the API returns HTTP 429 with a Retry-After header
GIVEN the API has been active for a period WHEN I view the API monitoring dashboard THEN I see request counts, error rates, and average response times per registered client
