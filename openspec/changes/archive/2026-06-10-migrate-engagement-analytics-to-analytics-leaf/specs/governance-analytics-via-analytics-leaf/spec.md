# Spec: Governance analytics via Analytics leaf

This file contains delta specifications for the migrate-engagement-analytics-to-analytics-leaf change.
...
### Requirement: Participant engagement and voting-behaviour dashboards via in-app chart services
**Reason:** Superseded by the analytics integration leaf (REQ-AN-LEAF-001). In-app chart rendering for engagement, action-item, and voting-behaviour dashboards duplicated the ADR-019 analytics abstraction, a review-blocking anti-pattern under ADR-022.
**Migration:** No object migration — analytics reads existing OR data. Chart components are removed; governance-specific calculations are retained in-app and fed to the leaf per REQ-AN-LEAF-002.
