...
The decidesk-specific `useSettingsStore`
(`src/store/modules/settings.js`) MUST remain in place, since it talks
...
The four orphan store modules `src/store/modules/{meetings,minutes,
decisions,actionItems}.js` MUST be removed from the source tree, since
