...
3. **Per-body statutory cap**: `maxProxiesPerHolder` on the body's process/voting configuration, superseding the global app config; validated at proxy registration AND at round open; the global `decidiq`/`max_proxies_per_holder` config remains the fallback so all existing voting-system cap scenarios stay true unchanged when no body value is set; fail-closed semantics kept.
...
- **DigiD/eHerkenning identity assurance levels** — which eIDAS assurance level a signature carries is the signing provider's concern; decidiq records the outcome only.
