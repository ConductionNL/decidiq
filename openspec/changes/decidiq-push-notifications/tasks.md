# Tasks: Decidiq web-push notification rules (title-vs-body model)
...
- [x] 1.1 Add a `meetingStartingSoon` rule to the Meeting `x-openregister-notifications` block in
  ...
  `filter {lifecycle: "scheduled"}`; `originApp: "decidiq"`; `channels: ["nc-notification",
  ...
  `{nl:"{{title}} begint in Decidiq. Openen?", en:"{{title}} is starting in Decidiq. Open it?"}`;
...
- [x] 2.1 Add an `actionItemAssignedToYou` rule to the ActionItem `x-openregister-notifications` block:
  trigger `created`; `originApp: "decidiq"`; `channels: ["nc-notification", "web-push"]`; recipients
  `[{kind:"field", field:"assignee"}]`; `subject` `{nl:"Aan jou toegewezen: {{title}}",
  en:"Assigned to you: {{title}}"}`; `message` `{nl:"Dit actiepunt staat in Decidiq. Openen?",
  en:"This action item is in Decidiq. Open it?"}`; one primary `action`
...
- [x] 3.1 Add a `voteRequested` rule to the Decision `x-openregister-notifications` block: trigger
  ...
  "decidiq"`; `channels: ["nc-notification", "web-push"]`; recipients `[{kind:"object-acl",
  permission:"read"}]`; `subject` `{nl:"Jouw stem is gevraagd: {{title}}", en:"Your vote is requested:
  {{title}}"}`; `message` `{nl:"Dit besluit staat in Decidiq. Openen?", en:"This decision is in
  Decidiq. Open it?"}`; one primary `action` `{label:{nl:"Besluit openen", en:"Open decision"},
...
- [x] 4.2 `openspec validate "decidiq-push-notifications" --type change --strict`; fix any errors.
