...
# Proposal: Decidiq web-push notification rules (title-vs-body model)

## Problem

Decidiq's existing notification rules (`decidesk-notifications`) only deliver over the in-app
`nc-notification` channel and only carry a `subject`. The three most time-sensitive decidiq events —
...
deeplinks. Decidiq already registers deeplinks for `meeting`, `action-item`, and `decision`, so an
"Open X" action resolves. Nothing consumes these features yet in decidiq.
...
`["nc-notification", "web-push"]`, `originApp` to `decidiq`, a `subject` (title) + `message` (body)
...
{{title}}", body "{{title}} is starting in Decidiq. Open it?", primary action "Open meeting".
...
per the brief. Title "Assigned to you: {{title}}", body "This action item is in Decidiq. Open it?",
...
Title "Your vote is requested: {{title}}", body "This decision is in Decidiq. Open it?", primary
