...
The dashboard MUST include a `RecentDecisionsWidget` component showing the latest N decisions (default 10) with outcome badge and publication status badge.
...
- AND each decision SHALL show its title, an outcome badge (approved/rejected/tabled/amended), and a publication status badge
...
The dashboard MUST include a `GovernanceHealthWidget` component that renders a chart showing `quorumPercentage` and `actionItemCompletionRate` from recent meetings' materialized fields.
...
- THEN it SHALL render an ApexCharts time-series chart with up to 12 recent meetings on the x-axis
- AND plot `quorumPercentage` as one series and `actionItemCompletionRate` as another
...
- GIVEN the user has voted in all open voting-rounds or none exist
- WHEN the widget loads
