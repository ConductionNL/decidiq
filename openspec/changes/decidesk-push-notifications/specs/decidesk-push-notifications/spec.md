# decidesk-push-notifications Specification
...
channel + `actions` deeplink targets in the `x-openregister-notifications` dialect); decidesk
...
decidesk only declares the rules in `lib/Settings/decidesk_register.json`.

## Purpose

Add **web-push** notification rules to decidesk using the title-vs-body content model so that the three
most time-sensitive decidesk events reach a participant's device even when no Nextcloud tab is open:
...
triggering object directly. All existing decidesk notification rules are preserved unchanged.
...
- `actions` (array, max 2; each `{label:{nl,en}, primary, target}`) → tappable buttons. `target.kind`
  of `object-detail` (with no `object`) deeplinks to the triggering object; decidesk has registered
  deeplinks for `meeting`, `action-item`, and `decision`, so "Open X" resolves.
- `channels` includes both `nc-notification` and `web-push`; `originApp` is `decidesk`.
- Placeholders use the safe `{{<field>}}` form already used by decidesk's existing rules (`{{title}}`).
...
that delivers over both `nc-notification` and `web-push` with `originApp` `decidesk`. The rule SHALL use
...
and its `message` SHALL be `{nl:"{{title}} begint in Decidesk. Openen?", en:"{{title}} is starting in
Decidesk. Open it?"}`. The rule SHALL carry exactly one primary `action` labelled
...
- THEN a notification with title "Meeting starts soon: <title>" and body "<title> is starting in
  Decidesk. Open it?" is delivered over `nc-notification` and `web-push`, carrying a primary "Open
...
rule that delivers over both `nc-notification` and `web-push` with `originApp` `decidesk`. The rule SHALL
...
en:"Assigned to you: {{title}}"}` and its `message` SHALL be `{nl:"Dit actiepunt staat in Decidesk.
Openen?", en:"This action item is in Decidesk. Open it?"}`. The rule SHALL carry exactly one primary
...
- THEN a notification with title "Assigned to you: <title>" and body "This action item is in Decidesk.
...
delivers over both `nc-notification` and `web-push` with `originApp` `decidesk`. Because the canonical
dialect in this register expresses lifecycle transitions as an `updated` trigger with a `condition`
(no `transition` trigger is used in decidesk), the rule SHALL fire on `updated` with a `condition`
...
en:"Your vote is requested: {{title}}"}` and its `message` SHALL be `{nl:"Dit besluit staat in Decidesk.
Openen?", en:"This decision is in Decidesk. Open it?"}`. The rule SHALL carry exactly one primary
