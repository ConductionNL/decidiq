...
Consumes OpenRegister ObjectService/FileService abstractions only (ADR-022); no changes to other apps. Docudesk is NOT required — unlike PDF generation (which delegates to Docudesk's PdfService when resolvable), scanning is decidesk-local so the compliance gate works on every install.
