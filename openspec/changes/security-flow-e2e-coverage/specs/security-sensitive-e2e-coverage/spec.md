...
- **WHEN** a Playwright test issues `POST /api/proxies` as A, then `POST /api/proxies/{id}/revoke`
  as A
...
- **WHEN** a Playwright test issues `POST /api/proxies/{id}/revoke` authenticated as an unrelated
  participant C
...
- **WHEN** a Playwright test issues `POST /api/eidas/validate-cert`
...
- **WHEN** a Playwright test issues a request to any eIDAS endpoint
...
- **WHEN** a Playwright test issues `POST /api/governance-reports` and `POST
  /api/regulator-exports`
- **THEN** both requests succeed and return a reference to the generated artifact

#### Scenario: Unauthenticated request is rejected
- **GIVEN** no authenticated session
- **WHEN** a Playwright test issues a request to either route group
