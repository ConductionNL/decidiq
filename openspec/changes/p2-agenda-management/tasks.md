...
- [x] 1.4 Register `AgendaService` and `AgendaController` in DI container (`lib/AppInfo/Application.php`) — Nextcloud DI auto-wires constructor-injected services; no manual registration required
...
- [x] 8.1 Add Dutch (nl) translation keys for all new user-visible strings in `l10n/nl.json`: agenda builder labels, BOB phase names (Beeldvorming, Oordeelsvorming, Besluitvorming), hamerstuk labels, COI dialog copy, publish/revise button labels, notification messages
...
- [ ] 9.3 Write Playwright browser tests for REQ-BLD-002 (drag-drop reorder persisted), REQ-PUB-001 (publish triggers notification), REQ-LIV-002 (BOB phase advances), REQ-LIV-003 (hamerstukken batch adopt), REQ-COI-001 (COI declaration saved as note), REQ-COI-003 (motion linked to item) — **deferred to #36**
