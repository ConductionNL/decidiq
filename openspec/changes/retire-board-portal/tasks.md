...
- **files**: `lib/Settings/decidesk_register.json`
- **acceptance_criteria**:
  - GIVEN the `GovernanceBody` schema WHEN a `mode=corp` seed `raad-van-commissarissen-acme-bv` (`bodyType=corporate-board`) is added THEN it validates against the schema
...
- [ ] `openspec validate retire-board-portal --strict` passes
- [ ] App boots; nav renders without Boards / Board meetings / Resolutions; unified search returns decisions + meetings
- [ ] Repo-wide grep confirms no live reference to a deleted schema/route/view remains

## Quality checklist

- All new/changed business logic covered by PHPUnit unit tests (`tests/Unit/`)
- New/changed (retargeted) API endpoints covered by Newman/Postman tests
- UI changes (nav shrink) covered by Playwright browser tests
- All tests pass (`composer test`, `newman run`)
- Feature documentation in `docs/` updated to drop the board portal if user-facing (ADR-010)
- Dutch (`nl_NL`) and English (`en_US`) strings removed for deleted board surfaces; corp seed strings added (ADR-007)
