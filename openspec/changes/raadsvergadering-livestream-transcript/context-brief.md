...
- **decidiq** (base) — eigenaar van `Vergadering`, `Agendapunt`, `Besluit`, `Stemming`, `Fractie`, `Persoon`. De nieuwe schema's `Livestream`, `Transcript`, `TranscriptSegment` en `Spreker` worden in het decidesk-register geregistreerd.
