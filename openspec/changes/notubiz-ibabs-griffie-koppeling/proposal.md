...
- `ExternalIdentifier` — polymorph koppeling tussen decidiq-object en vendor-id. Velden: `localObjectType`, `localObjectId`, `provider`, `externalId`, `externalUrl`, `externalEtag`, `lastSyncedAt`, `syncDirection`, `lastModifiedLocal`, `lastModifiedExternal`.
...
- **sync-push** — wijzigingen in decidiq-velden teruschrijven naar provider (per `(org, provider, objecttype)` configureerbaar).
...
2. **sync-push** — Push decidiq changes back to provider (writable fields only, configurable per org/provider/object-type).
...
None. This change is purely additive in openconnector; decidiq schema extensions are additive.
...
- `decidiq/lib/Settings/decidesk_register.json` — extend `Meeting`, `AgendaItem`, `Person`, etc. with sync metadata (fk to ExternalIdentifier).
...
- `decidiq` data model — schemas unchanged; sync fills them.
...
| **Conflict explosion during simultaneous edits.** Griffier + decidiq-user edit same field → conflict queue explodes. | Per-veld UI locking hints; SSE reactive updates; conflict-quota alert (>10/week). |
