...
The pre-adoption health route is `GET|OPTIONS /apps/decidesk/api/v1/health` (route names `health#status`/`health#statusOptions`), not the AppHost-standard `/api/health`. Decidesk SHALL keep `/api/v1/health` (both verbs) as `$extra` alias routes targeting the decidesk `HealthController` for a documented deprecation window, in addition to exposing the standard `/api/health` (route `health#index`). Alias removal SHALL be a tracked follow-up gated on consumer migration.
